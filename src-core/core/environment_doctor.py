...
def check_platform_tools(project_root: Path) -> dict[str, Any]:
    platform_root = project_root / "platform_tools"
    ...
    for tool_dir in sorted(platform_root.iterdir(), key=lambda item: item.name.casefold()):
...
def collect_environment_report(
    ...
    *,
    ...
    platform_tools = check_platform_tools(root)
    ...
    if not platform_tools["ok"]:
        failures.extend(
            f"platform tool missing entry: {item['tool']}"
            for item in platform_tools["missing_entries"]
        )
        failures.extend(
            f"platform tool invalid manifest: {item['tool']}"
            for item in platform_tools["invalid_manifests"]
        )
        failures.extend(
            f"platform tool missing executable: {tool_id}"
            for tool_id in platform_tools["missing_executables"]
        )
    ...
    if platform_tools["missing_entries"]:
        recommendations.append("Fix platform tool runtime.entry paths before packaging.")
...
def format_environment_report(report: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"Platform tools: {'OK' if report.get('platform_tools', {}).get('ok') else 'FAIL'}",
    ]
