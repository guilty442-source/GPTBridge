...
class AiCollaborationBrowserSession:
    def __init__(
        self,
        profile_name: str = "ai-collaboration",
    ...
    async def ensure_agent_page(
        self,
        agent: dict[str, Any],
        *,
        background: bool = BACKGROUND_BROWSER_MODE,
    ) -> Page:
        ...
        async with self._page_lock:
            context = await self._ensure_context(headless=background)
    ...
    async def open_agent(self, agent: dict[str, Any]) -> dict[str, Any]:
        page = await self.ensure_agent_page(agent, background=False)
        await page.bring_to_front()
        return {"ok": True, "url": page.url}

    async def send_prompt(self, agent: dict[str, Any], prompt: str) -> dict[str, str]:
        page = await self.ensure_agent_page(agent, background=True)
    ...
    async def close_background_context(self) -> None:
        """Release headless browser processes after a background request."""

        if self.context is None or self._context_headless is not True:
            return
        try:
            await self.context.close()
        except Exception:
    ...
    async def _ensure_context(self, *, headless: bool = True) -> BrowserContext:
        if (
            ...
            and self._context_is_usable()
            and self._context_headless is headless
        ):
            return self.context
        if self.context is not None:
            try:
