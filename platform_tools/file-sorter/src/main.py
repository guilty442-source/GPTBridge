...
from sorter_v2 import (  # noqa: E402
...
LEGACY_RULES_MIGRATION_INBOX_DIR = (
    CURRENT_DIR.parent / "runtime" / "legacy-rule-migration-inbox"
)
...
def read_custom_rules(
    ...
    *,
    ...
    if target_dir is None or _uses_explicit_legacy_rules_path():
    ...
    return [
        KeywordRule(keyword=item["keyword"], folder=item["folder"])
...
def write_custom_rules(
    ...
    *,
    ...
    if target_dir is not None and not _uses_explicit_legacy_rules_path():
        ...
        except (RuleConflictError, SorterV2Error) as error:
            raise FileSorterError(str(error)) from error
...
def _effective_state_root(
    ...
    if state_root is not None or not _uses_explicit_legacy_rules_path():
        return state_root
    return get_rules_path().parent / ".file-sorter-v2-state"
...
def run_enabled_profiles_once(
    *,
    ...
    """Run one safe watcher pass; suitable for a scheduler/background service.

    Automatic moves require the same size/mtime fingerprint in two consecutive
    passes.  The normal quiet-period check still applies before a file enters
    the observation set.
    """
    ...
    for snapshot in list_profiles(state_root=effective_root):
        if snapshot.migration_required_review or not snapshot.enabled:
            continue
        ...
        try:
            ...
            with _BACKGROUND_OBSERVATIONS_LOCK:
                previous_observations = _BACKGROUND_OBSERVATIONS.get(
                    observation_key,
                    {},
                )
        ...
        except (FileSorterError, SorterV2Error) as error:
            reports.append(
                {
                    "ok": False,
                    "profile_id": snapshot.profile_id,
    ...
    with _BACKGROUND_OBSERVATIONS_LOCK:
        for observation_key in list(_BACKGROUND_OBSERVATIONS):
            if observation_key[0] == root_key and observation_key not in active_keys:
