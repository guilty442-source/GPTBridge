...
def repair_tool() -> dict[str, Any]:
    ...
    data_repair: dict[str, Any] = {}
    local_ai_services = tool_root / "src" / "backend" / "services"
    ...
    try:
        ...
        repository = LocalAiRepository(tool_root)
        data_repair = repository.repair_data(vacuum=True)
    except (OSError, RuntimeError, sqlite3.DatabaseError) as error:
        errors.append(f"local-ai-data: {type(error).__name__}")
