...
export function useInvestmentWatchFeature({
      ...
      const pending = (async () => {
        try {
          ...
          if (result.state_revision) {
            stateRevisionRef.current = result.state_revision
          }
          if (result.mobile_sync) setMobileSync(result.mobile_sync)
          if (result.market_sessions) {
            setInvestmentState((current) => {
              const before = (current.market_sessions?.open_markets || []).join(',')
              const after = (result.market_sessions?.open_markets || []).join(',')
              return before === after
                ? current
                : { ...current, market_sessions: result.market_sessions }
            })
          }
  ...
  const syncOpenMarkets = useCallback((): Promise<void> => {
    ...
    const pending = (async () => {
      try {
        ...
        if (result.not_modified || !result.state) return
        setInvestmentState(result.state)
        setInvestmentDiagnostics(investmentDiagnosticsFromResult(result))
      } catch {
  ...
  const readPortfolioFile = useCallback(async () => {
    ...
    try {
      ...
      setInvestmentState(result.state || {})
      setInvestmentDiagnostics(investmentDiagnosticsFromResult(result))
      setMobileSync(result.mobile_sync || null)
