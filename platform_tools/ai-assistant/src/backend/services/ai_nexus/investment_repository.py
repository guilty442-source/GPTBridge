...
try:
    from .investment_privacy import decode_json_document, encode_json_document
    ...
    _privacy_path = Path(__file__).with_name("investment_privacy.py")
    ...
    _contract_path = Path(__file__).with_name("investment_contract.py")
...
class InvestmentWatchRepository:
    ...
    def clear_state(self) -> dict[str, Any]:
        with self._state_lock:
            return self._clear_state_locked()

    def _clear_state_locked(self) -> dict[str, Any]:
        remote_url = self.mobile_sync_remote_url()
        current = self.load_state()
