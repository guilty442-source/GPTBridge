...
from .investment_analytics import InvestmentAnalyticsStore, parse_datetime, utc_now, utc_text
from .investment_privacy import protect_text, unprotect_text
...
class NotificationManager:
    ...
    def _send(self, channel: dict[str, Any], config: dict[str, Any], title: str, body: str) -> None:
        ...
        if channel_type == "email":
            ...
            if not host or not config.get("from") or not config.get("to"):
                raise ValueError("SMTP 設定不完整")
            with smtplib.SMTP(host, int(config.get("port") or 587), timeout=12) as smtp:
        ...
        if channel_type in {"teams", "slack"}:
            ...
            if not webhook.startswith("https://"):
                raise ValueError("Webhook 必須使用 HTTPS")
            payload = {"text": f"{title}\n{body}"}
            request = Request(webhook, data=_json(payload).encode("utf-8"), headers={"Content-Type": "application/json"})
            with urlopen(request, timeout=12) as response:
                if int(response.status) >= 300:
                    raise ValueError(f"Webhook 回應 {response.status}")
            return
