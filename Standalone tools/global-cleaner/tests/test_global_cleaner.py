...
ROOT = Path(__file__).resolve().parents[3]
CLEANER_SRC = ROOT / "Standalone tools" / "global-cleaner" / "src"
...
def test_daily_trigger_is_maintenance_owned_and_uses_governed_shared_channel(
    tmp_path: Path,
) -> None:
    ...
    app = SimpleNamespace(
        ...
        decision_sovereign_service=SimpleNamespace(
    ...
    assert service.status()["owner"] == "health-maintenance-test-sub-sovereign"
...
def test_direct_mutation_cli_is_denied_without_governed_requester(
    ...
    completed = subprocess.run(
        ...
        cwd=str(ROOT / "Standalone tools" / "global-cleaner"),
        env=environment,
        check=False,
        capture_output=True,
        text=True,
...
ROOT = Path(__file__).resolve().parents[3]
TOOL_ROOT = ROOT / "Standalone tools" / "ai-collaboration"
SERVICES = TOOL_ROOT / "src" / "backend" / "services"
...
AI_COLLABORATION_SERVICES = (
    ROOT / "Standalone tools" / "ai-collaboration" / "src" / "backend" / "services"
)
...
ROOT = Path(__file__).resolve().parents[3]
TOOL_ROOT = ROOT / "Standalone tools" / "vaultly"
SERVICES = TOOL_ROOT / "src" / "backend" / "services"
