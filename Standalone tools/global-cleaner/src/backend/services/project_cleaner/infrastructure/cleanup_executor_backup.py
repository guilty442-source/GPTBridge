...
class CleanupBackupMixin:
        ...
        def _backup_source_files(self, owner_root: Path) -> Iterator[tuple[Path, str]]:
            ...
            for current_raw, directory_names, file_names in os.walk(
                ...
                directory_names[:] = [
                    ...
                    if self._backup_dir_kept(name, current / name, excluded_names)
                ]
        ...
        def _create_managed_backup_unlocked(self, owner_id: str) -> dict[str, Any]:
            ...
            try:
                file_count, total_source_bytes = self._write_backup_archive(
                    temporary_path, source_root, owner_id, requester
                )
                self._publish_backup_archive(
                    temporary_path, final_path, manifest_path,
                    owner_id, file_count, total_source_bytes,
                )
            ...
            return self._finalize_backup_create(
                owner_id, requester, owner_backup_root,
                final_path, manifest_path, file_count, total_source_bytes,
            )
        ...
        def _write_backup_archive(
            ...
            with zipfile.ZipFile(
                ...
                self._write_backup_embedded_manifest(
                    archive, owner_id, requester
                )
            verified, verified_files, verification_error = (
                self._verify_backup_archive(temporary_path)
            )
            if not verified or verified_files != file_count:
                raise RuntimeError(
                    verification_error or "backup file-count verification failed"
                )
            return file_count, total_source_bytes
        ...
        def _finalize_backup_create(
            self,
            owner_id: str,
            requester: str,
            owner_backup_root: Path,
            final_path: Path,
            manifest_path: Path,
            file_count: int,
            total_source_bytes: int,
        ...
        def _write_backup_embedded_manifest(
            self,
