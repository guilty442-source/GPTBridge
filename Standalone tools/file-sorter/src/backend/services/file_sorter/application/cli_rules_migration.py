"""Legacy rules migration helpers for file sorter CLI."""
...
from .cli_rules_document import _read_rules_document, get_rules_path
...
def _legacy_rules_migration_inbox_paths() -> list[Path]:
    ...
    for candidate in candidates:
        entry = _inbox_candidate_entry(candidate, source_priority)
...
def _migrate_absolute_destination(
    index: int,
    keyword: str,
    folder: str,
    target: Path,
    ...
    if (
        ...
        return None, _reject_rule(
            index, keyword, folder, "absolute-destination-outside-target"
        )
    try:
        canonical_destination = selected_destination.resolve(strict=False)
    except (OSError, RuntimeError):
        return None, _reject_rule(
            index, keyword, folder, "absolute-destination-unresolvable"
        )
    ...
    try:
        ...
        return None, _reject_rule(
            index, keyword, folder, "absolute-destination-unavailable"
        )
    else:
        attributes = int(getattr(destination_stat, "st_file_attributes", 0) or 0)
        link_status = stat_module.S_ISLNK(destination_stat.st_mode) or bool(
            attributes & 0x400
        )
    if link_status:
        return None, _reject_rule(
            index, keyword, folder, "absolute-destination-is-link-or-reparse"
        )
    return canonical_destination.name, None



def _existing_destination_rejection(
    index: int,
    keyword: str,
    folder: str,
    target: Path,
    ...
    if link_status is not False or not selected_destination.is_dir():
        return _reject_rule(
            index,
            keyword,
            folder,
    ...
    *,
    ...
    for index, item in enumerate(raw_rules, start=1):
        if not isinstance(item, dict):
            rejected.append(_reject_rule(index, "", "", "rule-is-not-an-object"))
            continue
        keyword = str(item.get("keyword", "")).strip()
        folder = str(item.get("folder", "")).strip()
        if not keyword or not normalize_match_text(keyword):
            rejected.append(_reject_rule(index, keyword, folder, "empty-keyword"))
            continue

        if is_absolute_destination(folder):
            converted_folder, rejection = _migrate_absolute_destination(
                index, keyword, folder, target
            )
            if rejection is not None:
                rejected.append(rejection)
                continue
            folder = str(converted_folder)
            converted_count += 1
        ...
        if require_existing_destination:
            rejection = _existing_destination_rejection(
                index, keyword, folder, target
            )
            if rejection is not None:
                rejected.append(rejection)
...
def _source_rules_migration(
    source_path: Path,
    inbox_paths: set[Path],
    target: Path,
) -> LegacyRulesMigration:
    try:
        ...
        return LegacyRulesMigration(
            rules=(),
            rejected=(_reject_rule(0, "", "", "rules-document-unreadable"),),
        )
    return _migrate_legacy_rule_values(
        raw_rules,
        target,
...
def _dedupe_migrated_rules(
    migration: LegacyRulesMigration,
    seen_keywords: dict[str, str],
) -> tuple[list[KeywordRule], list[dict[str, Any]]]:
        ...
        if previous_folder != normalized_folder:
            additional_rejections.append(
                _reject_rule(
                    ...
                    rule.keyword,
                    rule.folder,
                    "duplicate-keyword-destination-conflict",
                )
            )
    return accepted_rules, additional_rejections



def _migrate_rules_from_source(
    source_path: Path,
    inbox_paths: set[Path],
    target: Path,
    configured_path: Path,
    ...
    *,
    ...
    migration = _source_rules_migration(source_path, inbox_paths, target)
    accepted_rules, additional_rejections = _dedupe_migrated_rules(
        migration,
        seen_keywords,
    )
    ...
    return accepted_rules, len(source_migration.rejected)
...
def _legacy_rules_for_migration(
    ...
    *,
    ...
    for source_path in source_paths:
        accepted_rules, source_rejected_count = _migrate_rules_from_source(
            source_path,
            inbox_paths,
            target,
            configured_path,
        ...
        rejected_count += source_rejected_count
