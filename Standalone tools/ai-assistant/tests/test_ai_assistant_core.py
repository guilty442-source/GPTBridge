...
def test_investment_conversation_is_separate_and_uses_automatic_models() -> None:
    ...
    assert policy["computation_service_owner"] == "xingcheng-via-information-layer"
    assert policy["statistics_service_owner"] == "xingcheng-via-information-layer"
...
def test_accounting_is_sent_to_xingcheng_through_information_layer() -> None:
    class FakeClient:
...
def test_discussion_is_requested_from_xingcheng_through_information_layer() -> None:
    class FakeClient:
        call: tuple[str, str, dict[str, object], int] | None = None
...
def test_star_autonomous_accounting_is_validated_before_local_write(
    ...
    try:
        ...
        assert result["ok"] is True
        assert result["star_accounting"]["accounting_owner"] == "投資管家"
