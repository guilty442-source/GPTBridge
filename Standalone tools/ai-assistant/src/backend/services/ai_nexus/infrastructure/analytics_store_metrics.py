...
class AnalyticsStoreMetricsMixin:
    """Performance, risk, and stress-test analytics methods."""
    ...
    def performance(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        snapshots = self._portfolio_snapshot_rows()
        ...
        attribution = self._currency_attribution(positions)
        return {
    ...
    def _portfolio_snapshot_rows(self) -> list[dict[str, Any]]:
    ...
    def risk(self, state: dict[str, Any], benchmark: str | None = None) -> dict[str, Any]:
        ...
        returns_by_symbol = self._symbol_daily_returns(raw_weights)
        ...
        weights = self._covered_weights(raw_weights, eligible_symbols)
        aligned_returns = {
        ...
        return self._risk_report(
            ...
            portfolio_returns_by_date=portfolio_returns_by_date,
            benchmark_symbol=benchmark_symbol,
            common_dates=common_dates,
        )

    @staticmethod
    def _covered_weights(
    ...
    def _risk_report(
        ...
        *,
        ...
        var_cutoff, tail, values = self._tail_metrics(portfolio_returns)
        correlation = self._correlation_pairs(eligible_symbols, aligned_returns, common_dates)
        beta = self._portfolio_beta(portfolio_returns_by_date, benchmark_symbol)
    ...
    def _portfolio_beta(
        ...
        return _covariance(aligned_portfolio, aligned_benchmark) / denominator if denominator > 0 else None
    ...
    def _position_exposures(
        ...
        return {
            dimension: {key: rounded(value, 2) for key, value in values.items()}
            for dimension, values in exposures.items()
        }
    ...
    def stress_test(self, state: dict[str, Any], scenarios: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        ...
        for scenario in configured:
            results.append(self._stress_scenario(scenario, positions, total))
...
__all__ = ['AnalyticsStoreMetricsMixin']
