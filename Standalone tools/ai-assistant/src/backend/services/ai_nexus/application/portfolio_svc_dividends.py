...
class PortfolioSvcDividendsMixin:
    async def _sync_dividends(self, _payload: dict[str, Any]) -> dict[str, Any]:
        ...
        fx_result = self._dividend_fx_result(holdings)
        dividend_result = await self._search_dividend_updates(holdings)
        updates = {
            (str(item.get("market") or ""), str(item.get("symbol") or "")): item
            for item in dividend_result.get("updates", [])
            if isinstance(item, dict)
        }
        updated_holdings, event_payloads = self._apply_dividend_updates(
            holdings, updates
        )
        if event_payloads:
        ...
        dividend_sync = self._dividend_sync_metadata(
            state, dividend_result, fx_result
        )
        await asyncio.to_thread(
            ...
            sync_owned_fields=_DIVIDEND_SYNC_OWNED_FIELDS,
        )
        await self._audit_dividend_sync(
            dividend_sync, dividend_result, event_payloads
        )
        ...
        response.update(
            self._dividend_response_patch(dividend_result, fx_result)
        )
        return response

    def _dividend_fx_result(
    ...
    async def _search_dividend_updates(
        self, holdings: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        if not star_result.get("results"):
            raise ValueError(
                str(
                    star_result.get("message")
                    or "投資管家尚未連線，配息搜尋未送出且不排隊。"
                )
            )
        star_updates = [
            self._dividend_update_from_star(item, star_result)
            for item in star_result.get("results", [])
            if isinstance(item, dict) and item.get("trusted")
        ]
        return self._dividend_result_summary(star_result, star_updates)

    def _dividend_result_summary(
        self,
        ...
        return {
            ...
            "updated_count": sum(
                item.get("status")
                in {"updated", "distribution_evidence_only", "no_distribution"}
                for item in star_updates
            ),
            "no_distribution_count": sum(
                item.get("status") == "no_distribution" for item in star_updates
            ),
            "no_dividend_count": sum(
                item.get("status") in {"no_distribution", "no_external_dividend"}
                for item in star_updates
            ),
            ...
            "coverage_percent": round(
                len(star_updates)
                / max(1, int(star_result.get("requested_count") or 0))
                * 100,
                2,
            ),
            "methodology": "內建瀏覽器搜尋公開市場配息事件與官方基金配息公告",
            "limitations": [
                "基金公告未揭露可驗證金額時只保存公告與頻率，不推造配息金額。"
            ],
        }

    def _apply_dividend_updates(
        self,
        ...
        for holding in holdings:
            ...
            if isinstance(update, dict):
                self._apply_dividend_update(
                    enriched, update, holding, key, event_payloads
                )
            updated_holdings.append(enriched)
        return updated_holdings, event_payloads

    def _dividend_update_from_star(
        self, item: dict[str, Any], star_result: dict[str, Any]
    ) -> dict[str, Any]:
        (
            parameters,
            distribution,
    ...
    def _star_update_parts(
        ...
        distribution = (
            ...
            if isinstance(item.get("distribution"), dict)
            else {}
        )
        source = next(
        ...
        return parameters, distribution, source, market, requested_symbol, events

    def _apply_dividend_update(
        self,
        ...
        self._apply_dividend_identity(enriched, update, key)
        self._apply_synced_dividend_frequency(enriched, update)
        source_currency = self._apply_dividend_valuation(
            enriched, update, holding
        )
        event_payloads.extend(
            self._dividend_event_payloads(update, key, source_currency)
        )

    def _apply_dividend_identity(
        self,
        enriched: dict[str, Any],
        update: dict[str, Any],
        key: tuple[str, str],
    ...
    def _apply_synced_dividend_frequency(
        self, enriched: dict[str, Any], update: dict[str, Any]
    ) -> None:
    ...
    async def _audit_dividend_sync(
        self,
        dividend_sync: dict[str, Any],
        dividend_result: dict[str, Any],
        event_payloads: list[dict[str, Any]],
    ) -> None:
        await asyncio.to_thread(
