...
class PortfolioSvcSyncMixin:
    ...
    async def _sync_open_markets(self, _payload: dict[str, Any]) -> dict[str, Any]:
        ...
        open_markets, searchable_holdings = self._searchable_market_holdings(
            holdings, sessions
        )
        ...
        result = await self._search_market_quotes(
            searchable_holdings, open_markets
        )
        self._apply_market_quotes(holdings, result, open_markets)
        self._invalidate_v3_snapshot()
    ...
    def _searchable_market_holdings(
        self,
        ...
        return open_markets, searchable_holdings

    async def _search_market_quotes(
        self,
        searchable_holdings: list[dict[str, Any]],
        ...
        quotes, bars = self._star_quote_bars(star_result)
    ...
    def _star_quote_bars(
        self, star_result: dict[str, Any]
        ...
        for item in star_result.get("results", []):
            ...
            if price <= 0:
                continue
            quote = self._star_quote_entry(item, price)
            quotes.append(quote)
            bars.append(
                {
                    "symbol": quote["symbol"],
    ...
    def _star_quote_entry(
        self, item: dict[str, Any], price: Any
    ) -> dict[str, Any]:
        source = next(
    ...
    def _apply_market_quotes(
        ...
        for holding in holdings:
            ...
            if isinstance(quote, dict):
                self._apply_market_quote(
                    enriched, holding, quote, result, fx_cache
                )
        ...
        self._merge_synchronized_holdings(
            ...
            sync_owned_fields=_MARKET_SYNC_OWNED_FIELDS,
        )
    ...
    def _apply_market_quote(
        ...
        fx_quote = self._cached_fx_rate(fx_cache, source_currency)
        ...
        self._apply_principal_twd(enriched, holding, fx_cache)
    ...
    def _cached_fx_rate(
        ...
        if currency not in fx_cache:
            fx_cache[currency] = self.v3.fx_rate(
                currency,
                "TWD",
            )
        return fx_cache[currency]

    def _apply_principal_twd(
        self,
        enriched: dict[str, Any],
        holding: dict[str, Any],
        fx_cache: dict[str, dict[str, Any] | None],
        ...
        principal_fx = self._cached_fx_rate(fx_cache, principal_currency)
