...
1. **xlarge（502M）基底**：預訓練中（~step 260/1000，eval ppl 68）；6× 容量較可能長出 induction head → 收斂後接 v6 混合資料 SFT 重測 L5。
2. **base（100M）或 medium_moe 加練**：GPU 空檔時可做對照組。
3. **接受 L4 上限**：v6 作為對話基礎正式啟用（L5=2/4 但嚴格優於 v12 的 0/4），待更大基底再衝 L5。
