...
- **自動化集中（2026-09-22 總督核定）**：**所有自動化集中流程一律由「自動化核心」處理**（automation-sovereign 轉型之核心引擎）：
  統一編排與排程（Git 提交／同步、清理與保留 §10.67、資源管制與釋放 §10.64、發布／更新同步、按需啟動 §10.66）；
  模組只執行、不得各自內建排程；週期／觸發集中註冊、統一審計與 kill switch。
