...
| E14 | **收尾組**：EC-1–8、DSN（G89）、Embedding 殘項、效能殘項 | 低：可並行收尾 | 各自前置 |
...
| P2 | **Embedding 殘項** | 細項：①生產端呼叫政策（operator runbook／受管 flow 登錄）②PG metadata 軸 `apply_metadata` executor（現無，依設計 blocked） |
...
- Embedding 維度／index 相容性：漂移後新世代建置已由遷移工具觸發（migrate_schema → apply_migration → rebuild_canonical，見 P2 行）；殘項為生產端呼叫方之政策決定（run_schema_migration 已供顯式受管入口）與 G50 其餘呼叫點遷移。
