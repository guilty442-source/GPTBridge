...
| P0 | **資源管制殘項**（§10.64） | **已全部驗證**：GPU 載入 VRAM 釋放實測 PASS（`convergence/10_64-engine-release-gpu.json`：alloc 166.8→8.5MB、RSS 1446.7→107.7MB；`_release_engine` 補 `gc.collect()+cuda.empty_cache()` 已修復入庫） |
