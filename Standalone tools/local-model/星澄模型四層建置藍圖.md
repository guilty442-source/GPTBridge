...
- **完成標示自動核對（2026-09-23 總督核定）**：指定法典工作者須主動偵測全檔的 `已完成`、`已落地`、
  `已閉`、`PASS`、勾選符號及工作者完成標記，逐項唯讀比對 current 程式、設定、registry、revision、證據與驗收條件；
  全部成立即刪除該藍圖項目，部分成立則刪除完成敘述並只保留可執行殘項，證據 stale／缺失／不一致則撤銷完成標示。
  不等待使用者提醒，不以 commit 訊息、測試數量或工作者自述單獨認定完成；每次藍圖修改及工作者提交後皆自動執行。
...
| 級別 | 項目 | 狀態 |
| --- | --- | --- |
| P0 | **G66 封印與外部簽章**（governor 側） | 更新 `epoch_seal_manifest`，完成外部簽章、certification 與最終稽核 |
...
| P0 | **啟動 10 s 全綠殘項**（A199／A265／A537） | 完成對話路由取樣（待 INT-10 窗口後） |
...
| P1 | **C++ 推論執行層**（P3／G29／G31／G32／G34） | 長時驗證、生產啟用、batch>1 生產呼叫者及生產 `.pyd` 重建接線 |
...
| P1 | **外部協作法典同步**（G36） | 執行 A528／A538 staged amendment（待協調窗口） |
| P1 | **執行模組語言外移**（§1.1 模組語言政策） | M1–M5 逐模組 shadow→primary→retire；維持單一職責、按需不常駐，完成治理驗收與 `architecture-languages` 更新 |
...
| P2 | **效能殘項** | MS6 C++／C# 承接與 R9 生產啟用 |

### 3.2 現存缺口（G 清單；僅未完成／待續）

| 編號 | 缺口 | 剩餘工作 |
| --- | --- | --- |
| G29 | C++ 推論執行層 | 長時驗證（窗口型）與生產啟用（runtime switch 預設 `off`） |
...
| G39 | MoE 訓練品質控制殘項 | 完成 MoE 訓練／推論 GPU VRAM 對照矩陣 |
| G50 | SQL／RAG 一致性與模型排程殘項 | 接入 reranker／CUDA 載入點、ProcessRegistry 登錄並裁定開機自啟落差 |
...
| G90 | Runtime 規則索引 V2 | **已落地（2026-09-23）**：`convergence/runtime_rule_index_v2.py`＋`runtime-rule-index-v2.json`——current-effective-only（660 條文＋67 規則，A591/A592 與子主宰全數排除）、successor resolve-before-index／unresolved=deny（六條 superseded 控制條全解析：A591/A592→A604 等）、dedup dictionary＋sorted postings＋validate-then-atomic-swap＋generation fence（stale→INDEX_NOT_READY）。parity＋SLO 實測：exact p95 0.0005ms／multifilter p95 0.0044ms／build 0.062s；測試 8/8。證據 `convergence/g90-runtime-rule-index-v2-20260923.json`。舊 v1 檔仍存留為歷史產物（derived，可安全刪除）。 |
