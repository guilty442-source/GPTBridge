...
| P0 | 資源自動管制與自動釋放（§10.64）：工作者總帳 CPU≤10%／RAM≤30%、四項資源自動釋放 | 大部分實作（2026-09-22）：worker-plane 歸屬＋總帳＋滯回控制律＋admission-hold＋④週期工作 pause_check 已接入（`a4775ce5`）；引擎/GPU 釋放走既有 auto_release；殘項：工作者行程釋放執行面與 30 min p95 驗收量測 |
...
### 10.64 資源自動管制與自動釋放設計（未完成；2026-09-21 總督核定）
