...
`moe_num_experts` 可設 4／8。**缺口**：無專屬 MoE 測試；「Router FP32」為註解宣稱、實作未見顯式 cast（待驗證）；
推論為逐專家 Python 迴圈（稠密執行）。
...
| G39 | MoE 訓練品質控制未建立 | 無專屬 MoE 測試；Router FP32 未驗證；梯度可達性／坍塌／實測 FLOPs 未檢 | MoE 品質與成本不可證（詳 2.2、Phase 9B） |
