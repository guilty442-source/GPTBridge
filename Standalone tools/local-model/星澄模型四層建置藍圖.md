...
| ACC-4 | 排程與協同 | **已落地**：`gpu_coordinator` 訓練／推論（逐請求）／C++ 載入三路閘＋`auto_release` 引擎生命週期＋資源總管互補（證據 `10_64-engine-release*.json`、`p1-p3f-gpu-coordination-gate-20260922.json`）＋adaptive plane 第二生產者（perf-baseline cpu/ram 經 `observe_merge` 欄位級合併注入，`convergence/p4-adaptive-plane-second-producer-20260922.json`）；**殘項**：其餘訊號生產者（transport backlog／model_load 等）仍 opt-in 待接 | 無 OOM；資源動作可稽核；閒置可回收 | P4 |
