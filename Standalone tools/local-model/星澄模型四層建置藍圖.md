...
**設計（缺口）**〔施工狀態 2026-09-22：項 1 間隔閘／項 4 日預算＋推論互斥／項 8 熔斷已實作於
`self_learning.py`＋`self_learning_support.py`（政策預設關閉、受管設定檔明示開啟；推論互斥
偵測同行程引擎快取，跨行程由 executor GPU 閘門兜底）；殘項標 *〕

1. **觸發政策（自主，但政策不可自改）**：資料量門檻 ✅、品質漂移 *、能力退化探針失敗 *、排程窗口 ✅、
...
3. **課程選擇（訓練什麼）***：依 maturity 未達項決定課程（SFT／DPO／蒸餾／續訓／專家模組）；每循環只推一個課程，失敗即停。（現行固定 SFT）
...
| P1 | **星澄自主訓練**（§2.7） | 基礎管線已實作（star-self-learning/v1）＋閘門補齊（最低間隔／每日預算／連續失敗熔斷／推論互斥；政策檔預設關閉、受管設定檔開啟）；殘項：③課程選擇①漂移／退化觸發②合成比例上限⑨maturity 重測＋資源帳⑦rollback 實測（保留上一代語意與現行 prune-latest 衝突，待裁決）⑩連續 3 循環自主驗收⑥法典 amendment（自主訓練權限邊界） |
