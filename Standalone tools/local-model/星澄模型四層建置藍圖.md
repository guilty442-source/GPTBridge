...
- 現況（2026-09-22 實測）：`gpu_coordinator` 已接訓練／推論（逐請求）／C++ 載入三路＋`auto_release` 已接引擎生命週期（證據 `10_64-engine-release*.json`、`p1-p3f-gpu-coordination-gate-20260922.json`）；殘項：adaptive plane 訊號生產者仍僅 maintenance controller（單一，opt-in）。
...
| P1-1 | **C++ 長時驗證／device-resident KV／bf16 GEMM／fp8**（eval tps C++ 量測路徑已落地，證據 `e11-eval-tps-cpp-path-20260922.json`） | 中：效能上限 | GPU 工具鏈（**未認領可承接**） | ①長時驗證②device-resident KV③bf16 GEMM／fp8④生產啟用評估（GPU 窗口） |
