...
| G37 | Small 訓練基線未成報告 | **部分解決（實作領先）**：`pretrain.py` 已記錄 `gradient_norm`（裁切前）與 `torch.cuda.max_memory_allocated`；尚缺正式 `star-native-baseline/v1` 報告與 GPU 基線跑 | 基線報告與反背誦待補（詳 2.2、Phase 2B） |
