...
- Triton kernels：RMSNorm／SwiGLU／RoPE 程式預設啟用（`XINGCHENG_TRITON_KERNELS=0` 可關閉）；
  需要梯度時仍自動退回 PyTorch。（**待對齊**：受管設定 `native-engine.json` note 記「Triton 自研 kernel 預設關閉（品質優先）」與程式預設不一致，見 §5.4 ACC-2。）
...
| 模組 | 格式／位置 | 目標 |
| --- | --- | --- |
| 自我學習 | `star-self-learning-policy/v1`（`runtime/settings/self-learning.json`） | enabled、min_new_examples 24、auto_activate true、suite `star-native-eval-dialogue-v1`、device cuda、max_steps 400；watcher 常駐；循環尾端接保留策略（**殘**：watcher CLI 路徑未呼叫 `apply_retention`——僅 in-process `run_cycle` 有；首次退役未執行，21 版仍在、無 `retention.jsonl`） |
...
| ACC-2 | GPU 加速軌（獨立） | 既有 Triton kernels 逐層驗證達標才啟用；CUDA／Gluon 等技術另軌評估；**不塞進純 C 引擎**（C 核心僅 CPU kernel 與 C ABI）。**現況**：三 kernel 實編譯證據存在（sm86 cubin／ptx、triton 3.8.0）＋模型級 parity 測試（atol/rtol 1e-3）；殘項：驗收報告入帳、受管設定 note「預設關閉」與程式預設（`XINGCHENG_TRITON_KERNELS=1`）不一致待治理對齊、Gluon 另軌 | 實編譯證據（非 API 假設）；logits ≤1e-3；與 PyTorch 路徑等價；C 核心零 GPU／C++ | P3a、§5.5 |
...
- ~~**待辦**：IPC p95／SQL lane／RAG 階段耗時由各持有者在執行期注入~~ → **IPC 延遲注入已落地**：`ipc/latency_ledger.py`（1024 樣本環形帳本，p50/p95/p99＋per-command）＋`server_commands` dispatch 計時埋點＋`tasks/perf_baseline_job.py`（automation_core `perf-baseline` flow，300s、pausable，僅寫 latest 防快照累積）；**SQL lane 注入已落地**（`perf_baseline_job.py` 採集 `get_lane_pool().stats()`）；殘項＝RAG 階段注入（latest 快照 `rag: null`）
...
- **未完成（2026-09-22 實測）**：統一執行緒策略入口（`OMP／BLAS／PyTorch` 執行緒 × 平行工作數 ≤ 預算）尚未建立——
  現況為局部有界（模型側 `execution/backend.py` 有 `cpu_thread_budget`；主系統 `native/execution_policy.py` 有界 clamp）；
  `FR-MULTI-CORE-PARALLEL`（A590）評測器僅驗條文 active、無 bounded-workers predicate。
...
| 低負載 | 按需模組（§10.27）；Ollama 按需（§10.7）；資源總管；adaptive plane；workload class；背景讓路 | 常駐清單已落地（R5）；殘＝transport LISTEN／NOTIFY 消費端、RAG 階段注入 | §10.27 已落地；§10.63 殘項 |
| 低資源占用 | **idle 基線驗收**（§10.27）；5 核預算（§10.30）；模型不常駐（A540）；保留策略 | idle 基線已量測達標（R0/R2）；殘＝首次權重退役未執行（`retention.jsonl` 未生成） | 修正保留保護；啟用清理 |
...
> 現況 xingcheng-native 21 版（首次退役尚未執行；`retention.jsonl` 未生成——watcher CLI 路徑未呼叫 `apply_retention`）。RAG index 世代排程已落地——
...
### 10.69 Git 收斂與自動化提交／推送／收斂設計（未完成；2026-09-22 總督核定）

> 目標：Git 流程單一收斂核心；提交／推送／收斂全自動且受治理；現況（2026-09-22 實測）：`automation-flows.json`
> `push:true`、`workspace_sync` 於治理稽核後推送（state `last_sync=synchronized-and-pushed`）；殘項：
> F① 週期性 ahead==0 待持續證明（現 `origin/main` 0 behind／2 ahead）＋C① 推送前「強制測試 PASS」閘未見實作證據（現僅治理稽核閘）。
...
3. RAG 階段耗時注入（idle／效能基線已建，§10.11）。
...
| P0-7 | **啟動 10s 殘項**（對話路由取樣、冷啟分佈、逾時實測） | 中：啟動 SLA 完整證據 | P0-1 窗口後（**未認領可承接**） | ①對話路由取樣②多次冷啟分佈③逾時 fail-closed 實測 |
...
| P1-7 | **`retention` watcher 修復**（§2.5／§10.67 殘項）`[處理中 @devin-cli 2026-09-22T16:08Z]` | 中：資源回收／實質缺陷（首次退役未執行） | 無（可即時動工） | ①CLI watcher 路徑接 `apply_retention`（dry-run 先行）②首次權重退役（21 版→active＋上一代；`retired` 子表）③`retention.jsonl` 生成與引用保護實測 |
| P1-8 | **推送閘**（§10.69）`[處理中 @devin-cli 2026-09-22T16:11Z]` | 中：流程完整性（推送前無強制測試閘） | 無（可即時動工） | ①推送前「強制測試 PASS」閘實作②週期性 ahead==0 持續證明③推送審計入帳 |
| P1-9 | **P6 驗收證據** | 低：驗收證據缺口 | 各自前置 | ①7 工具 5 s 啟停量測②測試 20 s 專項③隔離 overrides 補齊（其餘 5 工具）④星澄助理／自動修復端到端 |
| P2-1 | **執行緒策略入口**（§10.30） | 中低：資源（統一入口未建） | 無 | ①統一入口（`OMP／BLAS／PyTorch 執行緒 × 平行工作數 ≤ 預算`）②bounded-workers predicate 進 A590 評測器 |
| P2-2 | **Triton 預設對齊**（ACC-2） | 中低：設定一致性 | 無 | ①決定預設（on／off）②受管設定 note 與程式（`XINGCHENG_TRITON_KERNELS`）一致化③治理登錄 |
| P2-3 | **RAG 階段耗時注入**（§10.11） | 低：量測完整 | 無 | ①RAG 階段計時介面②latest 快照 `rag` 非 null |
| P2-4 | **MS6／MS10**（E14②；效能殘項） | 低：效能殘項 | 各自前置 | ①MS6（C# 承接）②MS10（SLO 儀表） |
...
| P0 | **啟動 10 s 全綠殘項**（A199／A265／A537） | 冷啟 core-ready **3.70 s**＋路由可用（auth 47ms／session 78ms／status 422ms／tools 1.20s）已實測；殘項：①對話路由取樣（待 INT-10 窗口後）②多次冷啟分佈③逾時 fail-closed 實測 |
...
| G50 | SQL／RAG 一致性與模型排程殘項 | workload lanes 直連已清零（殘＝`pool.execute` 一次性介面遷移）；parity audit 已落地（殘＝RAG 階段耗時注入）；model resource 殘＝各載入點改經 `request_load`；transport 殘＝LISTEN／NOTIFY 消費端 |
