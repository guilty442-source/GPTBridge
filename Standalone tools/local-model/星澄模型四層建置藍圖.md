...
| P2-1 | **執行緒策略入口**（§10.30）`[處理中 @devin-cli 2026-09-22T16:34Z]` | 中低：資源（統一入口未建） | 無 | ①統一入口（`OMP／BLAS／PyTorch 執行緒 × 平行工作數 ≤ 預算`）②bounded-workers predicate 進 A590 評測器 |
| P2-2 | **Triton 預設對齊**（ACC-2）`[處理中 @devin-cli 2026-09-22T16:45Z]` | 中低：設定一致性 | 無 | ①決定預設（on／off）②受管設定 note 與程式（`XINGCHENG_TRITON_KERNELS`）一致化③治理登錄 |
