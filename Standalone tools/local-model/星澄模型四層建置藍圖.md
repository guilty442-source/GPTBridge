...
| 編號 | 項目 | 風險（為何先後） | 前置／依賴 |
| --- | --- | --- | --- |
| E1 | **INTEGRATION-10 24h 驗收**（含預算語義裁定＋error 日誌複驗＋報告） | 最高：系統穩定性未驗收；窗口已在跑（~13h/24h） | 總督裁定預算語義；窗口滿 24h |
| E2 | **Git 收斂**：推送 `main`（領先 origin 176 commits）＋單一寫者＋ahead==0 週期驗收 | 高：未推送＝歷史遺失風險（單機故障） | 稽核／測試 PASS；協調者推送 |
...
| E8 | **對話品質**：GPU 訓練 v14（5711 筆）→ v15 啟用 → L5 重認證 | 中：產品能力缺口（現役 0/4） | GPU 預算；品質達標 |
...
| E13 | **自主訓練殘項**（DPO/蒸餾、停機、rollback 裁定、3 循環、法典 amendment） | 中低：能力進化 | rollback 語意裁定 |
