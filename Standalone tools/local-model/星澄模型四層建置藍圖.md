...
| P0 | **自動化核心集中化**（§1.1 自動化集中） | 細項：①自動化流程清單與 owner②集中排程／觸發註冊③統一審計與 kill switch④逐流程遷移（Git／清理／資源／發布／按需）⑤驗收：模組零內建排程、單一自動化核心 |
...
| P0 | **啟動 10 s 全綠可用**（A199／A265／A537） | 細項：①核心就緒（法典／權限目錄／資訊層／決策／系統執行期／維護）②core-critical 依賴探針全綠③capability-critical 探針全綠④UI／對話／工具路由可用⑤總時 ≤10 s（實測取樣）⑥逾時 fail-closed＋明確狀態回報。〔實測 09-21T23:18Z 冷啟〕process spawn→`main_runtime_ready` **3.70 s**（boot-core-stdout startup_phase 序列；含法典讀取／權限目錄／sovereign stack／authority reanchor）＋`status:ready`；依賴探針於 readiness 快照全 reachable。〔路由取樣 `startup-route-probe-20260922-072439`〕ticket-auth 47ms／session 78ms／runtime-status 422ms／toolbox_list 1.20s（1.5MB payload）——UI／工具路由端到端可用。殘：對話路由（送 infer 會喚醒模型，待 INT-10 窗口後取樣）＋多次冷啟分佈 |
| P0 | **測試與審計套件 ≤30 s**（A537：強制測試 20 s／審計 30 s；法典既定） | **測試套件已是 C**；**審計須以 C／C++ 實作**（含 pre-commit 稽核流）；細項：①C／C++ 審計引擎（≤30 s）②C 測試套件 ≤30 s 實測③Python pytest／稽核僅過渡或參考，移出提交閘門④快取失效策略（法典變更即失效）⑤負載下連續 10 次提交 ≤30 s 驗收⑥逾時 fail-closed 保留⑦**工作者測試硬上限 30 s（訓練測試除外）；超過即中止測試並直接修正**，不得放行或等其跑完 |
...
**判定：21/21 PASS**（rc-2026-09-22；04B-15 payload 邊界、04B-16 wheel cache
覆蓋、04B-17 uv.lock 一致性、04B-18 離線 venv 重建、04B-19 IPC 契約客戶端、
04B-20 共享服務替身全數 PASS）。重打包修正：governance_rule 以「僅程式碼」
...
Qdrant／Ollama 協定級替身（PG 不相容替身已落地 04B-20）。
...
4. ~~共享服務替身（PG）~~ → **04B-20 PASS**：`_fake_pg_server` 接受 TCP 後回
   `ErrorResponse`（SQLSTATE 0A000）；隔離 backend 的相依探針因 bound-root
   fail-closed 在 runtime init 即中止（設計邊界），故替身在 **release 客戶端
   契約層**驗證——RC venv 之 psycopg 連線被拒（SSL 交換期收到錯誤回應 →
   `OperationalError`），證明不相容服務絕不被誤認為 ready。
   殘項：Qdrant／Ollama 協定級替身（其 readiness 契約目前僅 TCP 層，
   待 HTTP/協定就緒語義落地後比照辦理）。
...
> link 失敗不再虛報 built/installed（比對 artifact mtime）。**發布阻斷**：`.pyd` 被 INT-10
