...
| P0 | **資源自動管制殘項**（§10.64） | 細項：①30 min 總帳 p95 — **已驗證**（`convergence/10_64-evidence-20260922.json` p95 0.5/0.14 PASS）②引擎／GPU 釋放長時驗證 — 未驗③法典明文 — **已發布**（A593 `resource-governance-throttle-protection`：保護治理不節流、行動序、自動釋放、全稽核、kill switch、A116 優先序；r2 五方稽核 5/5 → executed） |
