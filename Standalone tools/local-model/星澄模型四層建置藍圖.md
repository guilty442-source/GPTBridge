...
- EC-2 工作階段與登入：六家 provider session 持久化、驗證碼／Cloudflare 偵測（`VERIFICATION_MARKERS` 已具）、
  逾時與恢復；不得以卡片呈現。
- EC-3 自動記憶 `[處理中 @devin-cli/cosmic-papaya 2026-09-22T14:05Z — correlation 欄位＋端到端驗證]`：每次協作完成自動記錄需求與回覆摘要（來源 AI、時間、correlation），可查詢重用；不得手動卡片。
...
| E14 | **收尾組**：EC-1–8、DSN（G89）、Embedding 殘項、效能殘項、A548 全面責任檢討 | 低：可並行收尾 | 各自前置 |
