...
- 評估（依職能、生態、效能與整體淨占用）：
  ...
  | 權限核心 | **Python** | 權限目錄、身份、審計與測試工具鏈（`permission_directory`／audit／pytest 生態） |
...
| P0 | **測試與審計套件 ≤30 s**（A537：強制測試 20 s／審計 30 s；法典既定） | 細項：①定義 30 s「強制快速套件」範圍（受影響模組）②changed-files→tests 判定機制③全量套件移出提交閘門（離峰／夜間排程）④審計熱路徑瘦身：快取／增量／並行⑤快取失效策略（法典變更即失效）⑥負載下連續 10 次提交 ≤30 s 驗收⑦逾時 fail-closed 保留 |
