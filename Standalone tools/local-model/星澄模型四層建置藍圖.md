...
8. **防爆走**：每循環步數上限 ✅（`max_steps` 等）；資料上限 ✅（`max_dataset_examples` 決定性截斷＋截斷量入帳）；GPU 上限 ✅（閘門＋互斥）；異常自動停：連續失敗 ✅（`max_consecutive_failures` 熔斷）、eval 退步 ✅（評估閘未過即 rejected 不啟用）、資源超支 *（訓練中即停需 executor 內嵌預算計時）；不得自我擴大權限或自改政策 ✅。
...
> 目標：Git 流程單一收斂核心；提交／推送／收斂全自動且受治理；現況缺口：`main` 已領先 `origin/main` **176 commits**
> （推送未自動化）、index 鎖競爭、稽核 30 s 預算負載超時、auto-commit 與人工提交並行重複。
...
**E. 現況修復（立即）**：①啟用協調者推送或執行一次 `git-worktree-sync --push`（收斂 176 commits）
②index 鎖競爭：單一寫者（auto-commit 與人工不並行）③稽核預算：依 §3.1 測試/審計 ≤30 s。
