...
| P1 | **星澄對話與推理**（L5／L6／L7；G40） | 細項：①九類探針達標＋assistant-only loss mask＋狀態式續接— **L5/L6/L7 全階梯已達**（v15 5653筆/1200步 loss 0.33，maturity certify L0–L7 全過：dialogue 4/4、reasoning_tools、controlled_evolution kill-switch＋lifecycle＋受管升級證據；lineage v13 0/4→v14 1/4→v15 4/4；證據 `l5-sft-v15-1200steps-20260922.json`＋`maturity-20260922-113923.json`；v15 權重為候選工件未啟用，現役仍 medium-dense-v1）②命令理解探針 **已閉**（`star-semantic-plan-probes/v1` 24/24，`c9996693`，證據 `semantic-plan-probes-20260922.json`）＋eval suite dialogue v1 **已執行**（medium-dense-v1：套件閘過 ppl 97.49／tps 6.98／生成健檢過，但人工抽檢 6 例全退化——重複迴圈／碎詞，誠實記錄於 `dialogue-eval-medium-dense-v1-20260922.json`，L5 對話品質缺口量化）＋人工抽檢紀錄 **已建**＋L5 探針 session 隔離 **已修**（每探針獨立 ChatSession 與 L6 同規，除跨探針汙染；重認證 medium-dense-v1 仍 0/4——缺口為模型品質非 harness，證據 `maturity-l5-probe-isolation-20260922.json`；v14 資料集已備 5711 筆待 GPU 訓練）③xl-v3 啟用決策 **已裁**（不啟用：dialogue-v1 閘未過——tps 0.71<5、ppl 1015.9 vs medium 97.5、抽檢退化；證據 `dialogue-eval-chat-foundation-xl-v3-20260922.json`，維持 medium-dense-v1）與 run-to-run 穩定化④L6 算術課程／工具路線 **已達**（v15 資料內建算術唯一值池＋比較記錄，maturity L6 reasoning_tools 過）⑤L7 自主升級閘門 **已達**（kill-switch fail-closed＋lifecycle mechanics＋受管升級證據全過，maturity-20260922-113923.json） |
...
| P2 | **Embedding 殘項** | G50 漂移後新世代建置觸發 **已落地**（遷移工具 lifecycle/migration.py 之 apply_migration 逐相位執行＋PipelineRecoveryMixin.migrate_schema 漂移入口接線 rebuild_canonical；缺 executor／incomplete rebuild／validate 拒絕皆 fail-closed 不 activate；證據 g50-drift-migration-tool-20260922.json，35 測試）；＋runtime 入口 RagRuntimeIntegration.run_schema_migration（顯式受管呼叫：無漂移無版本→no-op、漂移→migrate_schema 於 pipeline loop 執行、結果入 app._log，39 測試）；殘項：生產端由誰呼叫之政策決定（operator runbook／受管 flow 登錄）＋PG metadata 軸 apply_metadata executor（現無，依設計 blocked） |
