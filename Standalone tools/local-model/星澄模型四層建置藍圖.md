...
3. **課程選擇（訓練什麼）**：依 maturity 未達項決定課程——SFT 課程族已實作 ✅（`curriculum_enabled`：讀 `model-maturity.json` 認證層，狀態不可讀或 L<4 對話基礎未達 fail-closed、L4→`sft-dialogue`、L5→`sft-reasoning`、L6→`sft-evolution`、L≥7→`sft-refresh`；`curriculum_intent_map` 可依課程篩 intent，篩後空池 fail-closed）；DPO／蒸餾／續訓／專家模組課程型別 *；每循環只推一個課程 ✅。
4. **資源與排程**：GPU 協調器（`gpu_required_mb`）✅＋閒置窗口 ✅；與推論互斥 ✅（`inference_exclusion`）；每循環 GPU 時間預算 * 與每日次數預算 ✅（`max_cycles_per_day`）；超預算不啟動（fail-closed）✅。
...
9. **觀測**：self-learning state／log ✅；升級後重測 maturity ✅（`post_upgrade_maturity_recheck`：重測失敗記錄於報告與 `last_maturity_recheck`，不暗改 active）；資源帳 ✅（`resource_account`：耗時／步數／裝置／資料量入 state 與循環報告；GPU/RSS 細項 *）。
...
1. **門檻與滯回**：連續 3 樣本超過預算 → 進入管制；連續 5 樣本低於 80% 預算 → 解除（避免抖動）。
   **預節流（2026-09-22 總督裁定，INT-10 嚴格語義配套）**：工作者總量達預算 **80%** 一樣本即進入
   「預節流」（降優先，不記超限）；越預算仍立即管制（原 3 樣本滯回改為**首樣本即管制**，
   解除仍 5 樣本 <80%）。目標：嚴格語義下 over_budget 樣本趨零（單樣本由 <80% 跳 >100% 之
   尖刺除外，屬不可消除之量測殘差）。
...
| 編號 | 項目 | 風險（為何先後） | 前置／依賴 |
| --- | --- | --- | --- |
| E1 | **INTEGRATION-10 24h 驗收**（含預算語義裁定＋error 日誌複驗＋報告）`[處理中 @devin 2026-09-22T12:40Z]` | 最高：系統穩定性未驗收；窗口已在跑（~13h/24h） | **預算語義已裁定（2026-09-22 總督）：嚴格解讀——任一 worker 超限樣本即 FAIL**；控制律須改為預算前預節流使零超限可達（見 §10.64）；本窗 resource_budget 依嚴格語義記 FAIL、其餘判據照舊累積，修正後需另起窗口重驗；窗口滿 24h |
...
| E5 | **法典收斂 amendment**（去重引用化＋業務規則下放＋衝突清單） | 中高：治理一致性；需協調窗口避免並行修訂衝突 | 清單已備；總督協調窗口 |
...
| E13 | **自主訓練殘項**（DPO/蒸餾、停機、rollback 裁定、3 循環、法典 amendment） `[處理中 @devin 2026-09-22T12:45Z]` | 中低：能力進化 | **rollback 語意已裁定（2026-09-22 總督）：prune-latest 優先**——只留最新一代；rollback 目標＝已認證且相容之任一留存版本，無留存版本時 rollback 閘 fail-closed |
...
| P0 | **法典收斂殘項**（§3.5 A／E）：掃描完成（dup-scan 已入法典、衝突 0 真／8 重疊、delegation 537/537 歸屬全決）；殘項＝下放 amendment（successor 明文，需 subordinate-ordinance 承載體裁決）＋去重引用化 amendment。〔補 09-22〕§3.5-A step① 治理 vs 業務對照清單已產出 `convergence/governance-vs-business-rules-20260922-060043.json`（537→治理 93／業務候選 269／hybrid 175，primary_dimension＋治理主體 token）；step③ 去重驗證 `convergence/dedup-boilerplate-verify.json`（A235–A247、A503–A513 各 1 組完全相同 FORBID；A248–A251 與 A536 同內容、分隔符差異）。step②④⑤ 為 live codex 寫入（build_successor→五方稽核→apply），codex 觀測到並行修訂中，須總督協調窗口 |
...
**A. 法典收斂 amendment（總督已裁決：現在執行；直接執行 amendment）**

1. 以 runtime-rule-index（`main-system/runtime/state/runtime-rule-index.json`）分類為底，產出「治理 vs 業務規則」對照清單
   （輸出至 `governance_rule/execution/audit/convergence/` 下之報告）。
2. 業務規則下放：對應主宰裁決／審查、模組執行；法典以 successor 明文下放（保留引用、不刪歷史、不變效力）。
3. 內部去重（§3.5 A）：boilerplate（A235–A247／A503–A513／A248–A251＋A536）改共同引用；模板群與跨層重述改引用；
   Git 五重述以 A375 為單一控制條。
4. 衝突檢查（§3.5 E③）：抽出業務規則來源與法典逐項比對 owner／禁止／時限／預算／路由；真衝突不得雙軌。
5. 執行：build_successor → 五方稽核 → `execute_amendment --apply` → 稽核 PASS（流程同 2026-09-21 amendment）。
6. 驗收：清單與 amendment 發布；法典效力不變；稽核與 mirror PASS；衝突清單歸零或明確登錄。
...
1. 測試：已是 C（`native/test_suites`）；量測 ≤30 s 並移出 Python pytest 全量為閘門。
2. 審計：以 C／C++ 實作審計引擎（含 pre-commit 稽核流），目標 ≤30 s；Python 稽核僅過渡。
3. 驗收：負載下連續 10 次提交 ≤30 s；逾時 fail-closed。
...
- ~~法典 `codex_semantic_duplication_scan` 仍為佔位列~~ → **已填充**（r10 amendment executed：
  ...
  **去重設計草案（2026-09-22，待裁決）**：三個逐字相同的 FORBID 群（已核實字串相等）——
  ①`general-prohibition`（A235–A247 中 12 條 active；A236 已 superseded）：
  `parallel-authority+directory-self-registration+cross-domain-decision+permission-bypass+
  direct-ungoverned-execution+unversioned-change+unbounded-retry+success-without-proof+
  special-law-scope-overreach`；
  ②`privacy-prohibition`（A249–A251＋A536 共 4 條 active；A248 已由 A536 取代）：
  `unregistered-scope+cross-owner-authority+permission-bypass+secret-or-personal-data leakage+
  unbounded-operation+silent-failure+success-without-evidence+scope-overreach`；
  ③`sql-authority-prohibition`（A503–A513 共 11 條 active）：
  `authority inversion+implicit identity casting+unbounded fallback+out-of-band DDL+
  schema drift continuation+projection treated as permission authority+release before
  complete PASS evidence`。
  方案：每群建一條共同禁止條（新 provision，member 以 `INHERITS:<common-id>` 引用替換逐字清單，
  保留各條 domain 專屬 rule／exception 不動）；amendment 需對 27 條 active 條文產生
  successor 版本＋supersession 邊＋稽核。提交前須總督核定共同條措辭（review-required 門）。
