...
| 級別 | 項目 | 負責側 | 狀態 |
| --- | --- | --- | --- |
| P0 | **法典上線**（3 元件＋42 條立法） | 已直接施工（使用者命令，分 3 批）：版本 `2026-09-20T17:39:12Z`、A551–A592、鏡像與架構圖同步、稽核 PASS | ✅ 已上線；**封印根／修訂歷史與外部簽章待補** |
...
| G51 | Process Registry 與效能基準未建 | **Process Registry 已落地（2026-09-21）**：`process_registry.py` `star-process-registry/v1`（PID/module/health/restart/shutdown_state 持久化＋`is_owned` 所有權界線＋`reconcile` 孤兒偵測）；ToolboxService 已接線。效能基準（10.11）仍待建 | 基準數據待補 |
...
| G73 | Release 非自足（Source Tree 載入） | electron-builder `extraResources` 僅含 `.venv`／`src-core`／`config`；`shared-layer`、`governance_rule` **未封裝**。`main.py` 以 `parents[2]/"shared-layer"/"src"` 插路徑；source-production 模式（cwd=repo root）下 `governance_rule` 實測解析至 `E:\GPTBridge\governance_rule\__init__.py`（Source Tree）。已封裝仍可能從 Source Tree 載入 | Release 自足性（詳 §10.37） |
| G74 | 無 Backend Release Manifest 與 UI×Backend 契約驗證 | `main-system/runtime/releases` 不存在；只有工具契約 `tool-runtime-contract.json`（v1／min v1）與 DB manifest `database-release.json`；新 Backend Release 與舊 UI 的相容性無 manifest 可驗 | Release 契約缺口（詳 §10.37） |
| G75 | SHARED_RUNTIME 隔離缺口 | 後端 `shared_layer` 實測來自 `E:\GPTBridge\shared-layer\src`（Source Tree，非 Release 封裝內容）；**不得搬移／重建 Shared Layer**，待 Release 封裝時以 `SHARED_RUNTIME` 類納入並驗證 | 隔離缺口（詳 §10.37） |
| G76 | venv 漂移與可搬移性未驗證 | `main-system/.venv` 的 `pyvenv.cfg` 記錄建立於 `E:\GPTBridge\.venv`（實體在 `main-system/.venv`，已位移）；`shared-layer/.venv` 為 Python **3.14.5**（與後端 3.11.9 不同版）；`local-model/.venv` 不存在（用主 venv）。Windows venv 不可假設可複製；封裝前須在**最終隔離位置重建** runtime env | Release Runtime 可搬移性（詳 §10.38） |
| G77 | 依賴鎖未接入 Release 流程 | 已有 `main-system/uv.lock`、`requirements*.txt`、`doctor_environment.py`；**無 wheel cache／hash validation／環境建置器**接入 release；本次補：lock identity（sha256 排序 dists）、runtime env manifest 欄位、native 身分 | 依賴可驗證性（詳 §10.38） |
| G78 | Backend release payload 快照未記入 manifest | `package_integrity.collect_file_hashes`／`snapshot_digest` 能力已具，但 backend release manifest 尚未記錄 payload 快照 → 無法自動偵測 `pip install/upgrade` 原地修改 | Release 不可變性（詳 §10.39） |
| G79 | local-model runtime 依賴未分類 | `shared_layer_classification` 已列 13 項；`local-model`（星澄）模組／Ollama 按需 runtime 未列入契約（權重不得封裝已入 `forbidden_content`） | 共享依賴完整性（詳 §10.39） |
| G80 | .exe／.dll 原生清單未實測 | `pe_machine`／`native_extension_errors` 能力已具；契約僅列 1 個 `.pyd`，`required_dlls` 空 | 原生模組相容（詳 §10.39） |
| G81 | 服務重複實例僅驗證層 | `service_topology` 閘門已具（驗證層）；runtime 層無強制（本階段依指示不動服務拓撲、不實作 A/B 啟動） | Runtime 強制（詳 §10.39） |
...
### 10.39 實作缺口比對（2026-09-21，三輪需求 vs 實作）

| 需求 | 實作證據 | 狀態 | 缺口 |
| --- | --- | --- | --- |
| UI 進入點與生產依賴盤點 | §10.37 表（main/preload/renderer/build/runtime config/IPC） | ✅ | — |
| Backend Python 載入來源實測 | §10.37（`shared_layer`／`governance_rule` 來自 Source Tree；`main.py` 已 fail-closed） | ✅（問題確認） | G73／G75 |
| Release Dependency Contract 六類 | `DEPENDENCY_CLASSES`＋`release-dependencies.json` | ✅ | — |
| Python Import Origin Validation（7 檢查、Windows 路徑） | `python_release_dependencies.py`；8 測試 | ✅ | — |
| Shared Layer 隔離缺口列管（不搬移） | G75＋§1.8.1 | ✅ | — |
| 新 Backend × 舊 UI 可啟動性 | `python-backend.ts`／`ipcSession` 證據；契約未驗 | 🟡 | G74 |
| Python 執行環境盤點 | §10.38 表（venv/site/PYTHONPATH/prefix/cwd/native） | ✅ | G76 |
| `runtime/releases/<id>/{backend,dependencies,manifest.json}` | 未建（依指示不建第二套 builder；佈局已定） | ❌ 待建 | G74／W4-0c |
| Python 版本可驗證 | `runtime_environment.python`＋validator | ✅ | — |
| 套件版本可驗證 | `dependency_lock` identity（92 筆）＋validator | ✅ | — |
| 原生 Extension 與 ABI 相容 | PE machine＋ABI tag＋`native_extensions` | ✅（.pyd） | G80 |
| 不依賴開發 site-packages／user site／PYTHONPATH | 三閘門＋測試 | ✅ | — |
| 不從其他 Release 載入 | origins 允許根＋跨 Release 測試 | ✅ | — |
| Release 不受 `pip install/upgrade` 原地修改 | payload 快照能力已具；manifest 未記錄快照 | 🟡 | G78 |
| Dependency Manifest 欄位（runtime/arch/lock/native/env identity） | 實測寫入（env identity＝prefix＋pyvenv hash） | ✅ | — |
| Shared dependencies（shared-layer／local-model／governance_rule） | `shared_layer_classification` 13 項＋`governance_references` | 🟡 local-model 未分類 | G79 |
| 不封裝 Ollama 權重／不動星澄權重 | `forbidden_content`（`*.gguf`／`*.safetensors`／`*.pt`／`*.onnx`）＋測試 | ✅ | — |
| 原生模組相容（.pyd/.dll/.exe、ABI、arch、缺 DLL） | `pe_machine`／`native_extension_errors` | 🟡 清單未實測 | G80 |
| 12 項 Runtime 驗收 | `test_release_runtime_environment.py` 12/12 | ✅ | — |
| 修改既有 Validator/Manifest/Contract/Governance Check（不重複） | 三檔擴充；無第二套系統 | ✅ | — |
| Shared Layer 正確版本通過／不相容失敗 | `shared_layer_contract`＋2 測試 | ✅ | — |
| Governance／Permission Contract 不相容失敗 | `GOVERNANCE_/PERMISSION_CONTRACT_INCOMPATIBLE`＋測試 | ✅ | — |
| Codex 完整性失敗拒絕（重用既有驗證器） | `validate_database_integrity`＋legacy FK 基線 | ✅ | — |
| 缺治理依賴拒絕 | `GOVERNANCE_DEPENDENCY_MISSING`＋測試 | ✅ | — |
| Source Tree Leakage 偵測 | `SOURCE_TREE_LEAK`＋測試 | ✅ | — |
| 不建第二 Shared Layer Service／治理權威 | `service_topology` 閘門＋測試 | 🟡 驗證層 | G81 |
| 新舊 Release 不共用可變程式碼 | origins＋測試 | ✅ | — |
| 正式權限／稽核資料不封裝 | `official_state_paths`＋`forbidden_content`＋測試 | ✅ | — |
| 舊 IPC Contract 相容 | `validate_ipc_contract`＋測試 | ✅ | — |
| 原後端不受影響／不動法典 | pyvenv＋codex hash 前後一致；合成 codex 僅暫存 | ✅ | — |
| 不實作 A/B、Gateway Handover、更新切換 | 未實作（依指示） | ✅ 遵守 | — |
...
6. **Blue-Green**（standby → 切換 → 排空）＋**維護×更新鏈**（ACTIVE／PREVIOUS／ROLLBACK／ARCHIVE）。
    ...
    - 6d 待辦：以 `BootCore._maybe_handover` 注入實測更新期間停機 0。
...
>    `test_release_manifest_g56.py` 3/3 PASS。**開發／正式分離（§10.19）仍待 W4 Release
>    隔離**（G57/G73–G75，`runtime/releases` 尚未存在）。
>
> 全部 40＋3 新增測試全綠；`python -m governance_rule.execution.audit` **PASS**。
...
0b. **Release 隔離**（§10.37；G73–G75）：把 `shared-layer`／`governance_rule` 納入 release 封裝
...
0c. **Runtime 依賴隔離**（§10.38；G76–G77）：於最終隔離位置重建 venv（不可假設可搬移）、接入
