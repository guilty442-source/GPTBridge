...
> 原有分散文件仍保留為各自領域的權威或證據來源，不刪除、不搬移。
...
- 單一入口原則：本整合版發布後，後續藍圖更新以本文件為單一入口；領域文件只維護其領域事實。
...
| `shared-layer/SQL_LAYER_GOVERNANCE_BLUEPRINT.md`、`shared-layer/docs/DATA_OWNERSHIP_CONTRACT.md` | SQL 治理與資料所有權 | 第 6 章 |
...
| G24 | SQL 治理藍圖多數未落地 | `SQL_LAYER_GOVERNANCE_BLUEPRINT.md` Phase A–D 工作包 | lineage／authority／workload class |
...
- 驗收：maturity **L5 ≥75%** 探針通過；lifecycle 註冊新權重；eval suite（dialogue v1）＋人工抽檢紀錄。
- 依賴：Phase 0–4（已完成）。
...
| SQL 治理與所有權 | `shared-layer/SQL_LAYER_GOVERNANCE_BLUEPRINT.md`、`shared-layer/docs/DATA_OWNERSHIP_CONTRACT.md` |
