...
- **TRANSITION PRECONDITIONS（前置）**：① ~~A591／A592 正式規則離開 `proposed` 並達 parity（含機器評測器）~~
  **已滿足（2026-09-22 實測：A591／A592 `active`；`FR-SOVEREIGN-TO-CORE-ENGINE`／`FR-NO-SUB-SOVEREIGN-ALL-MODULES` parity VERIFIED）**；
...
4. 子主宰 registry 切換與退役（active 子主宰 6＋13 已 retired；`module_assignment_registry` 5 active＋12 legacy-active；`owner_sub_sovereign` 欄已自 registry 移除）。
