...
- **待辦**：TS Validation 執行器。〔IPC Compatibility Test 執行器已落地〕`scripts/ipc-compat-check.py`：後端 39 路由命令 × renderer 19 送出 × preload→ipcMain 通道三向對照，`runtime/logs/ipc-compat-20260922-073236.json` PASS（零未路由命令；19 條後端路由未被現行 renderer 使用屬能力面資訊）
