...
| 級別 | 項目 | 狀態 |
| --- | --- | --- |
| P0 | **法典上線殘項**：封印根／修訂歷史與外部簽章（G66）；評估器 parity（G67：40/64 verified，剩 24 條 typed-fact 型） | 部分（G66 governor 側） |
...
| P2 | Release pipeline 依賴鎖收尾（G77）；Embedding 維度／index 版本相容性；transport LISTEN／NOTIFY | 未完成 |
...
| G67 | 評估器 parity | 已發布 amendment（五方稽核通過）：40/64 條以實跑 evaluator PASS 證據轉 `evaluator-parity-verified`；剩 24 條為逐操作 predicate（caller/role/route 等 typed facts），需各規則 fact fixture 方能驗證；→active 另需 formal_rule_mapping VERIFIED 列 |
...
- 更新不覆寫 runtime 強制（設定／資料／權重／session／未完成任務）。
- Embedding 維度與 index 版本變更相容性未實作。
- transport 由輪詢改 LISTEN／NOTIFY（§6.5）。
...
8. 評估器 parity：40/64 條已 verified（實跑 PASS 證據）；24 條 typed-fact predicate 待 fact fixture（G67）。
