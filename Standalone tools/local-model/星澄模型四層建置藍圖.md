...
| P0 | **按需啟動與休眠**（§10.66）：自動按需啟動模組＋熱／溫／冷休眠與喚醒 | 部分實作：`SleepPolicyManager`（tasks/sleep_policy.py）＋政策檔＋drain 重檢＋審計 jsonl＋7 測試；按需啟動已由 activation broker／toolbox on-demand 承擔；溫層由 AutoReleaseManager 承擔。殘項：①預設 enabled=false，需總督核定啟用②喚醒時限實測（溫≤2s／冷≤5s）③30min 無抖動驗收④非治理核心清單與法典明文 |
...
| P0 | **G76／G77 收尾** | **已驗證**：①G76 venv 自足＋離線重建（04B-18：`--no-index --find-links` 重建 52/52 dist 與 lock 完全一致）②G77 `uv.lock` 雙向一致性（04B-17：runtime ⊆ uv.lock 且版本全等） |
...
**判定：20/20 PASS**（rc-2026-09-22；04B-15 payload 邊界、04B-16 wheel cache
覆蓋、04B-17 uv.lock 一致性、04B-18 離線 venv 重建、04B-19 IPC 契約客戶端
全數 PASS）。重打包修正：governance_rule 以「僅程式碼」入包（審計
ledger／archive／runtime 排除）、codex 改以 SQLite backup 原子快照入包且
manifest／validator 均以快照為準（live codex 持續修訂不再造成 drift）、
隔離執行的審計寫入經 `GPTBRIDGE_CODEX_AUDIT_PATH`／`GPTBRIDGE_AUDIT_CHAIN_DIR`／
`GPTBRIDGE_CAPABILITY_LEDGER` 導向 sandbox（release payload 不可變，
codex_entry_state 新增環境覆寫）、隔離啟動零補償（flat-layout main.py
原生解析）。completion 仍 NOT_COMPLETE——殘項：04B-09 共享服務替身
（PG/Qdrant/Ollama 相容性 test double）。
...
- **舊架構（先對齊再刪，codex-first）**：
  ①`main-system/shared-layer` 相容 shim（G72 已改指 `E:\GPTBridge\shared-layer`）——引用掃描後刪除；
  ②`Standalone tools/global-cleaner`（法典 A537 retired＋non-executable）——registry／manifest 對齊後刪除；
  ③`main-system/governance/sub-sovereigns/language_review_sub_sovereign.py`（A336／A592 廢止）——registry `language-review` 降 retired 後刪除；
  ④`.worktrees/*` 內已合併的舊分支工作區（逐一確認後移除）。
