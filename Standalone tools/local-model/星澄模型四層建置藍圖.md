...
| P0 | 主系統 Python 常駐資源縮減（§10.63） | R0/R1/R3 已施工並量測達標（ready 1.71 s／RSS p95 90.5 MB／固定週期 6）；R2 idle CPU 30 min 量測中；R4/R5 未施工 |
| P0 | 資源自動管制與自動釋放（§10.64）：工作者總帳 CPU≤10%／RAM≤30%、四項資源自動釋放 | 設計已定；未實作 |
...
| G59 | INTEGRATION 收斂未執行 | **僅 INT-10 24h 驗收未跑**（詳 10.20／§10.13） |
...
- 實測：ready **3.44→1.71 s**、phase-4 **1,578→0 ms**、phase-5 **6 ms**（目標 ≤3.0 s／≤600 ms 達標；
  `perf-baseline-20260921T163821Z.json`）。
...
- hot-reload health-monitor 折入 watcher `_loop`（60 s due-gate）；state outbox 改
  ...
  改牆鐘閘門保持原節奏）。
- 實測：**固定週期工作 14→6 項**（目標 ≤6 達標）；後端 RSS p95 **316.5→90.5 MB**
  （90 s 取樣；30 min 基線量測中）；相關測試 62 項 PASS（新增
  `tests/test_periodic_scheduler.py` 9 例，commit `661bc81f`）。
