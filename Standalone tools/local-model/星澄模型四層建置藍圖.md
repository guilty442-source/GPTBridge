...
| P0 | **INTEGRATION-10（24h 穩定驗收）** | 細項：①連續 24 h uptime（自 2026-09-21T23:18Z 起）②零非預期重啟／dead-grace 迴圈③健康探針（readiness／boot-core）全綠④RSS／CPU 於預算內⑤日誌零 ERROR／零孤兒程序⑥完成後留存驗收報告。驗收腳本 `scripts/integration-10-acceptance.py`（六項判據→`runtime/logs/integration-10-<ts>/report.json`）。〔mid-window 06:10Z，累積 6.9h〕✓無重啟 ✓三核心探針綠 ✓零孤兒；待裁定項：④89/885 over-budget 樣本（73 於管制中、16 管制啟動前偵測延遲；尖峰皆伴隨工具/模型啟動；預算語義待總督裁）⑤10 條 error 級日誌（8×auto-loop CODEX_UNAVAILABLE＋1×scheduled audit FAIL＋1×RuntimeWarning——後兩者已修：unawaited coroutine、isolation-monitor expected_stop TOCTOU） |
...
| P1 | **執行模組語言外移**（§1.1 模組語言政策） | ①已閉：清單 `python-execution-modules-20260922.json`（19 模組／12 migrate）＋外移順序 `module-language-migration-order-20260922.md`（M1 standalone→M5 boot-core，依爆炸半徑＋E3/E4 閘控＋契約可驗證性排序）；殘項：②逐模組以 C／C++／C# 重寫並守契約③**每模組單一能力、單一職責**（混責先拆）④**按需使用不常駐**（載入／卸載＋資源釋放驗收）⑤治理稽核＋迴歸測試⑥codex `architecture-languages` 更新（amendment＋hash rebind） |
