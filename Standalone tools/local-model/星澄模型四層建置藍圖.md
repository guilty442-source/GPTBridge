...
| P1 | **C++ 推論執行層**（P3／G29／G31／G32／G34） | 殘項＝長時驗證（窗口型）＋fp8（殘；bf16 GEMM＋device-resident KV 自研 kernel 已落地，nvcc＋v141/v142 工具集解決）＋生產啟用（`XINGCHENG_CPP_RUNTIME` 預設 `off`；batch>1 無生產呼叫者）＋生產 `.pyd` 重建接線（W2-1 殘）；已閉＝C 核心 grouped GEMM＋線上分塊注意力（W2-1）、INT8／INT4 量化、CUDA cuBLAS 橋接、device-resident 權重、KV INT8 cache、W1（QK SIMD／緩衝重用）、R5（top-k 稀疏）、R9（C++ `generate_batch`）、P3f GPU 協調閘，證據見 `audit/convergence/` 與 Git 歷史 |
