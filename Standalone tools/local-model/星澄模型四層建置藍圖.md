...
| ACC-4 | 排程與協同 | **已落地**：`gpu_coordinator` 訓練／推論（逐請求）／C++ 載入三路閘＋`auto_release` 引擎生命週期＋資源總管互補（證據 `10_64-engine-release*.json`、`p1-p3f-gpu-coordination-gate-20260922.json`）；**殘項**：adaptive plane 訊號生產者仍僅 maintenance controller（opt-in） | 無 OOM；資源動作可稽核；閒置可回收 | P4 |
...
| 低負載 | 按需模組（§10.27）；Ollama 按需（§10.7）；資源總管；adaptive plane；workload class；背景讓路 | 常駐清單已落地（R5）；殘＝transport LISTEN／NOTIFY 消費端 | §10.27 已落地；§10.63 殘項 |
