...
21. **主系統多核心平行（強制）**：主系統平行度有界並以核心數為上限（**初始 10 核心預算，邏輯處理器計**），
...
>
> **初始設計目標：10 核心預算**——本機 8C／16T，**以邏輯處理器計**；保留 6 個邏輯處理器
> 給系統與其他程式；支援**平行多工處理任務**；預算可配置（AdaptiveEnvelope），
> 但**不得超過可用邏輯處理器數**。

- 所有 worker pool 一律**有界**：`min(N, cpu_count)`；**禁止**無上限 `max_workers=len(...)`（AA2）。
- **執行緒預算**：`OMP／BLAS／PyTorch 執行緒 × 平行工作數 ≤ 核心預算`（**初始 10 個邏輯處理器**、
  保留 6 個給系統；上限為可用邏輯處理器數）；統一由 R8 執行緒策略管理。
