...
| P0 | **語料機密排除**（G33；現行預設來源含 `governance_rule/codex`，`.txt` 會納入機密 zh-TW 鏡像） | worker | 立即修正 |
| P0 | Small 基線完整化（G37：GPU memory／gradient norm 補記錄） | worker | 待辦 |
...
| G33 | 【優先】預訓練資料管線未完備 | `corpus.py` 缺 Registry／Version／Language 分類／License 驗證／NFC＋映射／近似去重；預設來源含 `governance_rule/codex`（會納入機密 zh-TW `.txt` 鏡像） | 語料治理與合規風險（詳 2.2、4.1 Phase 2R） |
...
> 路徑：`React → Electron Main → Backend Gateway → Request Registry（Request→Generation）→ Python Backend`。
