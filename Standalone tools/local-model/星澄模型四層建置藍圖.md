...
| G87 | 【未完成】self-health 測試缺檔 | 稽核 self-health 清單要求 `Standalone tools/chinese-semantic-engine/tests/test_chinese_semantic_engine.py`，實體不存在（工具 manifest v2.0.0 已登錄、僅 locales／src／manifest.json）→ 稽核 fail-closed（目前唯一失敗項） | 稽核紅燈（修復規格見 §10.45 W5-2） |
