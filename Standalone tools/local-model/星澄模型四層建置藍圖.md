...
| P0 | **單元制落地**（§10.68）：模組→單元→區塊層級＋目錄同步＋按需隔離 | 映射設計已定；細項：①法典層級表 amendment②tools→model→shared→core→xingcheng 依序受管搬移③跨單元介面強制④單元／模組級隔離實測 |
...
**法典側需求**：層級表（block/unit/membership）＋隔離職權明文（運行核心執行、決策核心裁決）
＋跨單元介面強制條文；可與 §3.5 修訂批併批。
