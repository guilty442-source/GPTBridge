...
| P0 | **主系統 Python 常駐外移**（§1.1 目標二） | 未實作；細項：①常駐職責清單化（對照 resident-core.json）②外移順序與替代宿主（C／服務／Electron）③過渡雙軌與回退④驗收（ready／RSS／CPU／零功能退化） |
| P0 | **資源自動管制殘項**（§10.64） | 細項：①30 min 總帳 p95 驗收量測（樣本源 `--log-samples`）②引擎／GPU 釋放長時驗證③法典側資源優先序與「不得為降載停用治理」明文 |
| P0 | **全系統收斂殘項** | 細項：①INTEGRATION-10 24h 穩定驗收②RC 重打包（RC `main.py` 舊治理路徑缺陷）③G76／G77（venv／lock 收尾） |
...
| P1 | **星澄對話與推理**（L5／L6／L7；G40） | 細項：①九類探針達標＋assistant-only loss mask＋狀態式續接②命令理解探針（`star-semantic-plan/v1`）＋eval suite dialogue v1＋人工抽檢③xl-v3 啟用決策與 run-to-run 穩定化④L6 算術課程／工具路線⑤L7 自主升級閘門 |
...
| 編號 | 缺口 | 剩餘工作 |
| --- | --- | --- |
| G24 | SQL 治理十階梯 | scheduler 無生產組裝點、TCP-probe 路徑無法認證 |
