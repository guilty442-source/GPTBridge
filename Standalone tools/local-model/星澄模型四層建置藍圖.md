...
| G56 | Release Manifest（部分完成） | release dependency contract＋frontend manifest＋RC `rc-2026-09-21/release-manifest.json`（identity／commit／hashes／lock／python runtime／IPC／governance compatibility／data boundary）已具；**`validate_release_bundle` 對 RC fail-closed 19 項**——venv hash／lock mismatch（G76/G77）＋native 清單以 source-tree 相對路徑查找但 RC venv 在 `rc-*/venv`（契約↔RC 佈局對齊缺口） | 切換前相容判定 |
...
- 實測：`validate_release_bundle` 對 RC fail-closed 回 19 項——`VENV_CONFIG_HASH_MISMATCH`、`DEPENDENCY_LOCK_MISMATCH`（G76/G77）、`REQUIRED_MODULE_MISSING:core_system/shared_layer`、`SOURCE_TREE_LEAK:governance_rule`，以及 `NATIVE_EXTENSION/DLL_MISSING`（契約以 `main-system/.venv/...` source-tree 相對路徑查找，RC venv 實體在 `rc-*/venv`，屬契約↔RC 佈局對齊缺口）。
- 剩餘：venv 最終位置重建（G76）、lock 接入（G77）、native 清單路徑改以 release root 解析、切換前五項相容檢查。
