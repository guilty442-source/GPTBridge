...
8. **防爆走**：每循環步數上限 ✅（`max_steps` 等）；資料上限 ✅（`max_dataset_examples` 決定性截斷＋截斷量入帳）；GPU 上限 ✅（閘門＋互斥）；異常自動停：連續失敗 ✅（`max_consecutive_failures` 熔斷）、eval 退步 ✅（評估閘未過即 rejected 不啟用）、資源超支 ✅（executor 內嵌預算計時：`max_train_seconds`／`max_train_vram_mb` 步邊界即停、summary 記 `stopped_reason`、循環層 `resource-overspend` fail-closed 不評估不啟用）；不得自我擴大權限或自改政策 ✅。
