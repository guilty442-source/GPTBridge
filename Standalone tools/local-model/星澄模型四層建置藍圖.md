...
- **模型規模上限（2026-09-21 總督核定）**：模型參數量上限 **1B**（先設定；含 MoE 總參數）。
...
- INTEGRATION-04B 04B-10（隔離啟動 harness 未建；判定 NOT_COMPLETE，見 §10.52）。
...
- **04B-10**：Backend 啟動中途失敗（需隔離啟動測試替身；§10.52）。
...
**完成標準未滿足項**：① 可獨立啟動／⑨ 可安全停止＝未驗證（04B-10 BLOCKED）；
