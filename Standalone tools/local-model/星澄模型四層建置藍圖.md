...
- **模型目錄 ~29 GB 中 ~10 GB 未登錄 lifecycle**（`base-dense-v1`／`medium-moe-v1`／`pretrain-xlarge-1k`／`baseline-small-*` 等 11 個頂層目錄）：
  retention 對無法解析路徑 fail-closed 保留 → 該批目錄不受 `keep_weight_versions` 治理、`model-dir-size` 異常每循環持續上報。
  殘項：逐一判定歸屬（登錄為權重版本／明確封存／受管刪除——屬總督或操作者裁決，不得由 worker 自行刪除）。
