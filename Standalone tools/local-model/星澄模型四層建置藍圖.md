...
- **xlarge 進展（2026-09-21）**：xlarge（502M／vocab 32000）預訓練 1000 步收斂——
  train loss 3.57→1.63、eval ppl **15.69**（82M 線約 245）。`chat-foundation-xl-v1`
  （v7-mixed，400 步 CPU）成熟度認證 **L4**（報告 `maturity-20260920-231038.json`，
  lifecycle weights v15）：L0–L4 全過（L3 ppl 3890／基線 32000=0.12），
  L5 **2/4**——turn_boundary（產出真正回合回覆「好的，有什麼可以幫你的嗎？」）與
  choice 通過，**echo 與 multiturn_memory（逐字複製 induction）仍未過**。
  結論：規模擴大改善語言建模與回合邊界，但 induction 複製能力是獨立瓶頸，
  需足量 SFT 步數（GPU 接續訓練 xl-v2 進行中）。
