...
| P0 | **啟動 10 s 全綠可用**（A199／A265／A537） | 細項：①核心就緒（法典／權限目錄／資訊層／決策／系統執行期／維護）②core-critical 依賴探針全綠③capability-critical 探針全綠④UI／對話／工具路由可用⑤總時 ≤10 s（實測取樣）⑥逾時 fail-closed＋明確狀態回報 |
...
| P0 | **INTEGRATION-10（24h 穩定驗收）** | 細項：①連續 24 h uptime（自 2026-09-21T23:18Z 起）②零非預期重啟／dead-grace 迴圈③健康探針（readiness／boot-core）全綠④RSS／CPU 於預算內⑤日誌零 ERROR／零孤兒程序⑥完成後留存驗收報告 |
