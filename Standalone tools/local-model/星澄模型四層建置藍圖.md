...
| G26 | 低負載機制未量化 | **部分解決（2026-09-21）**：連線池使用率已可觀測——`ConnectionManager.stats()`（size/max/idle/active）＋`collect_pool_pressure` 接真實池（原為全零 stub），telemetry 經 `peek_connection_manager` 取活池；輪詢→事件化仍開 | 高負載風險 |
