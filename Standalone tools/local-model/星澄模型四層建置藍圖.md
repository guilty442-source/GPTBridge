...
| P0 | **資源自動管制殘項**（§10.64） | 細項：①30 min 總帳 p95 驗收量測（樣本源 `--log-samples`）— **已驗證**（`governance_rule/execution/audit/convergence/10_64-evidence-20260922.json` p95 0.5/0.14 PASS, window 90 full, `scripts/verify-10_64-resource-budget.py:1`）②引擎／GPU 釋放長時驗證③法典側資源優先序與「不得為降載停用治理」明文 |
