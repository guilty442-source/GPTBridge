...
| G8 | 自我學習循環機制已驗證（2026-09-21） | 強制循環完整走通：19 例匯出→受管 SFT job（400 步，eval ppl 7.49）→adapter 註冊→eval gate→fail-closed 裁定。連修三缺陷：worker `No module named 'governance_rule'`（sys.path）、eval suite 檔名≠suite_id 致每循環 `suite-missing`（已改 `{suite_id}.json` 慣例）、venv 缺 `mpmath`（uv.lock 漂移，已補 1.3.0）；policy 新增 `gpu_required_mb` 透傳（1200MB 實測需求取代 2500MB 預設）。第二次循環進行中，待真實 eval 裁定 | 循環機制已驗；通過性 verdict 待第二輪 |
