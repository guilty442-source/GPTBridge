...
> CPU rate control（硬帽，以全機百分比計）對持續超限行程設限，可釋放；③**背景模式**——
> PROCESS_MODE_BACKGROUND（I/O／記憶體優先背景化，CPU 至 IDLE）；④**EcoQoS 效率模式**——
...
| E14 | **收尾組**：EC-1–8、DSN（G89）、Embedding 殘項、效能殘項、A548 全面責任檢討 `[處理中 @devin 2026-09-22T14:10Z — A548 全面責任檢討]` | 低：可並行收尾 | 各自前置 |
