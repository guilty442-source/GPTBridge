...
| G31 | 資源／推論優化缺口（R1–R10） | **已落地**：R2（C++ 引擎全經 C ABI）、R4（auto_release 已接線引擎生命週期）、R7（`native_engine` 延遲載入 native_transformer/torch：import 2.30s/1125 modules→0.115s/112 modules，輕量路徑不付 torch 成本）、R8（統一 `cpu_thread_budget`）；**部分**：R1（訓練已走 governed job 子程序＋GPU gate，常駐推論服務經 `model_service_http` sidecar 成形）、R10（idle/pressure→UNLOADED 已接；`*_TRAINING` 狀態資源動作未接）；**未做**：R3（cuBLASLt/flash profiler 證據，GPU 被 medium 訓練占用）、R5（MoE 稀疏執行/grouped GEMM）、R6（KV 分頁）、R9（batch>1／continuous batching）（詳 4.7） | RAM／VRAM／CPU 與吞吐 |
