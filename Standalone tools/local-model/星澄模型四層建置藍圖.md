...
> 目標：Git 流程單一收斂核心；提交／推送／收斂全自動且受治理；現況殘項：協調者推送需待 backend
> 重啟後生效（manifest `push:true` 已入組態，運行中服務為舊碼）＋F 驗收之週期性 ahead==0 待持續證明。
...
| E3 | **G66 封印與外部簽章**（root 重算、revision/epoch、簽章、re-anchor、certification） | 高：法典權威完整性 | governor 側 |
| E4 | **G67 規則 →active**（45 條 mapping＋semantic-hash 規格＋amendment） | 高：64 條規則效力待決 | 規格（repo 外）＋amendment |
...
| E8 | **對話品質**：GPU 訓練 v14（5711 筆）→ v15 啟用 → L5 重認證 | 中：產品能力缺口（現役 0/4） | GPU 預算；品質達標；**啟用裁決延後（2026-09-22 總督：INT-10 後再定）** |
| E9 | **休眠啟用**（量測已過） | 中：資源回收 | E1 完成後核定 |
...
| 級別 | 項目 | 狀態 |
| --- | --- | --- |
| P0 | **G66 封印與外部簽章**（governor 側） | 細項：①重算 `content_root`②重算 `identity_root`③重算 `full_root`④`revision_history` 追加條目（prev hash→entry hash）⑤更新 `epoch_seal_manifest`⑥`metadata` 版本軸一致（codex／current／binding／identity）⑦外部簽章⑧authority re-anchor⑨certification 完成⑩稽核 PASS |
| P0 | **G67 規則 →active** | 細項：①45 條 `formal_rule_mapping` VERIFIED 列②四個 semantic hash canonical 推導規格（repo 外）③產生並登錄 VERIFIED 列④amendment（狀態轉移＋parity 證據）⑤mirror／derived 重建＋稽核 PASS |
...
| P0 | **按需啟動與休眠殘項**（§10.66） | 量測全過（冷 1.59s／暖 1.42s／30min 無抖動）；殘項：①`enabled` 啟用待總督核定（INT-10 窗口後）②法典明文 |
...
| P1 | **C++ 推論執行層**（P3／G29／G31／G32／G34） | 殘項＝長時驗證（窗口型）＋device-resident KV（待 GPU attention kernel）＋bf16 GEMM／自研 kernel（nvcc/MSVC-18 不相容待新工具鏈）；其餘（INT8／INT4 量化、CUDA cuBLAS 橋接、device-resident 權重、KV INT8 cache、W1、R5、R9、P3f GPU 協調閘）已閉，證據見 `convergence/` 與 Git 歷史 |
...
| 編號 | 缺口 | 剩餘工作 |
| --- | --- | --- |
| G29 | C++ 推論執行層 | 殘項＝長時驗證＋device-resident KV（待 GPU attention kernel）＋bf16 GEMM／自研 kernel（待工具鏈） |
...
| G67 | 評估器 parity | 殘項：`→active` 需 45 條 `formal_rule_mapping` VERIFIED 列之 semantic-hash canonical 規格（64/64 已 verified） |
...
8. 評估器 parity：→active 待 mapping semantic-hash 規格（G67 殘項，governor 側）。
...
1. 量測：`SleepPolicyManager` 於測試環境啟用，量測喚醒時限（溫 ≤2 s／冷 ≤5 s）與 30 min 無抖動（樣本留檔）。
2. 證據：`xingcheng/runtime/logs/` 或 `main-system/runtime/logs/` 之 sleep 審計 jsonl＋量測報告。
3. 達標後：政策 `enabled=true`（受管變更、留審計）；未達標即修正，不得放行。
