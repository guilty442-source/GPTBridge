...
| G29 | 推論引擎元件仍為 Python | C++ 執行層第一切片已落地（`src/backend/cpp/`：mmap loader＋SHA-256、BPE、Dense forward、KV cache、greedy、bounded JSON；張量運算經 C ABI；`cpp_export.py` bundle 匯出；dense batch=1 FP64 parity 5 tests）；MoE／量化／prefix cache 重用／CUDA／batch>1／生產路由 fail-closed 或未接線 | 部分落地；P3b–P3f 完整驗收與 P3f 資源接線未完成 |
...
| G83 | 【未完成】resident 重啟迴圈與 registry 陳舊（部分完成） | `core_system/process_registry.py` `reconcile()` 已存在並由 `toolbox_service`／`toolbox_shutdown` 接線；新增 `toolbox-process-registry-reconcile` 定期排程（30s）並於主系統關閉前停止；仍待 30 分鐘持續運行實證 | 資源浪費、registry 可信度 |
...
- 現況（2026-09-21 第一切片）：`src/backend/cpp/`（`xingcheng_inference.hpp`／`engine.cpp`／
  ...
  與 tool-call 解析已驗（`test_cpp_inference.py` 5 tests）；MoE、INT8/4、prefix cache 重用、CUDA、
  batch>1 皆 fail-closed；P3f 的 `auto_release`／`gpu_coordinator` 接線與生產路由 feature flag 未完成。
...
- 現況：`process_registry.reconcile()` 已存在並由 toolbox 接線；`toolbox-process-registry-reconcile` 已加入 30 秒定期排程，主系統關閉前停止。
- 剩餘：30 分鐘持續運行的陳舊列清理實證；重啟原因 stderr 捕捉（§10.41）。
...
**統計**：完成 **46**、未完成 **45**（未完成：G2, G3, G5, G8, G9, G12, G15, G17, G24, G26, G27, G28, G29, G31, G32, G34, G36, G38, G40, G41, G45, G48, G56, G57, G59, G60, G64, G65, G66, G67, G68, G69, G70, G71, G72, G73, G74, G75, G76, G77, G81, G83, G85, G89）。
...
| G62 | ~~Request Lifecycle／Streaming 驗收未做~~ **已解**：十項驗收套件 23 測試 PASS | 完成 |
