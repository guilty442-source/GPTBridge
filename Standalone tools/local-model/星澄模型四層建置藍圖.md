...
- ~~**待辦**：IPC p95／SQL lane／RAG 階段耗時由各持有者在執行期注入~~ → **IPC 延遲注入已落地**：`ipc/latency_ledger.py`（1024 樣本環形帳本，p50/p95/p99＋per-command）＋`server_commands` dispatch 計時埋點＋`tasks/perf_baseline_job.py`（automation_core `perf-baseline` flow，300s、pausable，僅寫 latest 防快照累積）；SQL lane／RAG 階段待各持有者注入同介面
...
> 現況 xingcheng-native 18 版→下輪退役 16 版。RAG index 世代排程已落地——
