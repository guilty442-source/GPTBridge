...
- **未完成**：C++ 推論執行層支援 FP64＋MoE（R5）＋batch>1 packed prefill／continuous decode（R9）＋權重 INT8／INT4 量化（export→bundle→dequant-at-load）＋prefix cache（單序列路徑）；CUDA 仍 fail-closed；
  KV INT8 cache 與生產路由、資源接線未完成（§4.2 P3b–P3f）。
...
- **引擎層目標**：Python 訓練／開發路徑維持（`checkpoint.py`／`tokenizer.py`／`bpe.py`／
  ...
  核心運算續用 C 核心；引擎已支援 MoE／batch>1／權重 INT8·INT4（R5／R9／量化）；**未完成**：CUDA／prefix cache 重用與 batch>1·量化之生產路由、資源接線。
...
| 編號 | 缺口 | 剩餘工作 |
| --- | --- | --- |
| G29 | C++ 推論執行層 | MoE／batch>1／權重量化已由 R5／R9／INT8·INT4 解除；CUDA 仍 fail-closed；P3f 剩 GPU 協調語義與長時驗證 |
...
- **未完成（現況）**：`src/backend/cpp/`（`xingcheng_inference.hpp`／`engine.cpp`／
  ...
  **引擎層已支援 MoE（R5）、batch>1 packed prefill＋continuous decode（R9）與權重 INT8／INT4 量化；尚未支援：CUDA（fail-closed）、KV INT8 cache；GPU 協調對 C++ CPU-only 路徑暫不適用。**
