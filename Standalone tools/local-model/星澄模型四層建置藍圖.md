...
- **待辦**：各模型載入點逐步改經 `request_load`（漸進遷移）。
  ...
  殘：Ollama 按需啟動、embedding／reranker 載入點、引擎內部 CUDA 載入路徑。
...
| G50 | SQL／RAG 一致性與模型排程殘項 | workload lanes 直連已清零（殘＝`pool.execute` 一次性介面遷移）；parity audit 已落地；model resource：broker 載入點已接 `request_load`（殘＝Ollama／embedding／CUDA 載入點）；transport 殘＝LISTEN／NOTIFY 消費端 |
