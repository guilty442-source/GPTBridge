...
| G50 | SQL／RAG 一致性與模型排程殘項 | workload lanes 直連已清零，`pool.execute` 一次性介面已遷移單次唯讀站點；parity audit 已落地；model resource：broker＋local-embedding＋ollama-embedding 載入點已接 `request_load`，Ollama 需求啟動已落地（殘＝reranker／CUDA 載入點、ProcessRegistry 登錄、開機自啟落差裁定）；transport：LISTEN／NOTIFY 消費端已落地（`shared_layer/transport_notify.py` 共享監聽器＋`ConnectionManager.dedicated_connection` 專用連線；broker `ai` 頻道事件喚醒、輪詢保留 fail-closed 備援；殘＝sleep_policy drain／toolbox_execution 等其他輪詢方接入訂閱） |
