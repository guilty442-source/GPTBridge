...
3. **課程選擇（訓練什麼）**：依 maturity 未達項決定課程（SFT／DPO／續訓／專家模組）；每循環只推一個課程，失敗即停。
