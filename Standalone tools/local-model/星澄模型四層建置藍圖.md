...
| G67 | 正式規則 parity 流程進行中 | request `codex-amendment-request-formal-rule-parity-lifecycle-2026-09-21-r2` 已建（predecessor `2026-09-21T07:37:58Z`、history head `a9e4c794`、revision 65）；candidate DB 67 條全轉 `declared-pending-evaluator-parity`（missing evaluators 0、非法 transition 0、sha `19a96a18…`）；狀態 `successor-built`、lineage lock 持有中，待五方稽核＋governor 裁定 | 規則正式化與 parity 證據（governor 側） |
...
| G69 | 法典更新工具鏈已建（待 governor 啟用） | `codex_successor_builder`（request→candidate DB＋manifest，狀態轉移與 evaluator parity fail-closed）、`codex_amendment_lifecycle`（predecessor 過期拒絕＋lineage lock）、`codex_amendment_audit_gate`／`audit_runner`／`update_pipeline`／`update_validation` 已具；toolchain 測試 68 passed | 五方稽核與正式發布屬 governor（詳 §10.36） |
...
| G85 | 【部分解決】Backend Token 暴露給 Renderer | `app:get-backend-session` 不再回傳 token；Main process 產生 30 秒、workspace-bound、HMAC ticket，Renderer 只取得含 `ticket` 的 loopback WebSocket URL；backend `server_tokens` 驗證簽章、expiry、instance 並實施 nonce one-time use；完整 E2E handshake regression 待補 | 安全邊界（詳 §10.43） |
...
**整體**：G 缺口 **完成 68、部分落地 6（G29／G30／G39／G56／G67／G83）、未完成 17**（2026-09-21 以主帳本狀態欄重算，收斂 G3/G5/G73/G75/G80/G81/G84 與並行工作者 G15/G57/G90/G12/G28 等結案）；施工批次 **W0／W1 完成、W2–W4 未完成**；
Phase／P 標記 **7/23（≈30%）**。已完成部分集中在
「法典施工、模型訓練／評測前段、RAG 混合平面驗收、Release 驗證層與 RC 封裝演練、效能優化系列（O/S/W/X/Y/Z/AA）」；
未完成部分集中在「法典封印與工具鏈殘項（G66–G68；G69–G71 工具鏈已施工待 governor 裁定）、
治理阻塞（G85 token／G89 DSN／W5-5 審批）、C++ 推論層（P3/W1/G41/R5/R9）、模型品質（G40 九類未達標）與端到端演練（G36/G59 INT-10 24h、G83 30 分鐘實證）」。
...
**G85 Backend Token 邊界（部分解決）**
- 現況：Main process 以 IPC session token 產生 30 秒 HMAC ticket；Renderer 僅取得 loopback WebSocket URL＋ticket，不取得長期 token。
...
- 剩餘：補完整 WebSocket handshake E2E／replay 負向測試，將 policy 由 acknowledged gap 轉正式 must-not-expose。
