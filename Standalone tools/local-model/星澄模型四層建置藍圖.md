...
| 級別 | 項目 | 狀態 |
| --- | --- | --- |
| P0 | **法典上線殘項**：封印根／修訂歷史與外部簽章（G66）；評估器 parity（G67） | 未完成（governor 側） |
...
| G67 | 評估器 parity | 各規則的 evaluator 實作與 parity 證據（→verified→active）待施工；live 法典 64/67 條已在 `declared-pending-evaluator-parity` |
...
8. 評估器 parity：各規則的 evaluator 實作與 parity 證據（G67）。
