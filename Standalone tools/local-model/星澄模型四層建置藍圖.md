...
- **測試與審計唯一入口（2026-09-22 總督核定）**：**所有工作者的測試與審計只能使用「主系統的測試套件與審計套件」**
  ...
  不得以 Python pytest 作為閘門；結果一律以主系統套件輸出為準（逾 30 s 中止並修正，§3.1）。
...
> C① 推送前**強制測試閘已實作**：`git_tiers/push_gate.py` 以原生 `native/test_suites` 套件為閘——建置→逐套件有界執行→
> FAIL／逾時／缺工具鏈即拒推（fail-closed；`BLOCKED` 記 incomplete_evidence 不拒推）；接線 `repo_sync.evaluate_push_gates`＋
> `workspace_sync.synchronize`；tunables `automation-flows.json`→`flows.git-automation.push_gate`；測試 19/19
> （`main-system/tests/test_push_gate.py`）；套件最新實跑 134 pass／0 fail／2 blocked（手動閘驗；`874309dc`）。F① **ahead==0 收斂證據已實作**
