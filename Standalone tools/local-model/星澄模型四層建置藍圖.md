...
- 現況：權重僅以 checkpoint／hash／generation 識別，**無對外版本號**。
- 設計格式：`star-<tier>/v<major>.<minor>.<patch>[-int8|-int4]`；
  major＝架構或能力層級變更（如 L5→L6）、minor＝資料／訓練改善、patch＝量化或 eval 修正。
- 版本綁定：lifecycle weights version、checkpoint sha、tokenizer sha、資料集版本、eval suite 版本；
  發行以版本號＋hash pin 至 `native-engine.json`。
- 規則：每次 `activate` 必須有唯一版本號；maturity／eval 報告一律引用版本號；
  回滾回復上一版本號；不得重用或跳號覆蓋。
- 驗收：連續兩次升級可由報告追溯「版本號→checkpoint→資料集」；rollback 後版本號正確。
...
| P0 | 主系統 Python 常駐資源縮減（§10.63） | 規劃已定；未施工 |
| P0 | 全系統收斂殘項：INTEGRATION-10 24h 驗收（G59）；04B-10 隔離啟動 harness（§10.52） | 未完成 |
...
| G65 | 證據 checkpoint 保留缺口 | 保留策略已保護 lifecycle 引用、engine pin 與 maturity／self-learning evidence 報告路徑；既有被清理的 `chat-foundation-v1/final.pt` 需由重新產出的證據鏈補回 |
...
- INTEGRATION-04B 04B-10：隔離啟動 harness 已建並實測 PASS（§10.52）；正式判定待完整驗收重跑。
...
- 隔離啟動 harness（04B-10）與 venv／lock 收尾（G76／G77 殘項）→ 見 §10.52。
...
- **04B-10**：隔離啟動測試替身已建（§10.52）；待 `--with-isolated-start` 完整驗收。
...
**判定：NOT_COMPLETE**（04B-10 harness 已建並實測 PASS；正式判定待 `--with-isolated-start` 重跑完整驗收）。
...
**R1 啟動瘦身進度（2026-09-21）**：依賴探測改有界並發（≤4，`startup_executor_phases.py`，
commit `2127ce36` 自動收錄）——phase-5 1,750→1,484 ms；**但三項依賴現今皆可達（probe ≤3 ms），
殘餘 ~1.5 s 為階段邊界內的事件迴圈競爭＋lazy import**，需進一步歸因後施工（probe 並行化仍保留，
語意不變、fail-closed 不變）。
