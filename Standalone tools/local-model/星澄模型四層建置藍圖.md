...
| P0 | **法典收斂殘項**（§3.5 A／E）：內部去重（boilerplate／模板／跨層／Git 五重述）＋**業務規則下放主宰**（法典只留治理／權責／邊界） | 未施工；細項見 §3.5 A／E |
...
- **原則**：法典只保留治理、權責與邊界；**新增業務規則一律下放至對應主宰（sovereign／module domain）持有，
  不再寫入法典**。法典對業務規則只保留引用（必要時）與權責歸屬。
