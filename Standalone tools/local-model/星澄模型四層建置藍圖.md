...
| G50 | SQL／RAG 一致性與模型排程殘項 | workload lanes 直連已清零（殘＝`pool.execute` 一次性介面遷移）；parity audit 已落地；model resource 殘＝各載入點改經 `request_load`；transport 殘＝LISTEN／NOTIFY 消費端 |
