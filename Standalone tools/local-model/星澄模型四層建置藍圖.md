...
| P0 | **G76／G77 收尾** | 細項：①G76 venv 自足驗證②G77 `uv.lock` 與 manifest lock 雙向一致性 |
...
| P2 | **依賴鎖與 Embedding 殘項** | 細項：①G77 `uv.lock` 與 manifest lock 雙向一致性②G50 漂移後新世代建置觸發（遷移工具） |
...
| G77 | 依賴鎖 | 殘項：`uv.lock` 與 manifest lock 雙向一致性驗證 |
...
2. ~~venv 於最終隔離位置重建或複製（G76 殘項）＋`uv.lock` 接入（G77）~~ → **已驗證**：
   ...
   離線重建 venv 成功且 dist 集與 lock 完全一致。
