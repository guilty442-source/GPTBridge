...
| E13 | **自主訓練殘項**（續訓/專家課程型別、3 循環驗收、法典 amendment、eval tps 閘裁決） `[處理中 @devin 2026-09-22T12:45Z]` | 中低：能力進化 | 法典 amendment 需協調窗口；3 循環需 GPU 窗口；eval tps 待裁決 |
...
| G40 | 原生對話訓練（機制面全閉） | loss mask 驗證＋續接測試完成（證據 `g40-lossmask-continuation-20260922.json`）：僅 assistant 內容+`<|eot|>` 保留 label（system/user/tool/role-marker/BOS 全遮罩）；「繼續」回覆三情境互異＝狀態相依非固定答案；九類探針 L5 gate 3/4 已達；品質裁決（echo 漸近線／OOD parrot）移交 P1 殘項④⑥ |
