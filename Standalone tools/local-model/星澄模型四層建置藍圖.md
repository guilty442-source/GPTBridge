...
| G78 | 【未完成】Backend release payload 快照未記入 manifest | `package_integrity.collect_file_hashes`／`snapshot_digest` 能力已具，但 backend release manifest 尚未記錄 payload 快照 → 無法自動偵測 `pip install/upgrade` 原地修改 | Release 不可變性（詳 §10.39） |
| G79 | 【未完成】local-model runtime 依賴未分類 | `shared_layer_classification` 已列 13 項；`local-model`（星澄）模組／Ollama 按需 runtime 未列入契約（權重不得封裝已入 `forbidden_content`） | 共享依賴完整性（詳 §10.39） |
| G80 | 【未完成】.exe／.dll 原生清單未實測 | `pe_machine`／`native_extension_errors` 能力已具；契約僅列 1 個 `.pyd`，`required_dlls` 空 | 原生模組相容（詳 §10.39） |
| G81 | 【未完成】服務重複實例僅驗證層 | `service_topology` 閘門已具（驗證層）；runtime 層無強制（本階段依指示不動服務拓撲、不實作 A/B 啟動） | Runtime 強制（詳 §10.39） |
...
| G84 | 【未完成】Preload 通道漂移 | `preload.ts` `allowedInvokeChannels` 允許 3 個無 Main handler 的通道：`app:get-repair-status`、`app:open-path`、`app:set-ui-zoom`（`ipcMain.handle` 未註冊）→ 呼叫即失敗；IPC surface 快照已捕捉 | 前端契約漂移（詳 §10.42） |
...
**方法**：G 清單（G1–G81）關鍵字判定（已解決／已驗收／已定案／已落地／已具／已建／已結案／✅），
Phase／P 標題標記，W 批次標記；未完成者以 §10.39 之二分標示為準（G78／G80／G81 含「已具」但
狀態為未完成）。

| 軌道 | 缺口關閉 | 說明 |
| --- | --- | --- |
| 模型訓練／評測 | 7/16（約 44%） | Phase 2R／2T／5D／5F／5G、P2 完成；2B 完整基線、5E 一致性、6／7／7B、9／9A／9B 未完成 |
...
| 效能優化系列 | 0/14（0%） | O／S／R／MS／W／X／Y／Z／AA 系列多未執行 |
| 其他（UI／工具／修復） | 3/9（約 33%） | 外部協作、修復 E2E、保留策略等未完成 |

**整體**：G 缺口關閉率 **約 15–18/81（≈19–22%）**；施工批次 **W0 完成、W1–W4 未完成**；
...
**統計**：完成 **33**、未完成 **58**（未完成：G2, G3, G5, G7, G8, G9, G12, G13, G14, G15, G16, G17, G18, G19, G22, G24, G26, G27, G28, G29, G30, G31, G32, G34, G36, G38, G40, G41, G45, G47, G49, G57, G59, G60, G62, G64, G65, G66, G67, G68, G69, G70, G71, G72, G73, G74, G75, G76, G77, G78, G79, G80, G81, G83, G84, G85, G89, G91）。
...
| G11 | RAG 正式路徑驗收 | 完成 |
| G12 | SLA 自動驗收 | 未完成 |
| G13 | C# auto-release 未驗證 | 未完成 |
| G14 | 保留策略首次實證 | 未完成 |
| G15 | 星澄助理／自動修復端到端 | 未完成 |
| G16 | 綁定與建置未分層 | 未完成 |
| G17 | 綁定周邊不一致 | 未完成 |
| G18 | 無 CPU SIMD 純 C 核心 | 未完成 |
| G19 | GPU 自研核心未啟用 | 未完成 |
| G20 | GPU 加速定位 | 完成 |
