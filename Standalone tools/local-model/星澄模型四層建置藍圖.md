...
3. core engine equivalence＋capability dispatch parity：v2 語意碼證據已備（43 模組、17/17 parity）；殘＝runtime 接線＋activation gate。
4. 子主宰 registry 切換與退役（active 6＋13 retired；`module_assignment_registry` 5 active＋12 legacy；registry 欄已移除）；A591／A592 已 active／parity VERIFIED。
5. 67 machine schema parity／obligations／closures 收斂（含 hybrid 分類列）。
6. normative surface 與 search／module manifest 重建。
