...
| P1 | **C++ 推論執行層**（P3／G29／G31／G32／G34） | 細項：①MoE／量化／CUDA／batch>1 解除 fail-closed②**W1 已閉**（`8c9020e4`：streaming QK/PV＋AVX2＋純量回退，logits 差 1.5e-14、prefill256 −24.7%，證據 `c-w1-qk-simd-evidence-20260922.json`；遺留 FlashAttention 分塊／AVX-512 入 P2）③R5（MoE 稀疏執行／grouped GEMM）④R9（batch>1／continuous batching）⑤P3f GPU 協調語義＋長時驗證 |
