...
- 退役不遷移：`global-cleaner`、`language-review`（依 §10.67 F codex-first 刪除）。
...
| 編號 | 項目 | 風險（為何先後） | 前置／依賴 |
| --- | --- | --- | --- |
| E1 | **INTEGRATION-10 24h 驗收**（含預算語義裁定＋error 日誌複驗＋報告）`[處理中 @devin 2026-09-22T12:40Z]` | 最高：系統穩定性未驗收；原窗 12:25Z 後端當機破裂（其後 governance-audit 因受護治理檔可寫而 required-phase-failed 重試耗盡，12:55Z 已重拉起、backend healthy）；控制律預節流修正已落地（6cb00bc7），新窗自本次啟動累積 | **預算語義已裁定（2026-09-22 總督）：嚴格解讀——任一 worker 超限樣本即 FAIL**；控制律須改為預算前預節流使零超限可達（見 §10.64）；本窗 resource_budget 依嚴格語義記 FAIL、其餘判據照舊累積，修正後需另起窗口重驗；窗口滿 24h |
...
| E14 | **收尾組**：EC-1–8、DSN（G89）、Embedding 殘項、效能殘項、A548 全面責任檢討、global-cleaner 刪除 `[處理中 @devin 2026-09-22T12:40Z]` | 低：可並行收尾 | 各自前置 |
