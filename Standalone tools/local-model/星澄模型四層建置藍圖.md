...
**C. global-cleaner 刪除（待前置）**

1. 前置：backup-payload 政策定案（`global-cleaner` 之備份載荷處置）。
2. 步驟：法典 A537 已 retired（codex-first 已滿足）→ 引用檢查（manifest／registry／tests）→ 刪除目錄 → 稽核＋標的測試。
...
- **舊架構（先對齊再刪，codex-first）**：
  ...
  **暫緩刪除**：其 `data/business/backups/` 內含其他工具 35MB 備份包（唯一副本），需先確認備份
  留存政策或遷移後再刪；
