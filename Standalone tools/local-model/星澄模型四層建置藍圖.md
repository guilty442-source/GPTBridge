...
| G83 | 【未完成】resident 重啟迴圈與 registry 陳舊（部分完成） | `core_system/process_registry.py` `reconcile()` 已存在並由 `toolbox_service`／`toolbox_shutdown` 接線；缺**定期排程**與陳舊列清理實證（先前 16 筆 running vs 2 pid 存活） | 資源浪費、registry 可信度 |
...
**G73 Release 非自足（Source Tree 載入）**
- 現況：`extraResources` 已擴充（`governance_rule`／`shared-layer`／`main-system/governance`）；`main.py` 以 `GPTBRIDGE_RELEASE_ROOT` fail-closed。
- 剩餘：封裝後以 `validate_release_python_origins`（packaged root）確認實際載入來源；消除任何 source-tree 洩漏。
- 依賴：G74／G76。
- 驗收：packaged 模式 bundle 驗證 PASS、零 `SOURCE_TREE_LEAK`。

**G74 無 Backend Release Manifest 與 UI×Backend 契約驗證**
- 現況：`runtime/releases` 不存在；僅工具契約與 DB manifest。
- 剩餘：建立 release 佈局與 backend manifest；實作 UI×backend 契約配對驗證（IPC surface pairing 已具，需接上 release）。
- 依賴：G56／G77。
- 驗收：新 backend 對舊 UI 啟動演練 PASS；B 要求 A 不支援之 IPC → 拒絕進入可切換狀態。

**G75 SHARED_RUNTIME 隔離缺口**
- 現況：後端 `shared_layer` 來自 `E:\GPTBridge\shared-layer\src`（Source Tree）。
- 剩餘：以 `SHARED_RUNTIME` 類納入 release 並驗證；**不得搬移／重建 Shared Layer**。
- 依賴：G73／G74。
- 驗收：release 內 shared_layer origin 位於 SHARED_RUNTIME 根；`SHARED_RUNTIME_SOURCE_TREE_GAP` 消除。
...
**G83 resident 重啟迴圈與 registry 陳舊（部分完成）**
- 現況：`process_registry.reconcile()` 已存在並由 toolbox 接線。
- 剩餘：定期排程 reconcile；陳舊列清理實證；重啟原因 stderr 捕捉（§10.41）。
- 驗收：claimed `running` 與實際存活 pid 一致；30 分鐘內無陳舊列；重啟事件有證據。
...
**統計**：完成 **44**、未完成 **47**（未完成：G2, G3, G5, G7, G8, G9, G12, G15, G17, G24, G26, G27, G28, G29, G31, G32, G34, G36, G38, G40, G41, G45, G48, G56, G57, G59, G60, G62, G64, G65, G66, G67, G68, G69, G70, G71, G72, G73, G74, G75, G76, G77, G80, G81, G83, G85, G89）。
...
| G83 | resident 重啟迴圈與 registry 陳舊（部分完成） | 未完成 |
