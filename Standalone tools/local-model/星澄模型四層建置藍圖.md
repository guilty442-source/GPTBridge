...
| P0 | **測試與審計套件 ≤30 s**（A537：強制測試 20 s／審計 30 s；法典既定） | **測試套件已是 C**；**審計須以 C／C++ 實作**（含 pre-commit 稽核流）；細項：①C／C++ 審計引擎（≤30 s）②C 測試套件 ≤30 s 實測③Python pytest／稽核僅過渡或參考，移出提交閘門④快取失效策略（法典變更即失效）⑤負載下連續 10 次提交 ≤30 s 驗收⑥逾時 fail-closed 保留 |
