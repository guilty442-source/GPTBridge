...
| P0 | 資源自動管制與自動釋放（§10.64）：工作者總帳 CPU≤10%／RAM≤30%、四項資源自動釋放 | 大部分實作（2026-09-22）：worker-plane 歸屬＋總帳＋滯回＋⑤admission-hold（`a4775ce5`）＋④pausable 週期工作＋⑥ broker 對自行啟動 owner 的 governed `stop_tool` 釋放＋② 管制中 worker-plane affinity 聚合上限＋`--log-samples` 逐週期樣本（30 min p95 稽核資料源）；引擎/GPU 釋放走既有 auto_release；殘項：30 min 驗收量測執行、法典側資源優先序明文 |
...
> 待辦：真實 30 min 驗收量測執行；法典側資源優先序／預算權屬明文。
