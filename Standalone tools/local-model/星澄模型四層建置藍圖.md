...
## 12. 風險排序執行順序（整合；新編號；2026-09-22 總督核定）
...
| 編號 | 項目 | 風險（為何先後） | 前置／依賴 |
| --- | --- | --- | --- |
| E1 | **INTEGRATION-10 24h 驗收**（含預算語義裁定＋error 日誌複驗＋報告） | 最高：系統穩定性未驗收；原窗 12:25Z 後端當機破裂（其後 governance-audit 因受護治理檔可寫而 required-phase-failed 重試耗盡，12:55Z 已重拉起、backend healthy）；控制律預節流修正已落地（6cb00bc7）；12:55Z 窗因取樣器停擺恢復期兩筆超限污染，2026-09-22 總督裁定作廢重起，現窗 13:23Z 起累積 | **預算語義已裁定（2026-09-22 總督）：嚴格解讀——任一 worker 超限樣本即 FAIL**；控制律已改預算前預節流（§10.64）；現窗自 2026-09-22T14:50Z 起計，滿 24h 後以 `--window-start 2026-09-22T14:50Z` 驗收；**13:23Z 窗已裁定作廢（26+ 筆超限樣本，主因併發施工 fork-storm 快於 5s 採樣）；總督裁定等 Lasso 層穩定再重起——worker-plane 共享 Job Object 聚合硬頂已落地啟用（governor 14:42Z 重啟 `--worker-job-cap`，全 worker 進程加入同一 job、子進程出生即繼承 cap＝10% 聚合預算；實測 8-burner 持續段壓 ~1%、僅新 root ≤1 採樣間隔首見殘差；測試 18/18＋live probe 綠）** |
| E3 | **G66 封印與外部簽章**（root 已重算、revision 鏈完整、re-anchor 完成；殘＝epoch manifest＋外部簽章＋certification） | 高：法典權威完整性 | governor 側（非 worker 承接） |
| E4 | **G67 規則 →active**（64/64 evaluator parity 已驗；殘＝45 條 mapping＋semantic-hash 規格＋amendment） | 高：64 條規則效力待決 | 規格（repo 外）＋amendment（非 worker 承接） |

| E6 | **Python 常駐外移**：E4（治理核心受 C 閘控）＋雙軌 retire＋整體驗收（act-1 shadow 已歸檔：connection_watchdog／periodic_scheduler／state_outbox／maintenance_controller 四件接線完成、`native-shadow.json` 全 shadow；`pyd` 已重建含 NativeMaintenance `system_blocked`／`cancel`／`cache`） | 中高：資源／穩定性（常駐 Python） | E1–E3 已部分落地 |
| E7 | **單元制搬移**（tools→model→shared→core→xingcheng；跨單元介面；隔離實測） | 中高：拓撲與隔離；搬移錯誤影響大 | 層級表；受管搬移流程（**E1 解除後；未認領可承接**） |
| E8 | **對話品質**：候選 v15／v17／v18 已訓練並登錄（L5–L7 於候選通過；現役 v18 對話 0/4）→ 啟用裁決 | 中：產品能力缺口 | GPU 預算；echo／算術缺口；**啟用裁決延後（2026-09-22 總督：INT-10 後再定）**（未認領；裁決屬總督） |
| E9 | **休眠啟用**（量測已過；法典 A599 已 active） | 中：資源回收 | E1 完成後核定（總督核定；未認領） |
| E10 | **啟動 10s 殘項**（對話路由取樣、冷啟分佈、逾時實測） | 中：啟動 SLA 完整證據 | E1 窗口後（**未認領可承接**） |
| E11 | **C++ 長時驗證／device-resident KV／bf16 GEMM／fp8**（eval tps C++ 量測路徑已落地，證據 `e11-eval-tps-cpp-path-20260922.json`） | 中：效能上限 | GPU 工具鏈（**未認領可承接**） |
| E12 | **語言外移 M2–M5**（M1 shadow 進行中；A263 parity 放行套件） | 中低：工程債；不阻擋上線 | M1 完成 |
| E13 | **自主訓練殘項**（續訓/專家課程型別、3 循環驗收、法典 amendment） | 中低：能力進化 | 法典 amendment 需協調窗口；3 循環需 GPU 窗口；eval tps 閘已裁定（2026-09-22 總督：兩者並行——`min_tps_baseline_ratio` 相對閘已落地、證據 `convergence/e13-eval-tps-relative-gate-20260922.json`；C++ engine 量測路徑歸 E11） |
| E14 | **收尾組**：EC-5 法典同步、效能殘項（MS6／MS10） | 低：可並行收尾 | 各自前置（MS6／MS10 未認領可承接；EC-5 待協調窗口） |

**執行規則**：①每項完成後**先重讀藍圖再施作下一項**（§0）②同一時間僅一項進入「提交閘門」以避免 index 鎖競爭
③需 governor 裁定者（E1 預算語義、E5 窗口、E13 rollback）先送選項裁決再動工
④各項依下「分配順序」（P0→P1→P2、級別內依序）承接；受 E1 閘控項於窗口解除後動工。

**分配順序（2026-09-22 盤點後；依 §3.1 級別自 P0 起編；風險表維持風險序）**

代號規則：`P0-N`／`P1-N`／`P2-N`＝級別內執行序；`（governor）`＝非 worker 承接。

P0（最高）：

| 代號 | 項目 | 細項 |
| --- | --- | --- |
| P0-1 | **E1 INTEGRATION-10 24h 驗收**（現窗累積至 2026-09-23T14:50Z） | ①24 h uptime②零非預期重啟／健康探針全綠③RSS／CPU 預算④日誌零 ERROR／零孤兒⑤驗收報告留存 |
| P0-2 | **E6 Python 常駐外移**（act-1 shadow 已歸檔） | ①E1 執行面②E2 傳輸面③E3 啟動面④E4 治理核心⑤雙軌 shadow→primary→retire⑥淨占用量測⑦回退演練⑧整體驗收 |
| P0-3 | **E3 G66 封印與外部簽章**（governor） | ①`epoch_seal_manifest` 更新②外部簽章③certification |
| P0-4 | **E4 G67 規則 →active**（governor） | ①45 條 `formal_rule_mapping` VERIFIED②semantic-hash canonical 規格③amendment④mirror／derived 重建 |
| P0-5 | **E7 單元制搬移**（E1 解除後） | ①專用層級表（總督側 schema）②tools→model→shared→core→xingcheng 受管搬移③跨單元介面強制④單元／模組級隔離實測 |
| P0-6 | **E9 休眠啟用**（E1 解除後） | ①`enabled=true` 核定②啟用後審計與回退演練 |
| P0-7 | **E10 啟動 10 s 殘項**（E1 解除後） | ①對話路由取樣②多次冷啟分佈③逾時 fail-closed 實測 |
| P0-8 | **法典收斂殘項**（§3.5；governor） | ①dupscan `review-required` 跨層重述與模板實例②另行 amendment |

P1：

| 代號 | 項目 | 細項 |
| --- | --- | --- |
| P1-1 | **E11 C++ 推論執行層** | ①長時驗證②device-resident KV③bf16 GEMM／fp8④生產啟用評估（GPU 窗口） |
| P1-2 | **E8 對話品質**（裁決屬總督） | ①啟用或 base 重訓裁決②echo／算術缺口處置③啟用後重認證 |
| P1-3 | **E13 自主訓練殘項** | ①續訓／專家課程型別②3 循環全自主驗收③法典 amendment |
| P1-4 | **E12 語言外移 M2–M5** | ①逐模組 shadow→primary→retire②單一職責③按需不常駐④稽核＋`architecture-languages` 更新 |
| P1-5 | **EC-5 法典同步**（E14） | ①A528／A538 同步②staged request 待協調窗口 |
| P1-6 | **DSN 收尾**（G89；操作者決策） | ①User env `GPTBRIDGE_POSTGRES_*_DSN` 清除 |
| P1-7 | **`retention` watcher 修復**（§2.5／§10.67 殘項）`[處理中 @devin-cli 2026-09-22T16:08Z]` | ①CLI watcher 路徑接 `apply_retention`（dry-run 先行）②首次權重退役（21 版→active＋上一代；`retired` 子表）③`retention.jsonl` 生成與引用保護實測 |
| P1-8 | **推送閘**（§10.69）`[處理中 @devin-cli 2026-09-22T16:11Z]` | ①推送前「強制測試 PASS」閘實作②週期性 ahead==0 持續證明③推送審計入帳 |
| P1-9 | **P6 驗收證據** | ①7 工具 5 s 啟停量測②測試 20 s 專項③隔離 overrides 補齊（其餘 5 工具）④星澄助理／自動修復端到端 |

P2：

| 代號 | 項目 | 細項 |
| --- | --- | --- |
| P2-1 | **執行緒策略入口**（§10.30） | ①統一入口（`OMP／BLAS／PyTorch 執行緒 × 平行工作數 ≤ 預算`）②bounded-workers predicate 進 A590 評測器 |
| P2-2 | **Triton 預設對齊**（ACC-2） | ①決定預設（on／off）②受管設定 note 與程式（`XINGCHENG_TRITON_KERNELS`）一致化③治理登錄 |
| P2-3 | **RAG 階段耗時注入**（§10.11） | ①RAG 階段計時介面②latest 快照 `rag` 非 null |
| P2-4 | **MS6／MS10**（效能殘項） | ①MS6（C# 承接）②MS10（SLO 儀表） |
| P2-5 | **Embedding 殘項** | ①`apply_metadata` executor（依設計 blocked） |
