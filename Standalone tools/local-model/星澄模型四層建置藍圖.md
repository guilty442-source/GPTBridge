...
| E3 | **G66 封印與外部簽章**（root 已重算、revision 鏈完整、re-anchor 完成；殘＝epoch manifest＋外部簽章＋certification） | 高：法典權威完整性 | governor 側 |
| E4 | **G67 規則 →active**（64/64 evaluator parity 已驗；殘＝45 條 mapping＋semantic-hash 規格＋amendment） | 高：64 條規則效力待決 | 規格（repo 外）＋amendment |
...
| E7 | **單元制搬移**（tools→model→shared→core→xingcheng；跨單元介面；隔離實測） | 中高：拓撲與隔離；搬移錯誤影響大 | 層級表；受管搬移流程 |
| E8 | **對話品質**：候選 v15／v17／v18 已訓練並登錄（L5–L7 於候選通過；現役 v18 對話 0/4）→ 啟用裁決 | 中：產品能力缺口 | GPU 預算；echo／算術缺口；**啟用裁決延後（2026-09-22 總督：INT-10 後再定）** |
| E9 | **休眠啟用**（量測已過；法典 A599 已 active） | 中：資源回收 | E1 完成後核定 |
| E10 | **啟動 10s 殘項**（對話路由取樣、冷啟分佈、逾時實測） | 中：啟動 SLA 完整證據 | E1 窗口後 |
| E11 | **C++ 長時驗證／device-resident KV／bf16 GEMM／fp8**（eval tps C++ 量測路徑已落地，證據 `e11-eval-tps-cpp-path-20260922.json`） | 中：效能上限 | GPU 工具鏈 |
...
| E14 | **收尾組**：EC-5 法典同步、效能殘項（MS6／MS10） | 低：可並行收尾 | 各自前置 |
