...
| P1 | **C++ 推論執行層**（P3／G29／G31／G32／G34） | 細項：①量化／CUDA／batch>1 解除 fail-closed（MoE 已由 ③R5 解除）②**W1 已閉**（`8c9020e4`：streaming QK/PV＋AVX2＋純量回退，logits 差 1.5e-14、prefill256 −24.7%，證據 `c-w1-qk-simd-evidence-20260922.json`；遺留 FlashAttention 分塊／AVX-512 入 P2）③**R5 已閉**（MoE 稀疏執行＋分組 expert GEMM：router softmax→top-k→renorm→per-expert gather/SwiGLU/加權 scatter-add；logits parity 2e-3、greedy token-exact、interval 混合層 parity、畸形 config fail-closed；證據 `p1-r5-moe-sparse-inference-20260922.json`）④R9（batch>1／continuous batching）⑤P3f GPU 協調語義＋長時驗證 |
...
| G31 | 資源／推論優化殘項 | **R5 已閉**（見 P1 C++ 推論層行）；**未做**：R9（batch>1／continuous batching） |
...
3. ~~隔離 IPC 測試客戶端~~ → **已驗證**（04B-19 `_ipc_contract_probe` 對活體 RC
   ...
   更深項（cancellation／timeout／streaming 逐命令語義）留作契約套件擴充。
...
release 客戶端契約層驗證）；IPC 逐命令深層語義（cancellation／timeout／
streaming）留作契約套件擴充。其餘 23 項正式驗收全數 PASS 入帳。
