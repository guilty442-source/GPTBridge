...
- **未完成**：真實對話端對端驗收（待 INT-10 後或下一個 RC 驗證）。
...
| 期 | 目標 | 工作 | 驗收 | 依賴 |
| --- | --- | --- | --- | --- |
| ACC-2 | GPU 加速軌（獨立） | 完成 Gluon 另軌評估；不併入純 C 引擎 | 實編譯證據；logits ≤1e-3；與 PyTorch 路徑等價；C 核心零 GPU／C++ | P3a、§5.5 |
...
| P0-7 | **啟動 10s 殘項**（對話路由取樣） | 中：啟動 SLA 完整證據 | P0-1 窗口後；未認領可承接 | 對話路由取樣（待 INT-10 窗口後） |
| P0-8 | **法典收斂殘項**（§3.5；governor） | 高：治理完整性（法典收斂） | governor（非 worker 承接） | ①dupscan `review-required` 跨層重述與模板實例②另行 amendment |
| P0-9 | **Git 協調者進程重載**（§10.69） | 中：在駐程序仍持舊模組 | INT-10 窗口後 | 重載協調者並確認新推送閘生效 |
| P1-1 | **C++ 長時驗證與生產啟用評估**`[處理中 @devin-cli 2026-09-22T17:55Z]` | 中：效能上限 | GPU 窗口 | 執行長時 drift／RSS／tps 驗證及五組態 parity／tps／RSS／fail-closed 生產評估 |
...
| P1-9 | **測試 20 s 驗收**`[處理中 @devin-cli 2026-09-22T16:30Z]` | 低：驗收證據缺口 | governor 裁定 | 全量目前未達 20 s；依正式口徑完成效能收斂與驗收 |
| P2-4 | **MS6 C#／C++ 承接** | 低：效能殘項 | P3／P7 | 完成 C# 編排承接，移除熱路徑 Python 中介 |
