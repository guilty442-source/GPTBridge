...
| P0 | **主系統 Python 常駐外移**（§1.1 目標二；清單與設計已完成，§10.65 E1–E4） | 細項：依 E1→E4 逐元件實作與雙軌切換（shadow→primary→retire）＋整體淨占用驗收 |
...
| P0 | **測試與審計套件 ≤30 s**（A537：強制測試 20 s／審計 30 s；法典既定） | 未達標；證據：全量 pytest **415 s**（4700 項）、pre-commit 審計負載下 **30.3 s／61.7 s** 兩次逾時 fail-closed。細項：①定義 30 s「強制快速套件」範圍（受影響模組）②全量套件移出提交閘門（離峰／夜間排程）③審計熱路徑瘦身（快取／增量／並行）④負載下連續 10 次提交驗收 ≤30 s⑤逾時 fail-closed 保留 |
