...
- **舊架構（先對齊再刪，codex-first）**：
  ...
  ②`Standalone tools/global-cleaner`（法典 A537 retired＋non-executable）——registry／manifest 已對齊；
  **backup 政策已定案**（2026-09-22）：`tool-runtime-contract.json` 宣告其 `data/business/backups/`
  為受管封存庫（`backup_max_records_per_owner=1`、`backup_execution=frozen-retired-non-executable`）——
  已依上限剪定（每 owner 留最新一包，刪 main-system/shared-layer 舊包）＋清 runtime plans/locks 殘留；
  **全目錄刪除前置**：須以受管修訂把備份封存路徑遷出（改 contract `managed_storage.backups`＋
  權限目錄 `tool:global-cleaner` 殘留＋`test_global_cleaner_retired` 同步）後方可刪除；
...
| E6 | **Python 常駐外移**：E4（治理核心受 C 閘控）＋雙軌 retire＋整體驗收 | 中高：資源／穩定性（常駐 Python） | E1–E3 已部分落地 |
| E7 | **單元制搬移**（tools→model→shared→core→xingcheng；跨單元介面；隔離實測） | 中高：拓撲與隔離；搬移錯誤影響大 | 層級表；受管搬移流程 |
| E8 | **對話品質**：GPU 訓練 v14（5711 筆）→ v15 啟用 → L5 重認證 `[處理中 @devin-cli/cosmic-papaya 2026-09-22T12:50Z — 僅殘項④ echo 計數句式補池＋v17 重訓認證]` | 中：產品能力缺口（現役 0/4） | GPU 預算；品質達標；**啟用裁決延後（2026-09-22 總督：INT-10 後再定）** |
| E9 | **休眠啟用**（量測已過） | 中：資源回收 | E1 完成後核定 |
| E10 | **啟動 10s 殘項**（對話路由取樣、冷啟分佈、逾時實測） | 中：啟動 SLA 完整證據 | E1 窗口後 |
| E11 | **C++ 長時驗證／device-resident KV／bf16 GEMM／fp8** | 中：效能上限 | GPU 工具鏈 |
| E12 | **語言外移 M2–M5**（M1 shadow 進行中） | 中低：工程債；不阻擋上線 | M1 完成 |
| E13 | **自主訓練殘項**（續訓/專家課程型別、rollback 實測、3 循環驗收、法典 amendment、eval tps 閘裁決） `[處理中 @devin 2026-09-22T12:45Z]` | 中低：能力進化 | **rollback 語意已裁定（2026-09-22 總督）：prune-latest 優先**——只留最新一代；rollback 目標＝已認證且相容之任一留存版本，無留存版本時 rollback 閘 fail-closed |
...
③需 governor 裁定者（E1 預算語義、E5 窗口、E13 rollback、E14 backup-payload）先送選項裁決再動工。
...
| G41 | 逐層一致性殘項 | MoE／INT8／INT4 量化／CUDA-cuBLAS 逐層 RMS 探針已補（4 探針，證據 `g41-moe-quant-layerwise-probes-20260922.json`）；G41 殘項全閉 |
...
| G77 | 依賴鎖 | **已驗證**：wheel cache 52/52＋離線重建（04B-16/18）＋`uv.lock` 雙向一致性（04B-17：runtime ⊆ uv.lock、版本全等） |
...
- **G40 原生對話訓練**：角色／遮罩格式與 SFT 合併資料已具；九類端到端評估 harness 成立。
  **L5 探針已被證明可達**：`manual-sft-v15`（chat-foundation 4805 筆）L0–L7 全階梯通過——
  缺口為資料規模，已補；echo 逐字複誦有 run-to-run 變異（資料池可再補 echo 型樣本）。
  剩餘：echo 探針穩定化；assistant-only loss mask 驗證；狀態式續接（非固定答案）測試。
...
- **Phase 5 待補**：命令理解探針（`star-semantic-plan/v1` 對齊）✅ `star-semantic-plan-probes/v1`
  ...
  eval suite dialogue v1、人工抽檢紀錄仍待補。
...
**C. global-cleaner 刪除（backup 政策已定案；待 contract 修訂）** `[處理中 @devin 2026-09-22T12:40Z]`

1. 已定案：`tool-runtime-contract.json` 宣告 `data/business/backups/` 為受管封存庫（max-1/owner、
   frozen-non-executable）——已剪定舊包並清 plans/locks；刪除前置＝受管修訂遷出封存路徑。
2. 步驟：contract `managed_storage.backups` 遷移修訂 → 權限目錄 `tool:global-cleaner` 殘留清理 →
   `test_global_cleaner_retired` 同步 → 引用檢查 → 刪除目錄 → 稽核＋標的測試。
3. 殘留：`system-health-check` 宣告與 corpus 序列之引用同步清理。
...
- 細項：①以 runtime-rule-index 內容分類為底，產出「條文→治理／業務規則」對照清單
  ...
  **掃描器已建**（`scripts/codex-business-rule-scan.py` → `convergence/codex-business-rule-conflicts.json`；
  ...
  retired global-cleaner 殘留 `system-health-check` 宣告×2、ai-connections／investment-manager
