...
| P1 | **獨立工具穩定**（§10.41；G82／G83）：① 星澄自動迴圈 arity（`auto.py:216` 無參呼叫）；② resident 啟動去重＋`reconcile` 排程；③ 工具 stderr 捕捉／輪替 | worker（governance/xingcheng、toolbox、process registry） | 待施工（順序 ①→②→③；驗收：10 分鐘零 arity 錯誤、registry 與存活 pid 一致、重啟有 stderr 證據） |
| P1 | **推理線路與稽核完整性**（§10.45；G86／G87／G88）：W5-1 jieba lazy import ✅（並行工作者完成）→ W5-2 補 self-health 測試 → W5-3 ai-collaboration ✅（隨 W5-1 解除）→ W5-4 GPU 復驗（待顯存）；法典 A593／A594 已立案（request-only） | worker（chinese-semantic-engine）＋治理側 | W5-1／W5-3 完成；W5-2 待施工；W5-4 待顯存空檔；驗收：稽核 self-health 全綠、帳本新增 `"device":"cuda"` |
...
| G30 | C# 業務編排層部分落地 | `business-logic-csharp` 已具 Domain/Application/Infrastructure 與 17 項測試；模型服務 HTTP 契約、正式 Python endpoint 整合、Release 打包驗證仍未完成 | 四語言定位缺口（P7） |
...
| G44 | 權重演進機制未端到端 | 演練已完成（Phase 7B `test_weight_evolution.py` 9 綠）；能力矩陣 #5 已證（xl-v3 L5）；#6 待 P3 C++ 層；#7 MoE 收斂中（medium_moe 預訓練進行） | 發布安全與能力證據（詳 2.3、Phase 7B） |
...
| G48 | Contract 相容驗證未建 | `contract_validator.py` 五版本軸相容已建（`e5dffe4`） | 前後端欄位變動直接故障 |
...
| G56 | Release Manifest 未建 | `active_release_verify.py` manifest 驗證＋payload snapshot 已建（28 測試）；實體 release 待 G57/G73 | 切換前無法判定相容性 |
...
| G73 | 【未完成】Release 非自足（Source Tree 載入） | `extraResources` 已擴充為 `.venv`／`src-core`／`config`／**`main-system/governance`／`governance_rule`／`shared-layer`**（並行工作者，2026-09-21）；`main.py` 已以 `GPTBRIDGE_RELEASE_ROOT` fail-closed。殘餘：封裝後仍需以驗證器確認實際載入來源（G74／W4-0c） | Release 自足性（詳 §10.37） |
| G74 | 【未完成】無 Backend Release Manifest 與 UI×Backend 契約驗證 | `main-system/runtime/releases` 不存在；只有工具契約 `tool-runtime-contract.json`（v1／min v1）與 DB manifest `database-release.json`；新 Backend Release 與舊 UI 的相容性無 manifest 可驗 | Release 契約缺口（詳 §10.37） |
...
| G76 | 【未完成】venv 漂移與可搬移性未驗證 | `main-system/.venv` 的 `pyvenv.cfg` 記錄建立於 `E:\GPTBridge\.venv`（實體在 `main-system/.venv`，已位移）；`shared-layer/.venv` 為 Python **3.14.5**（與後端 3.11.9 不同版）；`local-model/.venv` 不存在（用主 venv）。Windows venv 不可假設可複製；封裝前須在**最終隔離位置重建** runtime env | Release Runtime 可搬移性（詳 §10.38） |
| G77 | 【未完成】依賴鎖未接入 Release 流程 | 已有 `main-system/uv.lock`、`requirements*.txt`、`doctor_environment.py`；**無 wheel cache／hash validation／環境建置器**接入 release；本次補：lock identity（sha256 排序 dists）、runtime env manifest 欄位、native 身分 | 依賴可驗證性（詳 §10.38） |
...
| G80 | 【未完成】.exe／.dll 原生清單未實測 | `pe_machine`／`native_extension_errors` 能力已具；契約僅列 1 個 `.pyd`，`required_dlls` 空 | 原生模組相容（詳 §10.39） |
| G81 | 【未完成】服務重複實例僅驗證層 | `service_topology` 閘門（驗證層）＋**新增 runtime 強制**：ToolboxService per-tool start lock（`_tool_start_locks`，G83 去重）＋`backend_lifecycle.register` 拒 `already-registered`；跨程序單例（lease fencing 已具）全覆蓋仍待驗收 | Runtime 強制（詳 §10.39） |
| G82 | 【未完成】星澄自動迴圈 arity 缺陷 | `main-system/governance/sovereigns/xingcheng/auto.py:216` 呼叫 `_notify_anomalies(batch)`，但 `review.py:185` 簽名為 `async def _notify_anomalies(self)` → 每輪 TypeError；自後端啟動首輪起每 ~10 秒記錄 `Xingcheng auto-loop error`（`backend-20260921.log` 01:57:17 起連續），異常持久化靜默失效。最小修：`auto.py:216` 改無參呼叫（batch 已於前一行併入 `_pending_anomalies`） | 星澄自動學習／異常通知失效＋日誌噪音 |
| G83 | 【未完成】resident 重啟迴圈與 registry 陳舊 | 單一後端 session（01:57→06:03 UTC）內 `resident-start-shared-layer-*` **15 筆**（各自新 pid，僅最後 1 個存活）；`process-registry.json` 16 筆 health=`running` 但實測僅 2 pid 存活（shared-layer 48816／file-sorter 39740）→ `reconcile` 未生效；工具 stderr 全 0 bytes（崩潰輸出未捕捉），重啟原因不可判 | 資源浪費、registry 不可信（G51 宣稱已落地但實測不符） |
| G84 | 【已解】Preload 通道漂移 | `app:get-repair-status` 為後端 command-router 通道（非 Electron invoke）——自 allowlist 移除；其餘 23 通道皆有 `ipcMain.handle`；**契約測試 `test_preload_channel_contract.py` 3 項**（allowlist↔handler、gptBridge 面、防回歸） | 已解 |
...
| G91 | 【未完成】self-health 測試檔被移除 | `governance_rule/tests/test_global_cleaner_retired.py` 實體不存在（並行工作者移除；self-health 清單仍要求）→ 稽核 fail-closed（2026-09-21 晚） | 稽核紅燈（worker 需補檔或治理側更新清單） |
...
- 缺口：見第 3 章 G19（GPU 自研核心未啟用）；G18 CPU SIMD 與 G21 VRAM 預算已完成，
...
| 原生模組相容（.pyd/.dll/.exe、ABI、arch、缺 DLL） | `pe_machine`／`native_extension_errors` | 未完成 | G80（.exe/.dll 清單未實測） |
...
**統計**：完成 25 項、未完成 6 項（對應 G74／G76／G78／G79／G80／G81；G73／G75 為已確認之隔離缺口本身，列於「完成」列之未完成欄）。
...
| 軌道 | 缺口關閉 | 說明 |
| --- | --- | --- |
| 模型訓練／評測 | 8/16（約 50%） | Phase 2R／2T／5D／5F／5G、P1／P2 完成；**Phase 5 L5 已認證**（命令理解探針／eval suite／人工抽檢待補）；2B 完整基線、5E 一致性、6／7／7B、9／9A／9B 未完成 |
| 法典／治理 | 3/14（約 21%） | 法典直接施工完成（A551–A592、鏡像、架構圖）；封印根、修訂鏈、更新工具鏈（A1–A10）、目錄對齊未完成 |
| Release 隔離 | 0/9（驗證層已具） | 驗證器／契約／32 測試完成；封裝、backend manifest、venv 重建、payload 快照未完成（G74／G76–G81） |
| 主系統收斂 | 13/19（約 68%） | lease／lifecycle／registry／blue-green／release manifest／backend contract／reconnect+backpressure／retention 已落地（G46/G48/G52–G56/G58/G61/G63；並行工作者＋本輪）；G47 統一 Runtime State、G49 維護閉環、G57 dev/prod 分離、G59 INTEGRATION-01、G62 串流驗收未完成 |
| 效能優化系列 | 部分（非整批完成） | 已落地：MS1–MS4、W2/W3/W4/W9、Z1/Z2/Z8/Z12/Z15、AA13、R8；O／S／Y 批與其餘項未執行 |
| 其他（UI／工具／修復） | 3/9（約 33%） | 外部協作、修復 E2E、保留策略等未完成 |

**整體**：G 缺口關閉率 **約 15–18/81（≈19–22%）**；施工批次 **W0／W1 完成、W2–W4 未完成**；
Phase／P 標記 **6/23（≈26%）**。綜合加權估計 **整體約 20–25%**；已完成部分集中在
...
| Preload → Electron Main | `ipcMain.handle`（24 註冊、22 唯一；app:／dialog:／embedded-browser:） | **漂移 G84**：3 通道無 handler |
...
**W5-4（GPU 復驗）**
- 執行：`python scripts/inference-loop.py --prompt "你好，請介紹星澄系統" --loops 2 --seed 42`。
- 驗收：`[META] device=cuda`、帳本新 cuda 執行、tok/s 與 latency 記入 §10.44。
...
| --- | --- | --- | --- |
| 高效能 | 純 C 核心＋SIMD；C++ 推論層；批次 GEMM；GPU 獨立軌；KV／prefix cache；INT8／INT4；MoE 稀疏 | GPU 自研核心未啟用（G19）；稀疏執行未建（R5）；C++ 推論層未建（G29） | P3（引擎）、ACC-2 GPU 驗證、Phase 9（MoE 收斂） |
...
| 高穩定 | Blue-Green＋**Execution Lease**（§10.17）；Task Lifecycle／INTERRUPTED（§10.16）；三層復原（§10.14）；維護×更新（§10.15）；有界重試 | 已落地：lease（G55）、lifecycle（G54）、連線層（G63）；待建：統一 Runtime State（G47） | INTEGRATION-01 盤點（G59） |
...
6. Phase 9A medium／base 擴展＋Phase 9B MoE QC 補完（速度/VRAM 對照）。
...
**統計**：完成 **40**、未完成 **51**（未完成：G2, G3, G5, G7, G8, G9, G12, G14, G15, G19, G22, G24, G26, G27, G28, G29, G30, G31, G32, G34, G36, G38, G40, G41, G45, G47, G49, G57, G59, G60, G62, G64, G65, G66, G67, G68, G69, G70, G71, G72, G73, G74, G75, G76, G77, G80, G81, G83, G85, G89, G91）。
...
| G16 | ~~綁定與建置未分層~~ **已解**：public C ABI／C core／C++ pybind11 binding 分層，`build_native.py --check` PASS，MSVC `/Tp`／`/Tc` 實證 | 完成 |
| G17 | ~~綁定周邊不一致~~ **已解**：舊 `_gptbridge_native*.pyd` 清理；dispatch summary 改 `native-compute-core`；`dist-native` 優先載入 | 完成 |
| G18 | ~~無 CPU SIMD 純 C 核心~~ **已解**：AVX-512/AVX2/scalar runtime dispatch；`native_core_v1.json` 全 parity PASS；dispatch tests 12/12 | 完成 |
| G19 | GPU 自研核心未啟用 | 未完成 |
| G20 | GPU 加速定位 | 完成 |
| G21 | 6 GB VRAM 預算未定 | 完成 |
| G22 | 五面無單一整合分工規格 | 未完成 |
| G23 | ~~Git 審計與 PG audit 未打通~~ **已解**：`audit_integration.py` 中央稽核轉接器，sync journal→`gptbridge_audit.event`；`test_audit_integration.py` PASS | 完成 |
| G24 | SQL 治理規格多數未落地 | 未完成 |
| G25 | CAG／DAG 無正式使用率與端到端驗收 | 完成 |
| G26 | 低負載機制未量化（部分改善：`perf_baseline_snapshot` §10.11 實測落盤＋workload lanes 曝池使用率；輪詢改事件仍缺） | 未完成 |
| G27 | 審核管線重複掃描／讀取（O1–O7 未動工） | 未完成 |
| G28 | 第二批效能缺口 S1–S8（**S1 已完成**：純 C AVX-512/AVX2 runtime dispatch＋benchmark parity；S5 排除清單已存在待實測、S7 已走連線池待驗收、S8 僅 worktree 檔非議題；S2–S4/S6 未做） | 未完成 |
| G29 | 推論引擎元件仍為 Python | 未完成 |
| G30 | C# 業務編排層未落地 | 未完成 |
| G31 | 資源／推論優化缺口（R1–R10） | 未完成 |
| G32 | 主系統十大優化缺口 MS1–MS10（**MS1–MS4 已落地**：懶載入 RAG/CAG、按需模型啟動、GPU 協調接線、熱更新健康閘門——見 AGENTS.md＋`test_p0_lazy_lifecycle_handover.py`；MS5–MS10 未完成） | 未完成 |
| G33 | 【優先】預訓練資料管線未完備 | 完成 |
| G34 | 第三批效能缺口 W1–W11（**W2/W3/W4/W9 已修**——vector 記憶化、oneshot 快照、dispatcher 零拷貝、函式內 import 清除；W7 實測上限已存在；W1/W5/W6/W8/W10/W11 未做） | 未完成 |
| G35 | NTP 正確性測試未成矩陣 | 完成 |
| G36 | 外部協作未完善 | 未完成 |
| G37 | Small 訓練基線未成報告 | 完成 |
| G38 | Medium／Base 擴展策略未執行 | 未完成 |
...
| G43 | 原生能力評估套件未建立 | 完成 |
| G44 | 權重演進機制未端到端 | 完成 |
| G45 | 效能優化第五批新項（Y5 反向索引／Y6 批次 upsert／Y13 無界歷史／Y14 記憶化待核對／Y15 分片指標皆未做） | 未完成 |
| G46 | ~~不中斷更新未建~~ **已解**：`blue_green_orchestrator.py`＋A330 `boot_core_handover.py`（standby 就緒→健康探針→穩定窗→global-success）；7 測試 PASS | 完成 |
| G47 | 統一 Runtime State 未建 | 未完成 |
| G48 | ~~Contract 相容驗證未建~~ **已解**：`contract_validator.py`（`star-contract-registry/v1`，五版本軸相容檢查、fail-closed）；`e5dffe4` | 完成 |
| G49 | 預防性維護閉環未建 | 未完成 |
| G50 | SQL／RAG 一致性與模型排程未閉環 | 完成 |
| G51 | Process Registry 與效能基準未建 | 完成 |
| G52 | ~~三層復原與事件序列未建~~ **已解**：`state_outbox.py`/`state_outbox_store.py`（A195 單調 revision+sequence+ack cursor+gap 偵測）；`test_g52_state_outbox.py` PASS | 完成 |
| G53 | ~~維護×更新整合與版本保留未建~~ **已解**：`release_retention.py`（`star-release-retention/v1`；ACTIVE/PREVIOUS/ROLLBACK/ARCHIVE 階層、三閘門 archive）；§10.15 已標落地 | 完成 |
| G54 | ~~統一 Task Lifecycle 未建~~ **已解**：`task_lifecycle.py`（`star-task-lifecycle/v1`；INTERRUPTED、冪等去重）；§10.16 已標落地，9/9 PASS | 完成 |
| G55 | ~~Runtime Execution Lease 未建~~ **已解**：`execution_lease.py`（373 行；租約＋fencing token＋原子交接）；215 行測試 PASS | 完成 |
| G56 | ~~Release Manifest 未建~~ **已解**：`validate_release_manifest`/`validate_backend_release_manifest`＋payload snapshot（`test_release_manifest_g56.py` 28 PASS）；實體 release 建置屬 G57/G73 | 完成 |
| G57 | 開發／正式環境未分離 | 未完成 |
| G58 | ~~統一 Backend Lifecycle Contract 未建~~ **已解**：`backend_lifecycle.py`（372 行；十欄位統一契約）；11 測試 PASS | 完成 |
| G59 | INTEGRATION 收斂：02–09 已落地（詳 §10.20 盤點表）；僅 INT-10（24h 持續運行驗收）未執行 | 未完成 |
| G60 | 第六批效能缺口 Z1–Z20（**Z1/Z2/Z8/Z12/Z15 已修**——watcher 目錄快取、WS 索引指標＋上限、jitter、事件驅動日誌、錯誤記錄；Z3–Z7/Z9–Z11/Z13/Z14/Z16 未做） | 未完成 |
| G61 | ~~Request Registry 未擴充~~ **已解**：`request_registry.py`（363 行）＋`016547da` 終態強制；11 測試 PASS | 完成 |
| G62 | Request Lifecycle／Streaming 驗收未做 | 未完成 |
| G63 | ~~連線狀態／重連／背壓未實作~~ **已解**：state_outbox A195（reconnect cursor＋bounded buffer）＋connection_watchdog；3 測試檔 PASS | 完成 |
| G64 | 第七批效能缺口 AA1–AA15（**AA13 已修**——profiler 改取峰值；AA2 啟動並發仍無上限等 14 項未做） | 未完成 |
...
| G73 | Release 非自足（Source Tree 載入） | 未完成 |
| G74 | 無 Backend Release Manifest 與 UI×Backend 契約驗證 | 未完成 |
| G75 | SHARED_RUNTIME 隔離缺口 | 未完成 |
| G76 | venv 漂移與可搬移性未驗證 | 未完成 |
| G77 | 依賴鎖未接入 Release 流程 | 未完成 |
| G78 | ~~payload 快照未記入 manifest~~ **已解**：`build_release_payload_snapshot`→`manifest.payload_snapshot`＋`validate_payload_snapshot`；28 測試 PASS | 完成 |
| G79 | ~~local-model runtime 依賴未分類~~ **已解**：`release-dependencies.json` 已分類（xingcheng/runtime=persistent_data、tests/scripts=development_only、settings sha256 釘入） | 完成 |
| G80 | .exe／.dll 原生清單未實測 | 未完成 |
| G81 | 服務重複實例僅驗證層 | 未完成 |
| G82 | ~~星澄自動迴圈 arity 缺陷~~ **已解**：`auto.py:216` 改無參 `_notify_anomalies()` | 完成 |
| G83 | resident 重啟迴圈與 registry 陳舊 | 未完成 |
| G84 | ~~Preload 通道漂移~~ **已解**：漂移通道移除＋契約測試 `test_preload_channel_contract.py` | 完成 |
...
| G91 | self-health 測試檔被移除 | 未完成 |

**施工批次（§11.2）**：W0 完成；**W1（主系統收斂）完成**（1–5＋6a–6d 全落地，稽核 PASS）；W2（引擎與加速）未完成（KV 矩陣＋§10.11 基線已做；ACC-1／P3／MoE 稀疏未做）；
W3（星澄模型）未完成；W4 治理與清理：0／0b／0c／0d 未完成、法典上線完成（封印／簽章未完成）；
W5（§10.45 推理線路與稽核）：W5-1／W5-2／W5-3 完成、W5-4 未完成、W5-5 法典待審。

**Phase／P（第 4 章）**：完成 7（2R／2T／5D／5F／5G／P1／P2）；未完成 16（2B／5／5E／6／7／7B／8／9／9A／9B／P3–P8）。
