...
| G84 | 【未完成】Preload 通道漂移（部分完成） | drift 3→2：`app:get-repair-status` 已有 handler；仍缺 `app:open-path`、`app:set-ui-zoom`（`ipcMain.handle` 未註冊） | 前端契約漂移（詳 §10.42） |
...
**統計**：完成 **38**、未完成 **53**（未完成：G2, G3, G5, G7, G8, G9, G12, G13, G15, G16, G17, G18, G19, G23, G24, G26, G27, G28, G29, G31, G32, G34, G36, G38, G40, G41, G45, G48, G56, G57, G59, G60, G62, G64, G65, G66, G67, G68, G69, G70, G71, G72, G73, G74, G75, G76, G77, G80, G81, G83, G84, G85, G89）。
...
| G11 | RAG 正式路徑驗收 | 完成 |
| G12 | SLA 自動驗收 | 未完成 |
| G13 | C# auto-release 已驗證 | 未完成 |
| G14 | 已解決（2026-09-21）保留策略首次實證 | 完成 |
| G15 | 星澄助理／自動修復端到端 | 未完成 |
| G16 | 綁定與建置已分層 | 未完成 |
| G17 | 綁定周邊已一致 | 未完成 |
| G18 | CPU SIMD 純 C 核心已啟用 | 未完成 |
| G19 | GPU 自研核心已啟用 | 未完成 |
| G20 | GPU 加速定位 | 完成 |
