...
| G5 | GPU 資源競爭（已解決） | 實證齊備：medium-moe 佔用 VRAM 期間推論 `gpu-budget-downgrade` 帳本 4 筆真實 `TimeoutError`（free 171–252MB < required 256MB），訓練釋放後 `device=cuda` 執行恢復（W5-4）；訓練側 `TrainingJobExecutor` VRAM 閘門 fail-closed `EXECUTOR_GPU_BUSY` 已有測試覆蓋（12 tests PASS）；`*_TRAINING` 進入時自動卸載推論資源（R10）；訓練走子程序隔離（R1） | 訓練／推論互相排擠（已治理） |
...
**整體**：G 缺口關閉率 **61/91（≈67%，2026-09-21 以主帳本關鍵字法重算；§10.49 已不存在，引用移除）**；施工批次 **W0／W1 完成、W2–W4 未完成**；
