...
- 現況（2026-09-22 實測）：`gpu_coordinator` 已接訓練／推論（逐請求）／C++ 載入三路＋`auto_release` 已接引擎生命週期（證據 `10_64-engine-release*.json`、`p1-p3f-gpu-coordination-gate-20260922.json`）；adaptive plane 訊號生產者已接第二者：`perf-baseline` tick 以 `observe_merge`（欄位級合併，不覆寫他方量測）注入系統 cpu_pct／ram_pct（證據 `convergence/p4-adaptive-plane-second-producer-20260922.json`）；殘項：其餘生產者（transport backlog／model_load 等）仍 opt-in 待接。
...
| P1-1 | **C++ 長時驗證／device-resident KV／bf16 GEMM／fp8**`[處理中 @devin-cli 2026-09-22T17:55Z]`（eval tps C++ 量測路徑已落地，證據 `e11-eval-tps-cpp-path-20260922.json`） | 中：效能上限 | GPU 工具鏈（nvcc 12.0＋MSVC v142 工具集實測可編譯：`-vcvars_ver=14.29`；v145 STL 硬性不相容已驗） | ①長時驗證②device-resident KV③bf16 GEMM／fp8④生產啟用評估（GPU 窗口） |
...
| P1 | **C++ 推論執行層**（P3／G29／G31／G32／G34） | 殘項＝長時驗證（窗口型）＋device-resident KV（待 GPU attention kernel）＋bf16 GEMM／自研 kernel（nvcc/MSVC-18 不相容待新工具鏈）＋生產啟用（`XINGCHENG_CPP_RUNTIME` 預設 `off`；batch>1 無生產呼叫者）＋生產 `.pyd` 重建接線（W2-1 殘）；已閉＝C 核心 grouped GEMM＋線上分塊注意力（W2-1）、INT8／INT4 量化、CUDA cuBLAS 橋接、device-resident 權重、KV INT8 cache、W1（QK SIMD／緩衝重用）、R5（top-k 稀疏）、R9（C++ `generate_batch`）、P3f GPU 協調閘，證據見 `audit/convergence/` 與 Git 歷史 |
