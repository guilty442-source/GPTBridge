...
- 評估（依職能、生態、效能與整體淨占用；**2026-09-22 再評估**）：
  ...
  | 核心 | 語言 | 再評估理由 |
  | --- | --- | --- |
  | 決策核心 | **Python** | 法典／政策／裁決與 fail-closed 判定須與 codex 工具鏈同源；低頻、可常駐最小化；不變 |
...
- **隔離（2026-09-22 總督核定）**：採**按需隔離**——可隔離**一個單元**或**單一模組**（粒度可選）；
  隔離為故障邊界（失敗不擴散、不影響其他單元）；隔離與解除由運行核心執行、必要時決策核心裁決，全程審計。
...
- **自動化集中（2026-09-22 總督核定）**：**「自動化核心」只負責流程與排程管理**（automation-sovereign 轉型之核心引擎）：
  ...
  模組只執行、不得各自內建排程。
- **核心分工（2026-09-22 總督核定）**：**所有決策與治理由「決策核心」處理**（法典政策、裁決、fail-closed 判定）；
...
- **發權／停權流程（2026-09-22 總督核定）**：**發權與停權一律由「決策核心」裁決**；**執行（含發權／停權）亦屬「運行核心」**（任何執行無例外）；
