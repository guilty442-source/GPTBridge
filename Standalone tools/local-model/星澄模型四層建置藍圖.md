...
**設計（缺口）**〔施工狀態 2026-09-22（二更）：項 1 間隔閘＋品質漂移閘／項 2 合成比例上限／項 3 SFT 課程
選擇／項 4 日預算＋推論互斥／項 8 熔斷／項 9 maturity 重測＋資源帳已實作於
...
`curriculum_intent_map`／`post_upgrade_maturity_recheck`／`maturity_recheck_device`；推論互斥
偵測同行程引擎快取，跨行程由 executor GPU 閘門兜底；閘門測試 10/10＋既有迴歸 28/28；
證據 `self-learning-curriculum-gates-20260922.json`）；殘項標 *〕

1. **觸發政策（自主，但政策不可自改）**：資料量門檻 ✅、品質漂移 ✅（`min_pool_avg_quality`：池平均品質低於政策即阻斷）、能力退化探針失敗 *、排程窗口 ✅、
...
8. **防爆走**：每循環步數上限 ✅（`max_steps` 等）；資料上限 *；GPU 上限 ✅（閘門＋互斥）；異常自動停：連續失敗 ✅（`max_consecutive_failures` 熔斷）、eval 退步 *、資源超支 *；不得自我擴大權限或自改政策 ✅。
...
| P1 | **星澄自主訓練**（§2.7） | 基礎管線已實作（star-self-learning/v1）＋閘門補齊（最低間隔／每日預算／連續失敗熔斷／推論互斥／品質漂移 `min_pool_avg_quality`／合成比例 `max_synthetic_ratio`＋前綴白名單；政策檔預設關閉、受管設定檔開啟）＋SFT 課程選擇（依 `model-maturity.json` 未達層映射課程（L4→dialogue／L5→reasoning／L6→evolution／L≥7→refresh）、L<4 fail-closed、`curriculum_intent_map` 篩池空即阻斷）＋升級後 maturity 重測與 `resource_account` 資源帳；證據 `self-learning-curriculum-gates-20260922.json`（閘門測試 10/10＋迴歸 28/28）；殘項：①能力退化探針觸發③DPO／蒸餾等課程型別⑧資料上限／資源超支停機⑦rollback 實測（保留上一代語意與現行 prune-latest 衝突，待裁決）⑩連續 3 循環自主驗收⑥法典 amendment（自主訓練權限邊界） |
...
| G41 | 逐層一致性殘項 | MoE／INT8 量化逐層 RMS 探針已補（`test_cpp_moe_layerwise_parity`／`test_cpp_quantized_layerwise_parity`，證據 `g41-moe-quant-layerwise-probes-20260922.json`）；INT4／CUDA 路徑探針待後續增量 |
...
- ~~`automation-sovereign` vs `synchronization-sovereign`~~ → **已施工**（2026-09-22）：
  ①registry `active` 改 `automation`②8 個 component `owner_sovereign` 同步③compatibility shim
  更正為 `synchronization-sovereign`→`automation-sovereign`④`SOVEREIGN_ALIASES` 單向舊名解析
  （`codex_amendment.py`＋`codex_amendment_audit_gate.py`）；registry validate 0 errors。
- ~~A592 退役子主權 vs `module_assignment_registry` 12 列仍指派 retired 子主權~~ → **已收斂**
  （r7 amendment executed：12 列標 `legacy-active under A334`，保留 A334 固定指派世系；
  manifest／permission routes 層級以 `architecture_registry.json` 對齊為準）。
- ~~`language-review`：法典已廢止（A336／A592），registry 仍 canonical／resident~~ → **已收斂**
  （`architecture_registry.json`：`lifecycle=retired`、`canonical=false`、`module_labels.trash` 登錄；
  registry validate 0 errors。程式碼刪除依 A592 codex-first 另辦）。
- ~~修復能力雙登錄~~ → **已收斂至 `central-automatic-repair`**（2026-09-22）：
  `data-architecture-contract.json` consolidated_capability、`central_repair.py` capability_id、
  `architecture-project.md`／`architecture-tool-local-model.md`／本藍圖敘述全部對齊；
  權限層 `central-automatic-repair`→`tool:system-rescue` 綁定原本即正確（system-rescue 為執行者）。
...
- ~~`star-chat` 路由 actor 未登錄於 `identity_groups`；`system-health-check` owner 仍為 retired `global-cleaner`~~ → **已收斂**（2026-09-22）：
  ①star-chat 註冊 `IDENTITY_GROUP_LOCAL_MODEL_DIALOGUE`（D00002/STARCHAT，companion 最小能力集）
  ＋`identity_permissions` 補 channel-submit/process＋ai-channel-submit 綁定；
  ②`system-health-check` owner 改 `tool:system-rescue`（active 且 manifest 已要求該能力）；
  治理稽核 0 errors。殘項：③AGENTS.md 重述收斂（見 B）。
- ~~審計章程 4/5 `owner_sub_sovereign` 已退役（A592）；`implementation_obligations` 實作／驗證同體~~ → **已收斂**（r12 amendment executed：
  章程 owner 改指自身主權核心引擎＋稽核測試同步 17/17；4 列 obligation owner 退役身分改 `decision-sovereign`／`xingcheng`，
  驗證主體與實作主體分離——SEVEN→decision、DAG→automation；五方稽核 ALL_FIVE_SOVEREIGNS_AUDITED＋全量稽核 0 errors）。
