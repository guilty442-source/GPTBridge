...
| 級別 | 項目 | 狀態 |
| --- | --- | --- |
| P0 | **法典上線殘項**：封印根／修訂歷史與外部簽章（G66）；G67 →active 需 mapping VERIFIED 列 semantic-hash 規格（governor 側） | 未完成（governor 側） |
| P0 | **主系統不再需要常駐 Python**（§1.1 新增目標二） | 未實作（待規劃） |
| P0 | 主系統 Python 常駐資源縮減（§10.63） | R0–R5 已施工並量測達標（ready 1.72 s／RSS p95 82.6 MB／idle CPU mean 2.97%／固定週期 5）；常駐清單已落地（resident-core.json）；後續空間屬 §1.1 目標二（Python 常駐外移） |
| P0 | 資源自動管制與自動釋放（§10.64）：工作者總帳 CPU≤10%／RAM≤30%、四項資源自動釋放 | 大部分實作（2026-09-22）：worker-plane 歸屬＋總帳＋滯回控制律＋⑤admission-hold（`a4775ce5`）＋④pausable 週期工作接 `regulation_active`（idle-memory/daily-cleaner/git-automation）＋⑥ broker 對自行啟動的 owner 在管制中且佇列空時走 governed `stop_tool` 釋放（`f2eb9c13`/`dc17ca76`）；引擎/GPU 釋放走既有 auto_release；殘項：30 min p95 驗收量測、法典側資源優先序明文 |
| P0 | 全系統收斂殘項：INTEGRATION-10 24h 驗收（G59）；04B 殘項（RC 重打包／G76/G77） | 未完成 |
| P1 | C++ 推論執行層（P3／G29／W1／R5／R9） | 部分落地 |
| P1 | 星澄對話與推理：L5 完整驗收、L6（Phase 6）、L7（Phase 7）（G40） | 未完成 |
| P1 | 星澄自主訓練（§2.7）：觸發政策／課程選擇／資源預算／回滾與驗收 | 設計已定；未實作 |
| P1 | 外部協作 EC-1–EC-8（G36／P8） | 未完成 |
| P1 | DSN 秘密遷移收尾（G89） | 部分（見 §3.2） |
| P2 | Release pipeline 依賴鎖收尾（G77）；Embedding 相容性殘項（GenerationManager 接線，G50）| 未完成 |
| P2 | 效能與資源殘項（W1／R5／R9／MS6／MS10） | 未完成 |

### 3.2 現存缺口（G 清單；僅未完成／待續）

| 編號 | 缺口 | 剩餘工作 |
| --- | --- | --- |
| G24 | SQL 治理十階梯 | scheduler 無生產組裝點、TCP-probe 路徑無法認證、十階梯 MODULE_PRIVATE／RECOVERY／READ_MODEL 無證據源止步 |
...
| G59 | INTEGRATION 收斂未執行 | **僅 INT-10 24h 驗收未跑**（詳 10.20／§10.13）；已修 boot_core pythonw 啟動時 relay 死結導致 dead-grace 重啟迴圈（`e7f724a0`/`3a9de19b`）；當前 session 自 2026-09-21T23:18Z 起累積 |
| G65 | 證據 checkpoint 保留缺口 | 已補回：self-learning job `a7a2a5e7` 產出完整證據鏈（final.pt 982MB＋sha256＋job-spec＋train-docs＋sft_summary），`native-engine.json` pin → retention 保護生效；舊 `chat-foundation-v1/final.pt` 不可恢復但其引用已由 active_weights_version=18 取代 |
...
| G77 | 依賴鎖（部分） | wheel cache／離線重建環境建置器正式接入 release pipeline |
...
- INTEGRATION-04B：**15/15 PASS**（04B-10 隔離啟動 harness 正式判定通過，§10.52）；殘項為 RC 重打包（main.py 舊版）與 G76/G77。
...
- Embedding 維度／index 版本相容性：canonical 側已有 A486/A487 世代生命週期（`index_fingerprint` 含 embedding 維度／模型／index schema，漂移→新世代→驗證→原子 alias 切換，capacity-gated）；degraded 本地快取補齊寫入側維度守衛＋`reconcile_dimension` 明確重建（epoch／index_version 記帳）。殘項：生產組裝點未建構 `GenerationManager`／`requires_rebuild` 無呼叫者（屬 G50 遷移接線）。
...
- 隔離啟動 harness（04B-10）已建並實測 PASS；venv／lock 收尾（G76／G77 殘項）→ 見 §10.52。
- 依賴鎖離線重建／wheel cache 接入 release pipeline（G77）。
- 舊 UI × 新 Backend 啟動演練屬 G73 殘項；A/B handover 已實作（見 §10.20），待端到端演練。
- Embedding 維度／index 版本相容性已實作（見 §3.2 無編號項）；殘項為 `GenerationManager` 生產組裝點接線（G50）。

### 3.4 端到端未驗收

- **INTEGRATION-10**：24 小時穩定運行驗收未執行（§10.13）。
- **A/B 切換**：已實作（A330 handover 鏈）；未做端到端實切演練（§10.20）。
- **04B-10**：PASS（`--with-isolated-start` 正式驗收 15/15 綠，§10.52）。
...
- 【P0 待裁決】啟動時限：法典 10 秒（A199／A265／A537）vs 實作 30 秒（`main-system/config/startup_manifest.json`、`startup_config.py`）。
- 【P0 待裁決】`local-model` 隔離政策 `restart_on_crash=true`（`tool-isolation-policy.json`）vs A540「不得背景自動重啟」。
- 【P1 過時引用】`tool_lifecycle_budget.py`（引 A294/A446/A266 → A540）；`crash_diagnosis.py`（幽靈 A73／600 行 → A432／E160）；`source_size.py`（A430 標題）；`codex_amendment_audit_gate.py`（proposed A537 → A537）。
- 【P2 重述】AGENTS.md（self-commit／lazy RAG/CAG／GPU 段）與 `architecture-*.md`、`architecture-tool-local-model.md` 逐字重複 → 改引用條號＋單一登錄來源。

**C. 權責衝突（真衝突；待裁決或下次修正）**

- `automation-sovereign` vs `synchronization-sovereign`：A486 已改名；registry `active` 仍用 `synchronization`、8 個 component `owner_sovereign` 未換、compatibility shim 錯置（`automation`→`automation-sovereign`）。
- A592 退役子主權 vs `module_assignment_registry` 12 列仍指派 retired 子主權。
- `language-review`：法典已廢止（A336／A592），registry 仍 canonical／resident。
- 修復能力兩套登錄：`central-automatic-repair`（capability_boundaries）vs `xingcheng-system-repair`（data-architecture-contract／central_repair）。
- A548（星澄自主修復）vs A366/A380/A408（使用者開關＋一次性確認）字面牴觸 → 需 CLARIFICATION 界線。
- `star-chat` 路由 actor 未登錄於 `identity_groups`；`system-health-check` owner 仍為 retired `global-cleaner`。
- 審計章程 4/5 `owner_sub_sovereign` 已退役（A592）；`implementation_obligations` 2 筆實作／驗證同體（`xingcheng-domain-sovereign`）。
- `XINGCHENG_INSTITUTION_ROOT` 缺 `architecture_activation_states` 列（唯一缺口）。

**D. 索引工具（已完成；派生、非權威，僅供使用）**

- `scripts/build-runtime-rule-index.py` → `main-system/runtime/state/runtime-rule-index.json`：法典版本、537 條內容分類、67 規則索引、時限／權屬索引；可重建、可刪除，分類不改任何條文效力。
...
**判定：15/15 PASS**（04B-10 正式 PASS；completion 仍 NOT_COMPLETE——殘項：rc main.py 重打包、04B-09 共享服務替身、venv 自足 G76/G77、IPC 契約客戶端）。
...
1. ~~建立隔離啟動 harness~~ → 已建 `scripts/integration-04b-isolated-start.py` 並接入驗收
   ...
   mid-start-kill 兩場景實測 PASS（零孤兒、port 釋放）。**發現兩個真實缺口**：
   ① rc `backend/main.py` 為舊版缺 flat-layout governance sys.path 補丁（下個 RC 須重打包）；
...
> 實作狀態（2026-09-22）：觀測歸屬、總帳、控制律 ①③⑤⑥ 已接線
> （`resource-governor.py` 歸屬＋滯回＋hold；`resource_governor_signal.py`
> 共用讀取；`PeriodicScheduler` `pausable` 實現 ④——idle-memory／
> daily-cleaner／git-automation 管制中延後；broker ⑥ 對自行啟動的 owner
> 在管制中且佇列空時走 governed `stop_tool` 釋放；引擎／GPU 走既有
> auto_release）。② CPU affinity 上限與 30 min 驗收量測仍缺。
...
**法典側需求（待確認／amendment）**：資源優先序（治理＞核心執行＞使用者工作＞背景維護）與
「不得為降載停用治理」之明文；資源預算值之變更權屬。
