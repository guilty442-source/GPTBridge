...
| E6 | **Python 常駐外移**：E4（治理核心受 C 閘控）＋雙軌 retire＋整體驗收 `[處理中 @devin 2026-09-22T14:45Z — E1 之後歸檔 E6 act-1 shadow：connection_watchdog/periodic_scheduler/state_outbox/maintenance_controller 四件全部接線完成，native-shadow.json 全 shadow；pyd 已重建含 NativeMaintenance system_blocked/cancel/cache；測試綠]` | 中高：資源／穩定性（常駐 Python） | E1–E3 已部分落地 |
...
| E12 | **語言外移 M2–M5**（M1 shadow 進行中）`[處理中 @devin 2026-09-22T13:00Z — A263 parity 放行套件]` | 中低：工程債；不阻擋上線 | M1 完成 |
| E13 | **自主訓練殘項**（續訓/專家課程型別、3 循環驗收、法典 amendment） `[處理中 @devin 2026-09-22T12:45Z]` | 中低：能力進化 | 法典 amendment 需協調窗口；3 循環需 GPU 窗口；eval tps 閘已裁定（2026-09-22 總督：兩者並行——`min_tps_baseline_ratio` 相對閘已落地、證據 `convergence/e13-eval-tps-relative-gate-20260922.json`；C++ engine 量測路徑歸 E11） |
...
④未認領項依下「分配順序」承接；受 E1 閘控項於窗口解除後方可認領。

**分配順序（2026-09-22 盤點後；執行用；風險表維持風險序）**

代號規則：`配-N`＝未認領可承接；`配-GN`＝governor 側（不由 worker 承接）；已認領項不列（E1／E6／E12／E13）。

第一波（可立即承接，不待 INT-10）：

| 代號 | 項目 | 細項 |
| --- | --- | --- |
| 配-1 | **`retention` watcher 修復**（§2.5／§10.67 殘項） | ①CLI watcher 路徑接 `apply_retention`（dry-run 先行）②首次權重退役（21 版→active＋上一代；`retired` 子表）③`retention.jsonl` 生成與引用保護實測 |
| 配-2 | **推送閘**（§10.69） | ①推送前「強制測試 PASS」閘實作②週期性 ahead==0 持續證明③推送審計入帳 |
| 配-3 | **執行緒策略入口**（§10.30） | ①統一入口（`OMP／BLAS／PyTorch 執行緒 × 平行工作數 ≤ 預算`）②bounded-workers predicate 進 A590 評測器 |
| 配-4 | **Triton 預設對齊**（ACC-2） | ①決定預設（on／off）②受管設定 note 與程式（`XINGCHENG_TRITON_KERNELS`）一致化③治理登錄 |
| 配-5 | **RAG 階段耗時注入**（§10.11） | ①RAG 階段計時介面②latest 快照 `rag` 非 null |
| 配-6 | **P6 驗收證據** | ①7 工具 5 s 啟停量測②測試 20 s 專項③隔離 overrides 補齊（其餘 5 工具）④星澄助理／自動修復端到端 |
| 配-7 | **E11（C++ 效能上限）** | ①長時驗證②device-resident KV③bf16 GEMM／fp8④生產啟用評估（GPU 窗口） |
| 配-8 | **E14 收尾組** | ①MS6（C# 承接）②MS10（SLO 儀表）③EC-5 法典同步（待協調窗口） |

第二波（E1／INT-10 解除後）：

| 代號 | 項目 | 細項 |
| --- | --- | --- |
| 配-9 | **E9 休眠啟用** | ①`enabled=true` 核定②啟用後審計與回退演練 |
| 配-10 | **E10 啟動 10 s 殘項** | ①對話路由取樣②多次冷啟分佈③逾時 fail-closed 實測 |
| 配-11 | **E7 單元制搬移** | ①專用層級表②tools→model→shared→core→xingcheng 受管搬移③跨單元介面強制④單元／模組級隔離實測 |
| 配-12 | **E8 啟用裁決** | ①啟用或 base 重訓裁決（總督）②echo／算術缺口處置 |

| 代號 | 項目 | 細項 |
| --- | --- | --- |
| 配-G1 | **E3 封印與外部簽章**（governor 側） | ①`epoch_seal_manifest` 更新②外部簽章③certification |
| 配-G2 | **E4 規則 →active**（governor 側） | ①45 條 `formal_rule_mapping` VERIFIED②semantic-hash canonical 規格③amendment④mirror／derived 重建 |
