...
| G86 | 【未完成】jieba 導入造成原生推論 GPU 降級回歸 | `shared_layer/tokenizer.py`（`auto-commit(jieba-integration)` 2026-09-20T20:21+08）被 `shared_layer/__init__.py` eager import 且頂層 `import jieba`；runtime venv 未安裝 jieba → `import shared_layer` 失敗 → `native_engine._gate_cuda_device` 的 GpuCoordinator import 失敗 → 推論降級 CPU（帳本 `gpu-budget-downgrade` reason `No module named 'jieba'`；最後一次 CUDA 執行 `2026-09-20T10:54:32Z`） | 原生推論 GPU 路徑失效（詳 §10.44） |
