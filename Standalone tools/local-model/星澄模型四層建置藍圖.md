...
- **CURRENT IMPLEMENTATION（現行）**：A334 固定分配；`architecture_registry` 43 元件已無 `owner_sub_sovereign`
  欄（實測 2026-09-22）；法典 `sovereign_hierarchy_registry` **6 列 `active`＋13 列 `retired`**、
  `module_assignment_registry` **5 列 `active`＋12 列 `legacy-active under A334`**（實測 2026-09-22）
  待 LEGACY_RETIRED 階段退役；runtime 路徑＝主宰→子主宰→固定 module assignment→executor。
...
| PARITY_VERIFIED | 每個映射模組的 dispatch parity 證據（舊解析 == 能力匹配） | **部分**：`capability_dispatch_parity.json` v2（語意碼）——
  ...
  碼制屬衍生證據，待 CUTOVER 閘核定；runtime 派工接線與 registry 切換未完成（註：`sovereign_transition_registry.json` 檔落後 DB——`current_stage` 仍 `PREPARED`、規則列仍記 `proposed`，待重生成） |
