...
> 殘項：協調者進程尚未載入新閘（既有推送未見閘證據）；**重載後生效**，首筆「閘後推送」證據待生成（整合阻斷細項見 §12 P0-9）。
...
| P0-9 | **Git 整合阻斷（推送停擺）**（§10.69）——worker 診斷已備 `convergence/p0-9-git-blockage-diagnosis-20260923.json`：7 筆 conflicted 全屬 stale（來源皆已入 origin/main）；`main` 領先 124／落後 1（僅 `d4bb42d7` auto-commit，內容已被本地涵蓋、無資料遺失）；**解封 queue 後 push 仍會 `remote-main-diverged`**——需先將 origin/main 併回本地 main（`merge-tree` 試做：3 檔真衝突 anomalies/connection/transport_notify，建議解向皆為 main 側已驗，執行屬人工）| 高：推送自 01:44 起暫停、sync 自 01:54 起 `conflict:git:retry-blocked`；`main` 領先 `origin`——完成定義（提交＋推送＋稽核 PASS＋測試 PASS）不可達、治理閉環缺證據 | 人工裁定（§10.69-D② 衝突即停待人工；不得自動解衝突） | ①`mq-000018`（`git` @ `d4bb42d7`）處置：source 已在 `origin/main` → 判 stale，reconcile／標記 resolved（人工）②復推 `main`→`origin` 並驗 `ahead==0`③清理其餘 `conflicted` 條目：`mq-000001`／`000002`（chinese-semantic-engine-core）、`mq-000008`–`000010`（rustic-calcium）、`mq-000013`／`000016`（local-model；多屬已退役分支）④協調者重載以載入新推閘（`push_gate.py`）；首筆「閘後推送」證據入帳（ledger＋`.git/gptbridge-push-gate.json`） |
...
### 3.5.1 法典收斂與權責對齊（未完成；依 2026-09-21 分析）

**A. 重複條文收斂殘項（法典內部；需法典修訂；索引不改效力）**

- 模板群：A303–A310、A316/A317、A299/A300（A300 已由 A494 取代）——待引用化。
- 跨層重述（dupscan `review-required`）：E7~A28、E16~A478、E28~A42、P20~A45、E35~A49、
  E44~A58、E126~A151、E141~A166、P9~E1、P19~E24、P24~E36、P49~E75、P85~E120 → 改引用＋領域差異。
- 形式規則模板：42 條 `declared provision …`＋6 條相同 evidence predicate（A334 掛 8 條）→ 產生端去重。
- 收斂流程：staged request → build_successor → 五方稽核 → 總督 apply → mirror／投影重建 → 稽核 PASS；
  `merge_candidate=review-required` 不自動合併（掃描器 `scripts/codex-semantic-dup-scan.py`）。

**C. 權責衝突（真衝突；待裁決或下次修正）**

- A548 vs A366/A380/A408 域澄清：clarification amendment 已 staged
  （`codex-amendment-request-a548-domain-clarification-2026-09-22.json`，change_class=clarification，
  四條文附加 DOMAIN-SCOPE 澄清、不改既有特別法效力）；殘項僅餘 codex 實際寫入——待協調窗口經
  五主宰一致稽核門執行。
