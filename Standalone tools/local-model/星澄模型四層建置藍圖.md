...
3. **課程選擇（訓練什麼）**：依 maturity 未達項決定課程——SFT 課程族已實作 ✅（`curriculum_enabled`：讀 `model-maturity.json` 認證層，L<3 基礎未達 fail-closed、L3→`sft-foundation`、L4→`sft-dialogue`、L5→`sft-tools`、L6→`sft-reasoning`、L≥7→`sft-refresh`；`curriculum_intent_map` 可依課程篩 intent，空池 fail-closed）；DPO／蒸餾／續訓／專家模組課程型別 *；每循環只推一個課程 ✅。
