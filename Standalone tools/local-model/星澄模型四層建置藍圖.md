...
| P1 | **推理線路與稽核完整性**（§10.45；G86／G87／G88）：W5-1 jieba lazy import ✅（並行工作者完成）→ W5-2 補 self-health 測試 → W5-3 ai-collaboration ✅（隨 W5-1 解除）→ W5-4 GPU 復驗（待顯存）；法典 A593／A594 已立案（request-only） | worker（chinese-semantic-engine）＋治理側 | W5-1／W5-2（G87/G91 測試檔已補）／W5-3 完成；W5-4 ✅（device=cuda 帳本已入）；驗收：稽核 self-health 全綠、帳本新增 `"device":"cuda"`（已達成）；W5-5 ✅ A593/A594 撤回結案 |
...
Git 自動化已由單一 in-process `GitAutomationService` 取代舊行程艦隊（2026-09-21 舊 `automation_supervisor`＋16 支 `self_commit --watch` 冗餘進程已停——in-process 服務涵蓋同一 worktree 集合，28 sweeps／6 syncs 運行中）；
...
- 現況：dispatch parity 已實測 6.6e-7（matmul／softmax／attention）；**全層比較 harness 已建立**（G41：C++ `layer_metrics`＋PyTorch hooks）。
...
| G2 | 【已解】eval suite 與訓練分佈落差大 | **評估結案（2026-09-21）**：實測 corpus-v2 train 1585 docs／13.284M chars／zh ratio 0.72%，eval suite 3778 chars／zh ratio 39.23%；落差是設計而非缺陷——eval suite 量測目標能力（對話探針）非訓練分佈匹配度；上線判準已由成熟度階梯結構性取代（L0–L7 連續閘門、僅實測定級、SFT 晉升需 eval 對照現役權重基線）——eval ppl 從不是單一判準，而是多閘門之一 | 已解決 |
| G3 | 工具整合（階段 9）（已解決） | `tool_call_format`＋`GovernedToolExecutor` 白名單路由＋端到端驗收 `test_tool_bridge_e2e.py` 3 passed（真實 `service.handle` 鏈：3 工具成功＋失敗回復＋converse 回填）；順帶修正 `sql_list_knowledge` 真實鏈 arity 缺陷 | 工具能力上線前題 |
| G4 | 【已解】prefix cache 未正式驗收 | **已驗收（2026-09-21）**：`test_prefix_cache_chat_session.py` 9/9 PASS——`PrefixKVStore` 容量驅逐、首次無命中→整段命中（`last_prefix_reuse == prefix_len-1`）、batch>1 自動停用、chat session 跨輪重用；實作於 `generate.py`（僅 batch=1＋use_cache 啟用） | 已驗收 |
| G5 | GPU 資源競爭（已解決） | 實證齊備：medium-moe 佔用 VRAM 期間推論 `gpu-budget-downgrade` 帳本 4 筆真實 `TimeoutError`（free 171–252MB < required 256MB），訓練釋放後 `device=cuda` 執行恢復（W5-4）；訓練側 `TrainingJobExecutor` VRAM 閘門 fail-closed `EXECUTOR_GPU_BUSY` 已有測試覆蓋（12 tests PASS）；`*_TRAINING` 進入時自動卸載推論資源（R10）；訓練走子程序隔離（R1）；native generation 已接 per-request transient VRAM budget gate（不重複計算常駐權重，帳本記錄 `gpu_budget_required_mb`／`gpu_budget_gate`） | 訓練／推論互相排擠（已治理） |
...
| G8 | 【已解決】自我學習端到端升級實證（2026-09-21） | 兩次強制循環完成全鏈：19 例匯出→受管 SFT 400 步→adapter 註冊→eval gate→裁定。第一輪 fail-closed `rejected`（suite-missing，檔名≠suite_id 缺陷）；修後第二輪（job `a7a2a5e7`）eval `star-native-eval-dialogue-v1` **PASS**（ppl delta +24.58% ≤ 40% gate、generation_ok、tok/s ok）→adapter `65bc1bf7` stage+activate→`native-engine.json` pin 新 final.pt→`active_weights_version=18`→retention 自動收尾。連修三缺陷：worker `No module named 'governance_rule'`（sys.path）、suite 檔名慣例、venv 缺 `mpmath`（uv.lock 漂移已補）；policy 新增 `gpu_required_mb` 透傳（1200MB 實測值）。報告：`self-learning-20260921-130352.json` | 全鏈含升級已實證 |
| G9 | 【已解】未落地變更未審計／提交 | **機制已落地**：`GitAutomationService` 每 60s 掃描＋60s 穩定 debounce＋staged-index 保護，self-commit 記 `auto-commit` 審計操作；後端日誌可見連續自動提交（`[self-commit] … on main`）；原 13 檔快照問題已由持續提交機制結構性解決 | 已解決 |
...
| G12 | SLA 自動驗收 | **已落地（2026-09-21）**：`test_sla_contracts.py` 鎖死四個法定預算與執行點——audit 30s（`AUDIT_FLOW_BUDGET_SECONDS`＋`audit_flow_budget_error` 注入點）、startup 10s（`STARTUP_HARD_DEADLINE_MS`＋phase budgets 加總不逾限）、工具啟動 5s（`TOOL_OPEN_BUDGET_SECONDS`＋deadline 證據）、pytest timeout 上限宣告；5/5 PASS | fail-closed 驗證 |
...
| G15 | 【已解】星澄助理／自動修復端到端 | **已驗收（2026-09-21，`f909781f`）**：`test_auto_repair_e2e.py` 首個全鏈測試走通 `process_health_signal` 七段＋finalize＋learn＋report（healthy 短路／unauthorized 拒絕於 stage 3／crash signal 全鏈 fail-closed）；**順帶修復生產級 bug**：`GovernanceAudit.record` 遇 `HealthState` enum 即 `TypeError` 崩潰——修復鏈此前從未成功跑完，現 enum→value／dataclass→dict 序列化；60 項相關測試全綠 | 已驗收 |
...
| G24 | SQL 治理 DoD 全項核實（2026-09-21） | `SQL_ARCHITECTURE_RULES.md` DoD 全勾：RAG metadata authority 已接線（`rag/pipeline.py` 實例化 `PostgreSQLMetadataAuthority`）；startup certification 已接 runtime（`_phase_postgresql` 跑 `certify_startup`＋`phases_execution` 以真實證據驅動十階梯 `startup_ladder`）；restore certification 已接 scheduler（`restore_certifier` 注入＋`backup_catalog` 寫入，修掉指向不存在 `gptbridge_audit.backup_catalog` 的 INSERT 缺陷）。殘留誠實標記：scheduler 無生產組裝點、TCP-probe 路徑無法認證、十階梯 MODULE_PRIVATE/RECOVERY/READ_MODEL 無證據源止步 | 7 測試綠；「20 條規格」逐條映射以外部規格文件為準 |
...
| G26 | 【已解】低負載機制未量化 | **已解決（2026-09-21）**：(a) 量化——`ConnectionManager.stats()`（size/max/idle/active）＋`collect_pool_pressure` 接真實池（原為全零 stub），workload lanes 曝池統計；(b) 事件化——查證主路徑已是事件驅動：`RuntimeServiceManager` 走 `ipc_event` push（30s heartbeat 為 push 失效兜底，A67 防 stale-status）、`useBackendSocket` 走 `online`/`visibilitychange` 事件重連（3s 僅 stale-socket 看門狗非資料輪詢）；殘留 interval 皆為刻意安全網非輪詢主路徑 | 已解決 |
| G27 | 審核管線重複掃描／讀取 | **已解決（2026-09-21）**：O1 mtime 存在性快取、O2 混合式發現、O3/O6 mtime-keyed 檔案快取、O5 token 集合交集——稽核輸出逐位相同；O4 判定刻意雙渲染；O7 掃描無實例結案（詳 4.5） | 稽核負載與 30 秒 SLA 風險（詳 4.5） |
| G28 | 【已解】第二批效能缺口（S1–S8） | **全 8 項結案**：S1 SIMD dispatch、S2 tuner 平滑＋遲滯、S4 探針並行、S7 adaptive pool 接線、S8 可攜 benchmark 皆已落地；S6 評估結案（跨執行稽核快取違 fail-closed）；S3 facade 惰性導入（25.4ms→按需鏈）；S5 RC 體積已實測（governance_rule 1214.07MB、venv 18.84MB、backend 4.61MB、shared-layer 4.24MB、shared_runtime 2.25MB） | 已解決 |
...
| G30 | C# 業務編排層部分落地 | `business-logic-csharp` 已具 Domain/Application/Infrastructure；模型服務 HTTP 契約 `star-model-service/v1` 已落地：`infrastructure/model_service_http.py`（loopback-only、`X-GPTBridge-Session-Token` 驗證、64KiB body 上限、inflight≤2、`POST /v1/infer`→`generate_via_native_engine`、`GET /v1/status`、`POST /v1/release`→AutoReleaseManager、descriptor `runtime/ipc/model-service.json`＋`model-service-session-token`）；已接入 `channel_runtime` 啟停（`XINGCHENG_MODEL_SERVICE_HTTP=0` 關閉）；C# `HttpModelClient` 對齊契約＋`ModelServiceLocator` descriptor 發現（schema/port/token fail-closed）；`dotnet publish -c Release` 已驗證產出 `StarBusinessLogic.dll`。Python 9 tests＋C# 29 tests PASS。已補：release-dependencies.json `non_python_components` 納入 C# 層與 `lifecycle_owner=local-model/channel_runtime.py`、consumer-only policy；尚缺 P7-3 正式生命週期主導權移轉演練 | 四語言定位缺口（P7） |
| G31 | 資源／推論優化缺口（R1–R10） | **已落地**：R2（C++ 引擎全經 C ABI）、R4（auto_release 已接線引擎生命週期）、R7（`native_engine` 延遲載入 native_transformer/torch：import 2.30s/1125 modules→0.115s/112 modules，輕量路徑不付 torch 成本）、R8（統一 `cpu_thread_budget`）；R10（`*_TRAINING` 進入時經 `_apply_lifecycle_resource_action` 卸載 auto_release 推論資源＋`lifecycle-resource-actions.jsonl` 帳本，2 tests PASS）；R1（真實訓練器 `training_job_worker` 子程序隔離——spec/summary/error JSON 檔案契約、stderr 落盤、timeout 有界、注入樁維持同進程；常駐推論服務經 `model_service_http` sidecar 成形）；R3（profiler 實測：mem_efficient SDPA 真實派發、flash 未編譯、cutlass tensorop GEMM＋tf32）；R6（C++ KV 改 16-token block 分頁：block table＋free list＋按需配置、`kv_memory_bytes` 實測 1 block 8192B < 最差 16384B、limit 於 block 配置時 fail-closed、prefix 快照改經 table gather/scatter、bitwise parity 13 tests PASS）；**未做**：R5（MoE 稀疏執行/grouped GEMM）、R9（batch>1／continuous batching）（詳 4.7） | RAM／VRAM／CPU 與吞吐 |
| G32 | 主系統十大優化缺口（MS1–MS10） | **9/10 結案（2026-09-21）**：MS1 受治理訓練子程序（R1）、MS2 按需啟動＋auto_release 接線（R4/R10）、MS3 GPU 協調接線＋配額、MS4 熱更新健康閘門（A330 handover 實測）、MS5 事件驅動主路徑已驗（G26：push 為主、interval 僅兜底看門狗）、MS7 啟動探針並行（S4）、MS8 修復鏈＋維護閉環（G49）、MS9 breakers＋tool-isolation-policy 存在、MS10 `SQLSLOMetrics` 六項法定 SLO 目標已具（可視儀表 UI 未建）；**MS6 仍開**：C++/C# 承接待 P3/P7 | MS6 待 P3 |
...
| G34 | 第三批效能缺口（W1–W11） | **10/11 結案**：W2–W9 已落地（gram 記憶化／oneshot／零拷貝／分片 LRU／Condition 池／BLOB 向量）；W10 評估結案（native mm_large 94.6× 基準，瓶頸在 MoE 路由非 matmul）；W11 評估結案（正確遷移標的是 psycopg3 async 非 asyncpg，雙池成本不抵收益）。**W1 仍開**：C++ attention Q×K SIMD＋scores 緩衝屬 P3 C++ 推論層（並行工作者施工中） | W1 待 P3 |
| G35 | 【已解】NTP 正確性測試未成矩陣 | **已解決**：`tests/test_ntp_correctness.py` NT-1–NT-12 全數通過（2026-09-21 實跑 12/12 PASS）；回歸掛載＝納入 pytest 套件即自動執行 | 已解決 |
| G36 | 外部協作未完善（部分驗證落地） | 送出／回覆鏈與 session 管理未端到端驗收；自動記憶僅實作；星澄研究路徑未驗證；A544 幾何法典同步待 staged request 落地（詳 4.2 P8）。**零權威負測試已落地（2026-09-21）**：`test_external_zero_authority.py` 5/5——全部出站 payload 無 direct_database_write/access/model_weight_access 真值旗標、注入回應以惰性字串返回、模組無 sqlite3/psycopg/subprocess/eval/exec 面、未綁定 client fail-closed | 外部協作品質與治理對齊 |
...
| G40 | 【未完成】原生對話訓練未建立（部分完成） | 角色／遮罩格式與 SFT 合併資料 v2–v6 已具；**九類端到端已實跑**（CUDA，`chat-foundation-xl-v3`，`capability-eval-20260921-201715.json`）：harness 成立但 checkpoint 僅過 en-ppl/code-ppl——缺口為模型對話品質（訓練資料／容量），非驗收鏈路 | 對話與命令理解前題 |
| G41 | 逐層一致性閘門已建立（2026-09-21） | C++ engine 新增 `layer_metrics` 除錯面（embedding／每層輸出／final_norm 的 hidden RMS）；`test_cpp_layerwise_parity_with_pytorch` 對 PyTorch 逐段比對，契約 `rtol=2e-2/atol=1e-3`；實測漂移 ≤3e-8（近 bit-exact）；既有 logits parity（atol 2e-3）＋greedy 逐 token 一致維持 | 雙路徑可換性逐層可證（詳 2.3、Phase 5E）；MoE／量化路徑上線後需各補對應探針 |
...
| G48 | 【已解】Contract 相容驗證未建 | **全軸已建（2026-09-21）**：`ContractRegistry`（`star-contract-registry/v1`）版本軸相容判定（consumer ≤ contract_version 且 ≥ minimum_supported）＋新補 `ipc-contract.json`／`sql-schema-contract.json`／`backend-lifecycle-contract.json`——六軸全覆蓋（ai-connection/data-architecture/tool-runtime/ipc/sql-schema/backend-lifecycle）；`check_contract_axes` 已接入稽核 fail-closed（軸缺漏/版本非法即 error）；稽核全綠 4.0s/0 errors | 已解決 |
...
| G51 | 【已解】Process Registry 與效能基準未建 | **兩半皆落地**：(a) `process_registry.py` `star-process-registry/v1`（PID/module/health/restart/shutdown_state＋`is_owned`＋`reconcile` 孤兒偵測），ToolboxService 已接線；(b) §10.11 效能基準 `perf_baseline_snapshot.py` `star-perf-baseline/v1` 已採集首份實測（git status 94ms、GPU、process RSS/CPU），4 測試綠。registry 可信度問題追蹤於 G83（30min soak） | 已解決（可信度見 G83） |
...
| G56 | Release Manifest（已解決） | release dependency contract＋frontend manifest＋RC `rc-2026-09-21/release-manifest.json`（identity／commit／hashes／lock／python runtime／IPC／governance compatibility／data boundary）已具；**`validate_release_bundle` 以 RC cwd／release-owned roots 實測 `ok=True, errors=[]`**；venv hash／lock／native pyd＋DLL 路徑已對齊 RC | 切換前相容判定 |
...
| G60 | 第六批效能缺口（Z1–Z20） | **已解決（2026-09-21）**：Z1/Z2/Z8/Z12/Z15 已修；Z3/Z4/Z5/Z7/Z9/Z10/Z11/Z13 核對已落地；Z6/Z14/Z16 評估結案（crash-consistency 必要設計／每模組 `*_constants.py` 慣例／關鍵路徑測試已覆蓋）——詳 §4.12 | 前端熱路徑與維運負載 |
...
| G64 | 第七批效能缺口（AA1–AA15） | **已解決（2026-09-21）**：AA2/AA3/AA4/AA5/AA7/AA11/AA12/AA13/AA14/AA15 已落地；AA1/AA6/AA8/AA9/AA10 評估結案（fsync 必要／WS 協定綁定／readiness 需全量探測／FTS5 改排序語意／content 於 chunk metadata 內）——詳 §4.13 | 啟動、檢索與事件延遲 |
| G65 | 【部分解決】證據 checkpoint 保留缺口 | 保留策略現已保護 lifecycle 引用、engine pin，以及 maturity/self-learning evidence 報告中的 `checkpoint`／`weights_path`／artifact path（G65 test PASS）；既有被清理的 `chat-foundation-v1/final.pt` 需由重新產出的證據鏈補回 | 證據引用失效（詳 §2.3／§2.7） |
| G66 | 封印根與外部簽章未補 | 法典已直接施工至 `2026-09-21T07:37:58Z`（最新正式版本），但 `seal_manifest` 最新列仍 `2026-09-19T14:58:26Z`；`content_root`／`identity_root`／`full_root` 未重算；`revision_history` 未追加條目；`epoch_seal_manifest` 未更新 | 封印完整性與外部可驗證性（governor 側） |
| G67 | 正式規則 parity 宣告已達成，評估器 parity 待施工 | r2 稽核拒絕後複查發現：live 法典 **64/67 條已在 `declared-pending-evaluator-parity`**（前批 re-tiering amendments 已完成宣告），3 條 retired 屬終態不應轉——原請求已被現況取代（r3 建構以 `declared-pending→declared-pending` 非法自我轉移 fail-closed 證實）；剩餘真缺口為各規則的 evaluator 實作與 parity 證據（→verified→active） | 評估器 parity 實作待施工 |
| G68 | 法典版本軸不一致 | 已解決：amendment `version-axis-and-shared-layer-root-2026-09-21-r3` 經五方稽核 PASS＋governor 核准發布——`current_version`／`active_provision_binding_version`／`current_version_identity` 已同步 `2026-09-21T12:00:00Z`，`codex_version` 推進 `2026-09-21T14:19:09Z`（commit `3b3f96ef`） | 已解決 |
| G69 | 法典更新工具鏈已建 | 全鏈已實證：request→successor→五方稽核→`ready-for-governor`→核准→`execute_amendment --apply` 五階段發布（`2026-09-21-r3` amendment）；wire 階段補 Windows delete-share 重試＋失敗復原 RO；發布檔 ACL（SY:R/BA:R/BU:RW 無 Delete）維持——os.replace 需擁有者臨授 Delete，屬刻意硬化 | 已解決（首次全鏈實證） |
| G70 | 雜湊與規則狀態慣例已文件化 | `codex_amendment_contract.py` 成碼記錄 `CONTENT_HASH_ALGORITHM=sha256-canonical-json-utf8-v1`、`REVISION_ENTRY_HASH_ALGORITHM=sha256-canonical-utf8-pipe-v1` 與可重算 `canonical_json`；規則狀態機（proposed→declared-pending-evaluator-parity→verified→active）由 builder 強制檢查 | 已解決（詳 §10.36 P4–P5） |
| G71 | Request 生命週期與世系鎖已實作 | `codex_amendment_lifecycle`：predecessor 版本比對、過期 request fail-closed 轉 `rejected`＋釋放 lineage lock、`successor-built` 狀態機、執行紀錄收斂；同世代競爭由鎖定序保證 | 已解決（詳 §10.36 P7–P8） |
| G72 | shared-layer 法典路徑漂移 | 已解決：同一 amendment 發布 `SHARED_LAYER.physical_root = E:\GPTBridge\shared-layer`（permission-sovereign 目錄 parity 驗證通過；commit `3b3f96ef`） | 已解決 |
| G73 | 【已解決】Release 非自足（Source Tree 載入） | RC 已納入 `backend`／`main-system/governance`＋`package.json`／`governance_rule`／`shared-layer`；RC venv（G76）＋`GPTBRIDGE_RELEASE_ROOT` 下完整 `import main` PASS，`core_system`／`governance_rule`／`shared_layer`／`governance` 全部解析於 RC root，零 `SOURCE_TREE_LEAK`；`validate_release_bundle` `ok=True`；flat-backend `governance` 路徑修正已提交（`a2754738`，下一 RC 建置自動納入） | Release 自足性（詳 §10.37） |
...
| G75 | 【已解決】SHARED_RUNTIME 隔離缺口 | RC 已納入 `shared-layer/src` 複本；完整 `import main` 於 RC venv＋`GPTBRIDGE_RELEASE_ROOT` 下，`shared_layer` 實測解析為 `rc-2026-09-21/shared-layer/src/shared_layer/__init__.py`（非 Source Tree）；`SHARED_RUNTIME_SOURCE_TREE_GAP` 消除 | 隔離缺口（詳 §10.37） |
| G76 | venv 最終位置重建（已解決） | RC `runtime/releases/rc-2026-09-21/venv` 以 `uv sync --locked` 於最終位置建立：CPython 3.11.9、`include-system-site-packages=false`、`home` 指向機器 Python（非 repo 路徑）；52 dists 實測 import（torch 2.14.0+cpu、numpy 2.4.6、psycopg、psutil）；契約 `require_built_at_final_location=true`＋`pyvenv_cfg_sha256` 釘定 | Release Runtime 可搬移性（詳 §10.38） |
| G77 | 依賴鎖已接入 Release 流程（已解決） | `uv lock --check` PASS（83 packages resolved）；`uv sync --locked --extra local-model` 於 RC venv 重現安裝 45 套件；`dependency_lock` 改記 RC 實測 identity（`a8199078…`，52 entries）；`validate_release_bundle` lock 驗證 PASS。註：lock 內 torch 為 PyPI **CPU** build；GPU（cu130）屬發行政策另行裁定 | 依賴可驗證性（詳 §10.38） |
...
| G80 | 【已解】.exe／.dll 原生清單實測 | `psutil.memory_maps` 實測 core_system+shared_layer import 面：`native_extensions` 7 項（sovereign pyd＋numpy×2＋psutil＋psycopg×2＋`_xingcheng_inference` C++ 引擎 pyd，required=false／G29 路由啟用時才載入）、`required_dlls` 5 項（openblas/msvcp140/libcrypto/libpq/libssl）；`native_extension_errors` 驗證 0 error；`native_measurement` 記錄量測方法與 torch/stdlib 邊界 | — |
| G81 | 服務重複實例防護（已解決） | `service_topology` 閘門＋runtime 強制：ToolboxService per-tool asyncio lock（`_tool_start_locks`）＋跨程序 atomic `runtime/state/tool-start-locks/<tool_id>` reservation＋`backend_lifecycle.register` 拒 `already-registered`；**跨世代 lease 已驗收**：`test_tool_start_lease_fencing.py` 9 passed（跨實例持有互斥 fail-closed、死 owner lease 回收、毀損 owner.json 視 dead、非 owner 不得釋放、tool_id 白名單 5 例防注入） | Runtime 強制（詳 §10.39） |
...
| G83 | 【已解決】resident 重啟迴圈與 registry 陳舊（2026-09-21 長時實證通過） | `reconcile()`＋啟動前即時 reconcile＋30s `toolbox-process-registry-reconcile` 排程；跨程序 start 目錄鎖＋程序內 asyncio lock 雙層去重；`_pid_alive` zombie-aware。**實證**：`scripts/toolbox-soak.py` 30 分鐘 soak（`toolbox-soak-g83.json`：1800s／60 採樣／零 anomalies／零 arity 標記／無重複-running 或 dead-PID 列）＋ local-model 10 分鐘 PASS（`toolbox-soak-g83-local-model.json`）＋60s smoke PASS；每筆都經受管 `start_tool` 路徑啟動。**真實後端 soak**（`toolbox-soak-g83-backend.json`）：`--backend` 模式以 boot-core 同源簽章 spawn `main.py --serve`，12 分鐘 24 採樣零 anomalies、後端日誌零 `auto-loop error`／`_notify_anomalies`（G82 arity 實證）、terminate 後 registry reconcile 乾淨收尾 | 30 分鐘 soak＋真實後端 12 分鐘皆 PASS |
...
| G85 | Backend Token 暴露給 Renderer | 已解決：`app:get-backend-session` 只回傳含 30s workspace-bound HMAC ticket 的 loopback URL；backend 驗簽章／expiry／instance／nonce one-time use；`test_g85_websocket_ticket.py` 3/3 含**真 socket handshake E2E**（accept→replay 403→無憑證 403→legacy token 通過→竄改 403） | 已解決（詳 §10.43） |
| G86 | 已解決（2026-09-21，並行工作者）jieba eager import 造成 GPU 降級回歸 | `shared_layer/__init__.py` 已改 lazy `__getattr__` export→`GpuCoordinator import OK`；VRAM 競爭屬設計內 fail-soft；**W5-4 已複驗**（`c55bf3b2`：`[META] device=cuda`＋帳本 3 筆 cuda 執行） | 已解決 |
| G87 | 已解決（2026-09-21，並行工作者；後收編）self-health 測試缺檔 | 測試檔已隨工具收編搬至 `Standalone tools/local-model/tests/test_chinese_semantic_engine_v2.py`（9 tests PASS） | 已解決 |
...
| G92 | 已解決（2026-09-21 總督裁定收編）工具 manifest 缺失 | `chinese-semantic-engine` 依總督指示收編為星澄內部模組，不再是獨立工具——獨立工具 manifest／identity／route 登錄全數移除（architecture_registry、tool_routes、identity_groups、identity_permissions、code_rule_directory、audit helpers、parity／sovereign registry）；v2 引擎改置 `local-model/.../xingcheng/infrastructure/chinese_semantic_engine_v2.py`（internal-module），直 import 星澄內部即為合法路徑 | 已解決 |
...
| O7 | 多檔 | ✅ 評估結案（2026-09-21）：稽核目錄全量掃描無手動布林旗標迴圈或深巢狀條件實例——殘存 append-過濾迴圈已屬慣用寫法；大規模重寫只會與並行編輯衝突 | — | — |
...
| 3 | S4 | `shared-layer/src/shared_layer/startup.py:105-174` | **已修（2026-09-21）**：qdrant＋ollama 兩個非 critical 探針改 `ThreadPoolExecutor(2)` 並行；critical hard gates 維持序列早退——報告形狀與 READY／DEGRADED／FAILED 判定不變 | 已落地 | 35 startup 測試 PASS |
| 4 | S2 | `adaptive/tuner.py:69-84` | **已修（2026-09-21）**：慢性成長 ×1.25（原 ×2）、非急性縮減 ×0.75（原 ×0.5）、upsert ×1.25/×0.75（原 ×1.5/×0.5）；`_last_direction` 遲滯——縮減後成長需 2× dwell；急性 `_latency_rising` 路徑 ×0.25 立即保護不變 | 已落地 | 21 adaptive 測試 PASS；envelope clamp 護欄不變 |
| 5 | S3 | `file-sorter` infrastructure（25 模組） | **已修（2026-09-21）**：`sorter_engine` facade 改 PEP 562 惰性再導出——facade import 零子模組載入（35.5ms 全量→僅按需鏈）；實體檔不合併（12 模組 23 引用點，合併 churn 無對價） | 已落地 | facade 惰性驗證＋19 file-sorter 測試 PASS |
| 6 | S6 | `governance_rule/execution/audit/__init__.py:22-31` | ✅ 評估結案（2026-09-21）：O1–O7 已供程序內 mtime 快取（稽核 ~8s）；**跨執行結果快取不做**——稽核是 pre-commit 閘門，檢查讀取的外部輸入（DB／env／git 狀態）無法完整納入內容雜湊，宣告式輸入清單漏一項即靜默失效，違 fail-closed | — | — |
| 7 | S7 | `workflow/saga.py`／`integration/data_platform.py` | **已修（2026-09-21）**：真缺口是 `apply_pool_limits` 從未被呼叫——已補 `ConnectionManager.set_max_size`（縮容於歸還時惰性退役、queue 滿即關）＋`peek_connection_manager`，並於 maintenance_controller_integration 觀測點接線 tuner→pool；workload→PriorityClass 對映（`admission.class_for_workload`）已存在 | 已落地 | 43 測試 PASS＋縮容退役 sanity 驗證 |
| 8 | S8 | `.kilo/worktrees/rustic-calcium/benchmark_detailed.py:2` | **已修（2026-09-21）**：主樹根目錄 `benchmark_detailed.py`／`benchmark_fast.py` 的硬編碼 worktree 路徑改為 `Path(__file__).parent` 相對解析；主樹直接可跑（fast 6.11×） | 已落地 | 可攜執行驗證 |
...
| --- | --- | --- | --- | --- |
| R1 | Python 常駐分離 | **已修（2026-09-21，`1bd82e70`）**：真實訓練器改於受治理子程序執行（`training_job_worker`，spec/summary JSON 檔案契約、不共享記憶體）；常駐推論進程不承載訓練負載；`isolate_process=False` 可關閉 | 已落地 | RAM、CPU |
| R2 | C++ 推論用 native/core | C 核心（parser／vector／transformer＋SIMD）僅經 dispatch 部分路徑 | C++ 引擎（P3）統一呼叫 C 核心，禁止重複實作 | CPU、延遲 |
| R3 | GPU 計算驗證 | **已驗（2026-09-21 profiler 實測）**：`expected_backend=mem_efficient`（profiler 觀測派發，非旗標）；`kernel_compiled`：flash_attention=**false**（本 build 未編譯，旗標 true 不代表啟用）、cudnn=true、mem_efficient=true；GEMM 實測命中 `cutlass tensorop_s1688gemm`（Tensor Core／cuBLASLt 路徑）、tf32=on | cuBLASLt／Tensor Core 生效有證據；FlashAttention 不存在已如實記錄 | GPU 效率 |
| R4 | 模型載入按需／卸載 | **已落地（2026-09-21）**：`native_engine`（:744-754）與 `cpp_runtime`（:447-458）皆註冊 `AutoReleaseManager`＋`touch`；R10 訓練態切換時主動釋放推論資源並記帳本 | 已落地 | RAM、VRAM |
...
| R6 | KV Cache 分頁／上限／量化 | **已修（2026-09-21，`92143ecc`）**：C++ 引擎改 16-token block 分頁 KV（block table＋free list＋按需配置）；`kv_memory_bytes` 回報實際用量；`KV_MEMORY_LIMIT_EXCEEDED` 保留 | 已落地＋`test_cpp_kv_paged_allocation_on_demand` | VRAM |
| R7 | Python Import 延遲載入 | **已修（2026-09-21）**：`native_engine` 改 `_native()` 惰性命名空間（double-checked lock）——torch/native_transformer 延到首次推論才匯入；control_status／settings 輕量路徑免 torch 成本 | 已落地 | 啟動、待機資源 |
...
| R10 | 生命週期切換 | **已修（2026-09-21，`1bd82e70` 同批）**：`*_TRAINING` 進入時 `_apply_lifecycle_resource_action` 卸載 auto_release 登錄的推論引擎；動作記 `lifecycle-resource-actions.jsonl` | 已落地 | 全機資源 |
...
| 中 | W5 | `shared_layer/cache.py:34-54`（另有 `database/locator_cache.py`） | **已修（2026-09-21）**：`LRUCache` 內部改 16 分片（key 雜湊＋每分片獨立 RLock）；API 不變；逐出為分片 LRU（每片 `ceil(max/16)`） | ~~分片 LRU~~ | 14 cache 測試 PASS；併發爭用降 |
| 中 | W6 | `shared_layer/store.py:76-122` | **已修（2026-09-21）**：忙等迴圈改 `threading.Condition.wait(remaining)`＋歸還時 `notify()`；池連線 `closed` 健檢丟棄後重試；timeout 語意不變 | ~~queue＋timeout＋健檢~~ | 142 store/cache 測試 PASS |
| 中 | W7 | `audit/audit_checks.py:_audit_workers` | **已支援 `GPTBRIDGE_AUDIT_WORKERS`；上限 4 為實測（8 慢 25%），非寫死** | 區分 I/O／CPU 檢查；保留實測上限 | 稽核時間 |
| 中 | W8 | `local/vector_store.py:241-269` | **已修（2026-09-21）**：向量改 BLOB（`struct.pack '<Nd'` float64）＋`_unpack_vector` 雙格式讀（舊 JSON 行相容）；IVF 評估結案——候選已限 ≤500 的降級本地快取，ANN 分區收益不抵索引維護成本 | 已落地 | 18 vector 測試＋雙格式 sanity PASS |
...
| 低 | W10 | `native/core/transformer.c` matmul | **評估結案（2026-09-21，有基準）**：`native_core_v1.json` 實測 mm_large native 94.6× Python（20.9ms vs 1980ms）、parity 全過；瓶頸實測在 MoE Python-loop 路由（505 tok/s）非 matmul；歷史融合／展開變體曾回歸。快取分塊屬 C++ 層（W1/P3）職責，C 核心維持現狀 | 延後至 C++ 層一併評估 | 大矩陣吞吐 |
| 低 | W11 | `store_async.py` | **評估結案（2026-09-21）**：池驅動為 psycopg3（非 psycopg2），其原生 `AsyncConnection`/`AsyncConnectionPool` 才是正確遷移路徑，無需 asyncpg；現 `to_thread` 橋接刻意共享受管池＋provenance/授權同步路徑。雙池雙驅動翻倍連線面與治理複雜度，受管請求吞吐（池上限節流）下收益不抵成本——記為未來遷移選項 | 已評估：延後，遷移標的更正為 psycopg3 async | 高併發 I/O |
...
| X2 | `chinese_semantic_engine.py:717-727` | **已修（2026-09-21）**：數字檢查提前至未折疊 token（digit 不受 casefold 影響，跳過 casefold）；`Counter` 取代手建 dict；長度檢查保留於折疊後（`ß`→`ss` 語意不變） | 7/7 引擎測試＋關鍵字 sanity PASS | 語意分析 2–5× |
| X3 | `fast_inference.py:234-270` | **已修（2026-09-21）**：`_example_token_index()` 以 `(id, len)` 失效的 intent→`frozenset` 索引快取取代每呼叫全量 tokenize；intent 預過濾一併完成 | 行為等價驗證（transient 一致＋失效正確） | 3–10× |
| X4 | `vector_store.py` | **與 W8 重複** | 併入 W8（BLOB＋結構化解析） | — |
| X5 | 同步審計子進程 | **與 O6／W7 重複** | 併入 O 系列（快取／庫調用） | — |
| X6 | `self_commit.py:113-119` | **已修（2026-09-21）**：狀態指紋改由單次 porcelain 掃描推得 staged/unstaged 摘要，省去每輪兩次 `git diff --numstat` 子進程；髒態偵測語意不變 | 與 X7 同批驗證 | watch 週期 −90% |
| X7 | `architecture_registry.py:285-327` | **已修（2026-09-21）**：DFS 位置索引 dict（`position[node]=depth`）取代 `visiting.index()`——環路徑擷取 O(1)，整體 O(N²)→O(N) | 43 governance 測試 PASS（4 項失敗皆為並行工作者 `git_automation.py` 之既存靜態違規，與本改無關） | 驗證 O(N²)→O(N) |
...
| X11 | `store_helpers.py:53-57`／`sqlite_store.py:129-134` | **已修（2026-09-21）**：`sqlite_store._id`/`_json` 委派 `store_helpers.normalize_id`/`encode_json`，移除本地 `_MAX_ID`/`_MAX_BYTES` 複本 | 83 store 測試 PASS | 一致性 |
| X12 | `tool_routes.py:104-162` | **評估結案（2026-09-21）**：三個 `authorize_*` 已是分層鏈（workflow→target→route 逐層委派），非重複分散；策略類包裝會遮蔽 fail-closed 層級，不改 | 代碼審閱 | 可維護性 |
| X13 | `database.py:112`、`automation_supervisor` | **部分修復（2026-09-21）**：`directory_authority.GPTBRIDGE_PROJECT_ROOT` 改為 env 覆寫＋模組位置推導（`Path(__file__).parents[2]`），release/非 E: 部署可攜；`governance_policy.py` 等政策宣告字面量屬凍結權威文本（IMMUTABLE_AUTHORITY_ROOTS），刻意不改 | 8 directory/path 測試 PASS，解析仍得 `E:/GPTBridge` | 可攜性 |
...
| Z6 | `ipc/server_tokens.py` | ✅ 評估結案：token 檔案是跨程序共享憑證，fsync/原子替換為崩潰一致性必要設計；讀寫僅發生於啟動/驗證路徑非熱路徑 | — | — |
| Z7 | `runtimeStatusStore.ts` | ✅ 核對屬實已修：逐欄位 `Object.is` 比較＋變更才 `{...state}` 拷貝＋按欄位 listener 桶通知 | — | — |
| Z8 | `connection_watchdog.py` | ~~無抖動~~ **已修**：probe interval 已加 `random.uniform(0.9,1.1)` jitter（:236） | ~~加入 jitter~~ | 避免 thundering herd |
| Z9 | `python-backend.ts` | ✅ 核對屬實已修：共用 `backendProbeAgent`（keepAlive:true） | — | — |
...
| Z14 | 多處 | ✅ 評估結案（2026-09-21）：專案慣例為**每模組 `*_constants.py`**（24+ 個常數模組）＋`git_tiers/paths.py`；單一全域常數模組與既有架構衝突。根路徑字面量已由 X13 集中至 `GPTBRIDGE_PROJECT_ROOT`（env＋模組位置推導） | — | 可維護性 |
| Z15 | `hot_reload_watcher.py` | ~~except pass 無日誌~~ **已修**：`_loop` 例外走 `_log({type:hot_reload_watcher_error})`（:249） | ~~記日誌~~ | 可診斷性 |
| Z16 | 全專案 | ✅ 評估結案（2026-09-21）：關鍵路徑覆蓋已存在——Attention `test_phase_native_dispatch.py` 12/12（C ABI parity）、VectorStore 22 測試、Saga/workflow `test_workflow_*` 38 測試、Repair 5 個專屬測試檔 | — | 回歸保護 |
...
| 編號 | 位置 | 現況／重複關係 | 行動 | 收益 |
| --- | --- | --- | --- | --- |
| AA1 | `ipc/server_tokens.py` | ✅ 評估結案（同 Z6）：跨程序憑證必須落地；fsync 為原子性要求，不節流 | — | — |
...
| AA8 | `tasks/state_change_notifier.py` | ✅ 評估結案：readiness 本質需全量探測才能偵測變化（差量無法知道未探測者是否變）；評估已 `to_thread` 卸載不阻塞 IPC loop | — | — |
| AA9 | `local_sqlite_rag_repository.py:303-347` | ✅ 評估結案（2026-09-21）：A207 已把 `instr` 粗篩＋`LIMIT ≤500` 下推 SQL——Python `str.count` 只跑在 ≤500 有界候選上；FTS5 需虛表＋寫入同步＋既有行遷移，且 `bm25` ≠ 現行 `Σcount(token)` 會改變排序語意——不改 | — | 檢索效能 |
...
| AA12 | `command_router/_audit_writer.py` | ✅ 已落地（2026-09-21）：`append_audit_record` 共用寫入器；兩 handler 各 -7 行重複碼 | — | 一致性 |
...
| P9 | 版本軸不一致 | 已同步（G68 結案，amendment r3 發布） | 已解決 |
...
| 新 Backend × 舊 UI 可啟動性 | `python-backend.ts`／`ipcSession` 證據＋IPC surface pairing 驗證已接（G74） | 完成 | 啟動演練列 G73 |
| Python 執行環境盤點 | §10.38 表（venv/site/PYTHONPATH/prefix/cwd/native） | 完成 | G76（重建待做） |
| `runtime/releases/<id>/{backend,dependencies,manifest.json}` | `runtime/releases/rc-2026-09-21/` 已建（backend／config／governance_rule／main-system／runtime／shared-layer＋release-manifest.json＋acceptance-report.json；依指示不建第二套 builder） | 完成 | 內容完整性待 G76／G77 |
...
| Release 不受 `pip install/upgrade` 原地修改 | `star-release-payload/v1` 快照於封裝後寫入 manifest，`validate_payload_snapshot` 驗證（G78，28 tests） | 完成 | — |
| Dependency Manifest 欄位（runtime/arch/lock/native/env identity） | 實測寫入（env identity＝prefix＋pyvenv hash） | 完成 | — |
| Shared dependencies（shared-layer／local-model／governance_rule） | `shared_layer_classification` 13 項＋`governance_references`＋local-model runtime 分類（`xingcheng/runtime` 列 persistent_data、tests/scripts 列 development_only；G79） | 完成 | — |
...
| 不建第二 Shared Layer Service／治理權威 | `service_topology` 閘門＋runtime 強制層已驗收：跨程序 lease fencing 9 tests（G81） | 完成 | — |
...
32/32 測試與治理稽核 PASS；G76 venv 最終位置重建、G77 lock 接入、G73/G75 完整啟動演練（`import main` 全鏈零 `SOURCE_TREE_LEAK`、`validate_release_bundle` `ok=True`）皆已於 RC `rc-2026-09-21` 完成；維持「驗證通過後才啟 Standby Backend」。
...
| 法典／治理 | 4/14（約 29%） | 法典直接施工完成（A551–A592、鏡像、架構圖）；更新工具鏈已建（G69–G71：successor builder／lifecycle lock／hash 慣例成碼）；封印根、修訂鏈、目錄對齊未完成（governor 側） |
| Release 隔離 | **已收斂（2026-09-21）** | 驗證器／契約／32 測試完成；payload snapshot（G78）、local-model runtime 分類（G79）、原生清單（G80）、lease fencing（G81）、RC manifest＋IPC pairing（G74）、venv 重建（G76）、lock 接入（G77）、完整啟動演練零 `SOURCE_TREE_LEAK`（G73/G75）皆已落地 |
| 主系統收斂 | 14/19（約 74%） | lease／lifecycle／registry／blue-green／release manifest／backend contract／reconnect+backpressure／retention／runtime-state／retry-policy／dev-prod 分離已落地（G46–G49/G52–G58/G61/G63；並行工作者＋本輪）；G62 串流驗收已過 |
| 效能優化系列 | **近全收斂（2026-09-21）** | O1–O7、S1–S8、W2–W11、X 全列、Y 新項、Z1–Z20、AA1–AA15、R 系列多項皆已落地或評估結案；僅 W1（C++ attention，屬 P3）與 G26 事件化半邊仍開；MS5–MS10 屬 G32 |
| 其他（UI／工具／修復） | 3/9（約 33%） | 外部協作、修復 E2E、保留策略等未完成 |

**整體**：G 缺口 **完成 70、部分落地 6（G29／G30／G39／G56／G67／G83）、未完成 15**（2026-09-21 以主帳本狀態欄重算，收斂 G68/G72 amendment 發布與並行工作者結案）；施工批次 **W0／W1 完成、W2–W4 未完成**；
Phase／P 標記 **7/23（≈30%）**。已完成部分集中在
「法典施工、模型訓練／評測前段、RAG 混合平面驗收、Release 驗證層與 RC 封裝演練、效能優化系列（O/S/W/X/Y/Z/AA）」；
未完成部分集中在「法典封印殘項（G66 外部簽章／G67 parity 補證據重提；G68–G72 已由 amendment r3 全鏈實證發布結案）、
治理阻塞（G85 token／G89 DSN／W5-5 審批）、C++ 推論層（P3/W1/G41/R5/R9）、模型品質（G40 九類未達標）與端到端演練（G36/G59 INT-10 24h）」。
...
`contextBridge`＋通道白名單；**G85 部分解決**：`app:get-backend-session` 僅回傳 30 秒 workspace-bound HMAC ticket URL，Renderer 不取得 backend session token；backend 驗證 expiry／instance／nonce one-time use。
...
**W5-2（G87）**
- 檔案：原 `Standalone tools/chinese-semantic-engine/tests/test_chinese_semantic_engine.py`——工具已收編進星澄，現為 `local-model/tests/test_chinese_semantic_engine_v2.py`。
...
**W5-5（法典）**
- ✅ **已結案——請求撤回**：A593／A594 的關注點已由既有條文與實作覆蓋（jieba lazy import 修復、
  self-health 測試補回），治理側決定**不擴張 Article**（NEW_MAIN_ARTICLE = DENY，§10.36 硬規則④）；
  請求檔已更名 `…-2026-09-21.withdrawn.json` 防誤送，未進入 amendment 生命週期。

**順序**：W5-1 → W5-2／W5-3 → W5-4 → W5-5（法典）。**禁止**：修改正式運行環境、以測試為由
pip install、繞過既有驗證器、動法典與主宰權責。

**現況（2026-09-21 晚）**：W5-1 ✅、W5-2 ✅（測試檔已補）、W5-3 ✅、W5-4 ✅（VRAM 釋放後複驗：`device=cuda`、帳本新增 cuda 執行項、latency≈9.2s／eval=32）；
...
**G3 工具整合（階段 9）（已解決）**
- 現況：`tool_call_format` 格式層＋`GovernedToolExecutor`（唯讀白名單、參數逐欄位過濾、static 注入不可覆寫、結構化錯誤不擊穿迴圈、`tool_trace` 稽核）＋`test_tool_call_dataset.py`／`test_tool_orchestrator_distill_bridge.py` 已具。
- 端到端驗收（`tests/test_tool_bridge_e2e.py`，3 passed）：`status`／`memory_list`／`sql_list_knowledge` 經真實 `LocalAiService.handle`（命令 registry→參數驗證→handler）執行成功；真實鏈路失敗（tmp root 非 git repo 的 `platform_status`）回 `TOOL_EXECUTION_FAILED` 且後續呼叫不受影響；`converse` 完成「生成→工具執行→tool 訊息回填→最終回覆」完整回合。
- 順帶修正真實鏈路 arity 缺陷：`LocalSqliteCognitionRepository` 建構子多傳 `pool_manager`、`list_knowledge` 誤傳 `knowledge_type`/`limit`（改於應用層過濾＋截斷）——`xingcheng_sql_list_knowledge` 原先在真實鏈上必然失敗。
- 驗收：≥3 工具端到端案例（含失敗回復）✅；schema 驗證（`TOOL_CALL_INVALID`／`TOOL_NOT_ALLOWED` fail-closed）✅；路由稽核（`tool_trace`）✅。

**G40 原生對話訓練未建立（部分完成）**
- 現況：角色／遮罩格式與 SFT 合併資料 v2–v6 已具。
- **九類端到端已實跑（2026-09-21，CUDA）**：`capability-eval-20260921-201715.json` 對 `chat-foundation-xl-v3/final.pt`（sha `56bc1c63`）跑完 `star-capability-suite-v1` 全部九類——en-ppl（ppl 2817.7）與 code-ppl（321.2）通過；zh-TW ppl 10684.8／3097.5 未達標、math/code-regex/reading/multi_turn/context_tracking/instruction/tool_call 全數未過（chat_format 回覆多為非預期 token 序列）。結論：**評估 harness 端到端成立，缺口在模型對話品質（SFT 資料／容量），非驗收鏈路**。
...
**G56 Release Manifest（已解決）**
- 現況：release dependency contract＋frontend release manifest 已具；RC `rc-2026-09-21/release-manifest.json` 已產生（release_id／builder／source_commit／dirty_state／build_hashes／dependency_lock／python_runtime／ipc_contract／governance_compatibility／persistent_data_boundary，secrets 未複製）。
- 實測：`validate_release_bundle` 以 RC cwd、release-owned `backend`／`governance_rule`／`shared-layer/src`／`venv` roots 驗證，結果 `ok=True, errors=[]`。
- 已完成：venv 最終位置重建（G76）、lock identity 接入（G77）、native pyd＋DLL 清單改以 RC release root 解析；切換前五項相容檢查仍由 release verifier 執行。
- 依賴：G74（`runtime/releases` 佈局）✅、G76/G77。
- 驗收：manifest 產生＋`validate_release_bundle` PASS；舊 UI 可與之配對。

**G73 Release 非自足（已解決）**
- 現況：RC 已納入 `backend`、`governance_rule`、`shared-layer/src`、`main-system/governance`、`main-system/package.json`；`main.py` 以 `GPTBRIDGE_RELEASE_ROOT` fail-closed。
- **啟動演練已驗（2026-09-21）**：RC venv（G76 重建）＋`GPTBRIDGE_RELEASE_ROOT` 下完整 `import main` PASS——`main`／`core_system`／`governance_rule`／`shared_layer`／`governance` 全部解析於 RC root 內，零 `SOURCE_TREE_LEAK`；`validate_release_bundle` 對 RC `ok=True, errors=[]`。
- 順帶修正 flat-backend 佈局缺口：`import governance` 需 `<release>/main-system` 上 sys.path，`main.py` 已補（`a2754738`）；現行 RC 快照的 `backend/main.py` 為修正前版本，下一 RC 建置自動納入（manifest `build_hashes.backend` 屆時重算）。
...
**G75 SHARED_RUNTIME 隔離缺口（已解決）**
- 現況：RC 已納入 `shared-layer/src` 複本；release venv origin probe 實測 `core_system`／`governance_rule`／`shared_layer` 全部解析於 RC root 內。
- **完整 import 鏈證據（2026-09-21）**：`import main` 於 RC venv＋`GPTBRIDGE_RELEASE_ROOT` 下，`shared_layer` 解析為 `rc-2026-09-21/shared-layer/src/shared_layer/__init__.py`——非 source tree；`SHARED_RUNTIME_SOURCE_TREE_GAP` 消除。
- 依賴：G73 ✅。

**G76 venv 漂移與可搬移性（已解決）**
- RC venv 已在 `runtime/releases/rc-2026-09-21/venv` 最終位置重建；`pyvenv.cfg` hash 已釘入 contract，Python 3.11.9／Scripts／native 依賴可用。
- 驗收：`require_built_at_final_location=true`、Release bundle validation PASS。

**G77 依賴鎖（部分解決）**
- RC 已以 `uv.lock` 重建最終位置 venv；7 distributions 的 lock identity 與實際安裝一致，`uv lock --check`／`uv sync --locked` 通過。
- 剩餘：wheel cache／離線重建環境建置器正式接入 release pipeline。

**G80 原生清單（已解決）**
- RC contract 已登錄 `.pyd`／required DLL 清單，並將路徑解析至 RC `venv`／`backend` release roots。
- `validate_release_bundle` 已實測缺 DLL／錯 arch／錯 ABI fail-closed；現行 RC 實體清單一致。
...
**G84 Preload 通道漂移（已解決）**
- 現況：drift 3→0——`app:get-repair-status`／`app:open-path`／`app:set-ui-zoom` 皆有 `ipcMain.handle`。
- 驗收：`allowedInvokeChannels ⊆ ipcMain.handle` 自動檢查已落地（`test_preload_channel_contract.py` 3/3 PASS）。

**G85 Backend Token 邊界（已解決）**
- Main process 以 IPC session token 產生 30 秒 HMAC ticket；Renderer 僅取得 loopback WebSocket URL＋ticket，不取得長期 token。
- Backend 驗證 ticket 的 workspace instance、expiry、HMAC 與 nonce one-time use；legacy token path 保留供非-Renderer governed clients 使用。
- 驗收：`test_g85_websocket_ticket.py` 3/3——含真 `websockets.serve` socket E2E（合法 ticket 握手成功、同 nonce replay 403、無憑證 403、legacy token 通過、竄改簽章 403），policy 轉正式 must-not-expose。

**G89 DSN 秘密存於 Windows 使用者環境（大部分解決）**
- 已施工：`shared_layer/security/credential_store.py`（win32cred CredRead/CredWrite/CredDelete，`GPTBridge/` 命名空間）；`dsn_policy.resolve_dsn` 支援 `credman:` 引用＋canonical `GPTBridge/postgres/dsn/<purpose>` 回退查找；`scripts/migrate-dsn-to-credman.py` 已實遷移 runtime/admin/backup 三條 DSN（`resolved_via_store: true` 實測）。
- 剩餘：User env `GPTBRIDGE_POSTGRES_*_DSN` 清除屬操作者決策（env 仍為 dev fallback 優先級高於 store）；reader DSN 未設（本就無 env 值）。
- 驗收：`test_credential_store.py` 6/6（真 CredMan 往返＋引用解析＋canonical 回退＋fail-closed 未解析引用）。
...
staged 切換、總督啟用、舊身分退役；版本軸已同步（G68 結案，2026-09-21T12:00:00Z）。
...
### 10.56 重新分層 Batch 1（2026-09-21，successor resolution；未新增 Article）

**已執行（candidate `2026-09-21T11:30:53Z`；稽核 PASS、鏡像 errors NONE）**
- **special-law 遷移（3 條）**：`A551`（語言定位）、`A588`（模組架構來源結構）→
  `CODEX_LANGUAGE_SOURCE_NATIVE_SPECIAL_LAW`；`A552`（GPU 加速獨立軌）→
  `CODEX_MODEL_RESOURCE_GPU_SPECIAL_LAW`；同步 `provision_law_classification`（tier=special-law）、
  `provision_special_law_migration`（special-governs）與 `special_law_directory.special_provision_links`。
- **義務遷移（9 條）**：`A559／A562／A563／A569／A572／A576／A578／A582／A583` →
  `OBL_*`／`FINAL_CONVERGENCE_IMPLEMENTATION_PLAN`（`implementation_obligations`＋分類列；
  migration_status=`superseded`，successor=obligation code）；義務總數 52→61。
- **未動**：Article 內容與歷史（successor resolution，非刪除）；未 seal、未翻 current。

**進度**：重新分層 12/40（special law 3、obligation 9）；餘 28 條（contract 14／test suite 8／registry 4／
obligation 2）待 Batch 2。

**仍待（本線）**：Batch 2 分層（28 條）→ core engine equivalence／capability dispatch parity → 子主宰 registry
切換與退役 → 67 schema parity／61 obligations／closures 收斂 → normative surface／search 重建 → candidate seal
→ 外部簽章。

### 10.57 收斂框架（framework-first，2026-09-21；未動 live 法典）

> 套件：`governance_rule/execution/convergence/`（successor pipeline＋capability dispatch skeleton）。
> 全部操作先 staging；`publish` 需顯式 `approve=True` 且 staged 驗證通過；**框架不自動改 live 法典**。

| 模組 | 介面 | 用途 |
| --- | --- | --- |
| `successor_framework.py` | `stage_copy`／`apply_re_tiering`／`apply_formal_rule_disposition`／`apply_closures`／`validate_staged`／`publish`（guarded）／`version_axis_report`／`projection_status`／`compute_closures` | 受治理 successor pipeline：分層計畫執行（special-law 遷移／obligation 委派／retain-with-planned-successor）、規則處置、closure 寫入、版本軸與 derived projection currentness 報告 |
| `capability_dispatch.py` | `ModuleCapability`／`validate_module_capability`／`evaluate_dispatch`（`RULE_CAPABILITY_DISPATCH_V1`） | A592 successor 分配模型骨架：11 欄位 registry schema、能力匹配、fail-closed 評估器（禁自我選擇／自我授權；衝突或無候選一律 FAIL_CLOSED） |
| `core_engine_registry.json` | `core-engine-registry/v1` | A591 目標骨架：五核心引擎（decision／permission／system-runtime／synchronization／xingcheng），`authority_after == authority_before`、naming 由 canonical migration 決定、未啟用 |

**驗證**：`test_convergence_framework.py` 9 項（stage→apply→validate→publish-guard、版本軸／projection／
closure 報告、dispatch 五情境 fail-closed、schema 驗證、live 法典雜湊不變）；總計 76/76；稽核 PASS。

**下一步（透過框架執行，不另寫 ad-hoc 腳本）**
1. **Batch 2 分層**：以 `apply_re_tiering` 套用 `re_tiering_plan.json`（`executed` 帶入 Batch 1 的 12 條）；
   `A553／A554` → 新 special law `CODEX_XINGCHENG_MODEL_SPECIAL_LAW`（`target_law` 欄位）；其餘 contract／
   test／registry 目標先以 retain-with-planned-successor 記錄，待 successor artifact 落地再 `special-governs`／
   `superseded`。
2. **子主宰退役**：以 staged copy 演練 registry 切換（`architecture_registry` `owner_sub_sovereign` 退役）＋
   法典身分退役，通過 `validate_staged` 與稽核後才 `publish(approve=True)`。
3. **Core engine／dispatch**：equivalence 證據寫入 `core_engine_registry.json` 後才允許
   `CORE_ENGINE_TRANSITION_CLOSURE=PASS`；dispatch evaluator 待 runtime 接線（本階段不接）。
4. **重建與封印**：`projection_status` 轉 PASS 需重建 search／module manifest／normative surface；
   `version_axis_report` 轉 PASS 需 seal candidate＋修訂鏈（governor 程序）。

### 10.58 重新分層 Batch 2（經框架執行；2026-09-21）

**執行方式**：`successor_framework.stage_copy → apply_re_tiering → validate_staged → publish(approve=True)`
（live 法典僅在 staged 驗證通過後以顯式核准寫入；框架已補 `metadata.codex_version` 推進）。

**結果（candidate `2026-09-21T11:59:11Z`；稽核 PASS、鏡像 errors NONE）**
- **special-law 遷移（2 條）**：`A553`（MoE 架構方向）→ `CODEX_MODEL_RESOURCE_GPU_SPECIAL_LAW`；
  `A554`（自主訓練與升級）→ `CODEX_LEARNING_SYSTEM_SPECIAL_LAW`。
- **保留待產物（26 條）**：contract／test suite／registry 目標以 `retain`＋
  `successor-artifact-pending` 記錄（不假裝已遷移）；待對應契約／測試套件／registry 落地後再轉
  `special-governs`／`superseded`。
- **修正紀錄**：原擬新建 `CODEX_XINGCHENG_MODEL_SPECIAL_LAW`，因稽核規則「目錄 introduced_version 不得
  超過 sealed version」而撤回（未 seal 的 candidate 不得新增 law 目錄列）；改遷入既有 special law。
  框架已將 `metadata.codex_version` 納入 `apply_re_tiering`。

**分層總進度**：**14/40 完成遷移**（special law 5：A551／A552／A553／A554／A588；obligation 9）；
26 條已記錄、待 successor artifact。

**下一步**：① 產出 26 條之 successor artifacts（contract 14／test suite 8／registry 4）→ 經框架轉遷；
② core engine equivalence＋dispatch parity；③ 子主宰 registry 切換與退役；④ schema／obligation／closure 收斂；
⑤ search／module manifest／normative surface 重建；⑥ seal candidate＋外部簽章。

### 10.59 重新分層 Batch 3（artifact-backed；2026-09-21）

**執行**：框架 `apply_re_tiering(..., artifacts=<successor_artifacts.json>)` — 26 條以 artifact-backed
`superseded` 遷移（artifact 存在才遷；不存在則維持 retain）。

**產物（26 件）**：`governance_rule/execution/convergence/artifacts/`
- **contracts 14**：A555／A558／A570／A571／A573／A574／A575／A577／A579／A580／A581／A585／A589／A590
  （骨架：欄位清單＋owner＋`declared-pending-implementation`）。
- **registries 4**：A557／A584／A586／A587（欄位骨架）。
- **test suites 8**：A556／A560／A561／A564／A565／A566／A567／A568 → 既有測試／報告證據（全數存在，無缺）。
- 清單：`convergence/successor_artifacts.json`。

**結果（candidate `2026-09-21T12:02:51Z`；稽核 PASS、鏡像 errors NONE）**
- `provision_special_law_migration`（A551–A590）：`special-governs` 5、`superseded` 35 → **40/40 處理完畢**。
- 法層 tier 保留 `main-codex` 作為歷史參照；successor resolution 以 migration 表為權威（不刪歷史）。
- 契約重綁（`7cd81812…`）、唯讀還原。

**待辦（artifact 實作）**：14 契約＋4 registry 骨架需實作（`declared-pending-implementation`）方能進
contract/registry closure；其後為 core engine equivalence／dispatch parity、子主宰退役、schema／obligation／
closure 收斂、search／module manifest／normative surface 重建、seal＋外部簽章。

### 10.60 原生測試套件（C／C++／C#；不用 Python，低負載，2026-09-21）

> 位置：`native/test_suites/`。8 個 successor test suite 改以原生實作；Python 測試僅保留為歷史參考
...
**建置與實測**：`build.ps1`（MSVC vcvars64＋`cl /O2`）→ 8 執行檔；`dotnet run`（.NET 10）彙總。
結果：**8 suites／25 PASS／0 FAIL／3 BLOCKED**（verdict=`INCOMPLETE_EVIDENCE`）；核心套件總耗時 **≈0.04 ms**
（KV 0.004／MoE 0.007／NTP 0.008／parity 0.001／maturity 0.016），無 Python 行程、無外部依賴。

**證據**：`native/test_suites/bin/native-report.json`、`bin/native-orchestration-report.json`；
successor artifacts 已改指原生實作（`convergence/successor_artifacts.json`）。
**待辦**：baseline／eval／dialogue 三套件待原生模型 runtime＋checkpoint（BLOCKED）；其餘可再擴充
（INT8 KV 矩陣、GQA RoPE launch、專家權重管理等）。

### 10.61 Core Engine 等價證據與子主宰退役演練（2026-09-21）

**等價證據（`convergence/core_engine_equivalence_evidence.json`）**：五引擎逐一比對
（decision／permission／system-runtime／synchronization／xingcheng）：
`authority_after == authority_before`（rank／area／basis 相同）、decision／permission 範圍不變、
execution 僅決策協調（不執行模組工作）、資訊路徑不變；不變式：**無新增主權、無新增最高控制者、
無隱式執行權**（全部 true）。closure 維持 `INCOMPLETE_EVIDENCE`（dispatch parity 待 runtime 接線＋啟用閘門）。

**子主宰退役演練（staged only；未發布）**：框架 `apply_sub_sovereign_retirement` 於隔離複本退役
**18 個 active 子主宰身分**（sovereigns→historical/`-retired`、hierarchy edges→retired）；`validate_staged`
errors NONE；複本結果：**active 子主宰 0**、僅剩 1 條 terminal `abolished` 邊（language-review）。
**未發布**：依 A592 codex-first，需先完成 registry 切換（`architecture_registry` `owner_sub_sovereign` 退役）
＋ dispatch parity ＋ activation gate。

**下一步**：① capability dispatch parity（固定分配 vs 能力匹配，逐模組等價）；② registry 切換 staged 演練
（含 permission routes 同步）；③ 14 契約／4 registry 骨架實作；④ schema／obligation／closure 收斂；
⑤ search／module manifest／normative surface 重建；⑥ seal candidate＋外部簽章。

### 10.62 Capability Dispatch Parity（2026-09-21）

**證據**：`convergence/capability_dispatch_parity.json`（框架 `evaluate_dispatch`／`RULE_CAPABILITY_DISPATCH_V1`）。

- **45 模組**全部通過 `MODULE_CAPABILITY_REGISTRY` schema（11 欄位）。
- **身份能力派發等價**：40 個可派發模組（resident／on-demand）以自身能力需求派發 → 唯一選中自身（parity PASS）。
- **5 個 N/A**（fail-closed 排除，兩模型下皆不可派發）：`boot-core`／`bootstrap-entry`／`launcher-ui`（per-boot／per-launch）、
  `local-vector`（degraded-only）、`global-cleaner`（retired）。
- **8 個衝突**：粗粒度 `role:*` 能力碼由多模組共用；**身份碼唯一（無派發歧義）**；語意能力目錄待 artifact 實作。
- closure 維持 `INCOMPLETE_EVIDENCE`：語意能力碼、runtime 接線（dispatch 尚未啟用）、registry 切換＋activation gate。

**下一步**：① registry 切換 staged 演練（architecture_registry `owner_sub_sovereign` 退役＋permission routes 同步）；
② 14 契約／4 registry 骨架實作；③ schema／obligation／closure 收斂；④ search／module manifest／normative surface
重建；⑤ seal candidate＋外部簽章。

### 10.63 主系統 Python 常駐資源縮減計畫（規劃；2026-09-21）
...
| 啟動至 ready | **≈4.54 s**（01:57:16.707 → 01:57:21.249）；最大單一階段 `phase-5-classify-dependency-dag` **1,922 ms** | `backend-20260921.log` |
...
| 重複／陳舊 | `resident-start-shared-layer-*` 單 session 15 筆（僅 1 存活）；process registry 16 筆 claimed running vs 2 pid 存活；file-sorter 曾雙實例 | G83／§10.41 |
