...
- **未完成**：C++ 推論執行層支援 FP64＋MoE（R5）＋batch>1 packed prefill／continuous decode（R9）＋權重 INT8／INT4 量化（export→bundle→dequant-at-load）＋prefix cache（單序列路徑）＋CUDA matmul（cuBLAS fp64，env 閘 `XINGCHENG_CPP_CUDA`）；
  KV INT8 cache、device-resident 權重／KV、生產路由與資源接線未完成（§4.2 P3b–P3f）。
...
- **引擎層目標**：Python 訓練／開發路徑維持（`checkpoint.py`／`tokenizer.py`／`bpe.py`／
  ...
  核心運算續用 C 核心；引擎已支援 MoE／batch>1／權重 INT8·INT4／CUDA matmul（獨立加速軌）；**未完成**：prefix cache 重用與 batch>1·量化·GPU 之生產路由、資源接線。
...
- 待補：命令理解探針 ✅（`star-semantic-plan-probes/v1` 24/24，見收斂證據）；eval suite dialogue v1＋抽檢紀錄 ✅（v18 vs 現役權重 FAIL，見 §3.3.1）；xl-v3 啟用決策；run-to-run 變異穩定化。
- 依賴：Phase 0–4。
...
- **未完成（現況）**：`src/backend/cpp/`（`xingcheng_inference.hpp`／`engine.cpp`／
  ...
  **引擎層已支援 MoE（R5）、batch>1 packed prefill＋continuous decode（R9）、權重 INT8／INT4 量化與 CUDA matmul（`XINGCHENG_CPP_CUDA=1` opt-in，cuBLAS fp64）；尚未支援：KV INT8 cache、device-resident 權重／KV；GPU 協調由 Python 呼叫方承擔。**
...
| 期 | 目標 | 工作 | 驗收 | 依賴 |
| --- | --- | --- | --- | --- |
| ACC-1 | CPU SIMD（純 C） | AVX2／AVX-512 intrinsics 核心（matmul／softmax／rmsnorm／rope／vector）；CPUID 執行期派發＋純量回退；基準擴充 `[處理中 @devin 2026-09-22T14:36Z — 驗收證據：等價／回退／基準]` | 與純量結果等價（門檻內）；加速可重現；無 AVX 機器自動回退 | 無（可與 Phase 5–9 並行） |
...
> **act-1 shadow 接線（2026-09-22，devin）**：`connection_watchdog` 雙軌 shadow 已落地——
> `tasks/connection_watchdog_native_shadow.py`（`WatchdogNativeShadow`：policy 驅動、C FSM 並行、
> state/consecutive_dead/repair-trigger/自適應間隔逐項比對、分歧寫 `runtime/logs/native-shadow/connection-watchdog.jsonl`、
> native 異常即自停＋單筆 shadow-disabled 稽核、primary/retire 拒絕 fail-closed）＋ runtime flag
> `main-system/config/native-shadow.json`（`native-shadow-policy/v1`，connection_watchdog=shadow，
> 其餘三元件 off）＋ `tests/test_native_shadow_wiring.py` 9 測試（含真實 native lockstep）；
> 測試 23/23 綠（wiring 9＋watchdog 回歸 9＋e1 shadow 5）。生效於下一次受管啟動；
> `periodic_scheduler` shadow 接線亦落地（C 側已補 pausable/run_immediately/next_due 契約＋
> `NativeScheduler` binding，`tasks/periodic_scheduler_native_shadow.py` 同法逐項比對
> run_count/paused_count/next_due/membership，policy=shadow，`PeriodicScheduler(project_root=…)`
> 由 app_lifecycle 注入；`tests/test_scheduler_native_shadow.py` 11 全綠——pyd 已重建含
> NativeScheduler，真實 native lockstep（含 pause/defer）通過）。
> 殘：state_outbox（另有工作者施工中）、maintenance_controller 同法接線。
...
| E14 | **收尾組**：EC-1–8、效能殘項 | 低：可並行收尾 | 各自前置 |
...
| P0 | **法典收斂殘項**（§3.5 A／E）：同批 amendment 已由總督側發布（rev68 `2026-09-22T10:23:53Z`：A601 三組共同禁止條＋27 條 INHERITS、A602 業務規則下放 successor 取代 A5、A603 Git 收斂至 A375；mirror 與 runtime-rule-index／business_rule_delegation 投影 10:27Z 重建；衝突掃描 0 真／5 重疊已登錄）；殘項＝dupscan `review-required` 列之跨層重述（10 對）與模板實例（A308/A309、A316/A317）需另行 amendment 收斂 |
...
| P0 | **INTEGRATION-10（24h 穩定驗收）** | 細項：①連續 24 h uptime（新窗自 2026-09-22T12:55Z 重拉起累積；原窗 12:25Z 後端當機破裂）②零非預期重啟／dead-grace 迴圈③健康探針（readiness／boot-core）全綠④RSS／CPU 於預算內⑤日誌零 ERROR／零孤兒程序⑥完成後留存驗收報告。驗收腳本 `scripts/integration-10-acceptance.py`（六項判據→`runtime/logs/integration-10-<ts>/report.json`）。〔原窗至 12:25Z 破裂（backend 當機；governance-audit 因受護治理檔可寫而 required-phase-failed 重試耗盡），12:55Z 已重拉起〕✓三核心探針綠 ✓零孤兒；待裁定項：④101 個 worker 預算超限樣本（80 於管制中、21 在 hysteresis 偵測延遲內——舊控制律 3 樣本滯回於嚴格解讀下不可達；**全域硬上限違規 0 次**、管制 10 進 10 出無卡死——證據 `convergence/int-10-resource-budget-evidence-20260922.json`；**已定（2026-09-22 總督裁定）：採嚴格解讀，本窗判據④記 FAIL**，控制律修正已落地（`6cb00bc7` 首樣本即管制＋80% 預節流、測試 6/6、證據 `7f7b3ba9`）；新窗自 12:55Z 累積、滿 24 h 後結算）⑤31 條 error 級日誌（8×auto-loop CODEX_UNAVAILABLE 集中於單一 7 分鐘 codex 修訂窗、1×scheduled audit FAIL、1×RuntimeWarning、**21×`lock-busy:workspace-sync.lock`**——手動 sync --push 與運行中協調者鎖競爭所致，屬例行序列化被誤記為 flow failure，`6eddfdcb` 已修為 skipped:lock-busy；其餘來源均已修：`7199bdb6` 重複錯誤去重、unawaited coroutine、isolation-monitor TOCTOU；04:27Z 後零新增〔lock-busy 群 ~18:00–18:50 一次性〕；運行中 backend 為舊碼，修復效力在下一視窗）〔補〕`permission-automation-audit` 流程 07:57Z 被 kill-switch 停用（無 actor/reason 記錄）——delegated 稽核 lane 整體關閉，log 清潔部分為結構性；**視窗後須復啟**（oracle 負載下 >30s 自判 FAIL 之結構問題另待處理：lane 自身須改用 native engine 或放寬排程預算） 〔窗口作廢重起（2026-09-22 總督裁定）：12:55Z 窗因 governor 停擺恢復期兩筆 worker 超限樣本污染，作廢；取樣器 13:23Z 已以正確 5s 間隔在跑，新窗自 2026-09-22T13:23Z 起計 24h（終點 2026-09-23T13:23Z，驗收用 `--window-start 2026-09-22T13:23Z`）；其餘判據（無重啟／健康／日誌／孤兒）全綠，uptime 累積中〕|
| P1 | **C++ 推論執行層**（P3／G29／G31／G32／G34） | 細項：①量化已解除 fail-closed（權重 INT8／INT4：per-tensor 對稱量化＋真 4-bit packing，lm_head／router／embeddings 保 FP64，dequant-at-load 與 quantize_model parity 2e-3、greedy 逐字一致、fp8 仍 fail-closed；證據 `p1-quant-int8-int4-weight-only-20260922.json`）；CUDA 已解除 fail-closed（cuBLAS fp64 matmul 橋接：env 閘 opt-in、無 GPU 建置／裝置時 load fail-closed、與 C-core parity 1e-6、greedy 逐字一致；證據 `p1-cuda-cublas-matmul-bridge-20260922.json`；device-resident 權重已落地（`p1-cuda-device-resident-weights-20260922.json`）；KV 常駐、bf16 GEMM、自研 kernel 屬後續增量）；batch>1 已由 ④R9 解除；KV INT8 cache 已落地（per-token/per-head 對稱量化 packed elem＋scale，pool stride 縮 ~8x，greedy＋batch＋MoE 逐字一致、prefix-cache round-trip 一致，證據 `p1-kv-int8-cache-20260922.json`；長上下文誤差剖面歸長時驗證窗）②**W1 已閉**（`8c9020e4`：streaming QK/PV＋AVX2＋純量回退，logits 差 1.5e-14、prefill256 −24.7%，證據 `c-w1-qk-simd-evidence-20260922.json`；遺留 FlashAttention 分塊／AVX-512 入 P2）③**R5 已閉**（MoE 稀疏執行＋分組 expert GEMM：router softmax→top-k→renorm→per-expert gather/SwiGLU/加權 scatter-add；logits parity 2e-3、greedy token-exact、interval 混合層 parity、畸形 config fail-closed；證據 `p1-r5-moe-sparse-inference-20260922.json`）④**R9 已閉**（batch>1 packed prefill＋continuous decode：per-seq KV slot 命名空間共享分頁池、EOS 即時釋放、greedy 與 sequential 逐字一致（dense＋MoE）、fail-closed 邊界測試；證據 `p1-r9-batch-continuous-batching-20260922.json`）⑤P3f GPU 協調語義已接線（CUDA opt-in 經 GpuCoordinator 預算閘：授權才設 env、拒絕降級 CPU＋`gpu-budget-downgrade` 帳本、`cuda_active()` 內省；證據 `p1-p3f-gpu-coordination-gate-20260922.json`）；殘＝長時驗證（窗口型）＋device-resident KV（待 GPU attention kernel）＋bf16 GEMM／自研 kernel（nvcc/MSVC-18 不相容待新工具鏈） |
...
| P1 | **外部協作 EC-1–EC-8**（G36） | 細項：①送出／回覆鏈②session 管理③自動記憶端到端 ✅（`ec3-auto-memory-correlation-20260922.json`：correlation＋來源 AI＋可查詢重用＋免手動卡片，22/22 回歸綠）④星澄研究路徑驗證⑤A544 幾何法典同步（staged request） |
...
| P1 | **DSN 遷移收尾**（G89） | ①User env `GPTBRIDGE_POSTGRES_*_DSN` 清除（操作者決策）；②reader DSN ✅已設立（`gptbridge_reader` LOGIN ∈ `gptbridge_xingcheng_reader`、CredMan `postgres/dsn/reader`、masked-env 解析＋唯讀驗證全過，證據 `convergence/g89-reader-dsn-20260922.json`） |
| P2 | **Embedding 殘項** | ①生產端呼叫政策 ✅已登錄受管 flow（`rag-schema-migration` kind=on-demand 入 `automation-flows.json`：僅 operator／受管流程顯式呼叫，絕不自動觸發；證據 `convergence/embedding-residual-call-policy-20260922.json`）②PG metadata 軸 `apply_metadata` executor（現無，依設計 blocked——含 APPLY_METADATA 相位之 plan 恆 fail-closed 阻斷） |
...
| 編號 | 缺口 | 剩餘工作 |
| --- | --- | --- |
| G29 | C++ 推論執行層 | MoE／batch>1／權重量化／CUDA matmul 已由 R5／R9／INT8·INT4／cuBLAS 橋接解除；P3f 剩 GPU 協調語義（device-resident＋coordinator 接線）與長時驗證 |
| G30 | C# 業務編排層 | 尚缺 P7-3 正式生命週期主導權移轉演練 |
| G31 | 資源／推論優化殘項 | **R5／R9 已閉**（見 P1 C++ 推論層行） |
| G32 | 主系統十大優化殘項 | **MS6 仍開**：C++／C# 承接待 P3／P7 |
| G34 | 第三批效能殘項 | **W1 已閉**（`8c9020e4`，見 P1 C++ 推論層行）；殘項收斂至 R5／R9 |
| G36 | 外部協作未完善 | 送出／回覆鏈與 session 管理未端到端驗收；自動記憶端到端 ✅（`ec3-auto-memory-correlation-20260922.json`）；星澄研究路徑未驗證；A544 幾何法典同步待 staged request（詳 §4.2 P8） |
...
| G89 | DSN 秘密存於 Windows 使用者環境 | 已遷移 CredMan；reader DSN 已設立（`convergence/g89-reader-dsn-20260922.json`）；殘項僅餘 User env `GPTBRIDGE_POSTGRES_*_DSN` 清除——屬操作者決策（env 仍為 dev fallback 優先級高於 store） |

**無 G 編號之未完成項**（由後續章節收斂）：

- Embedding 維度／index 相容性：漂移後新世代建置已由遷移工具觸發（migrate_schema → apply_migration → rebuild_canonical，見 P2 行）；生產端呼叫政策已定並登錄受管 flow `rag-schema-migration`（on-demand，僅顯式呼叫）；殘項為 G50 其餘呼叫點遷移。
...
- **G40 原生對話訓練**：角色／遮罩格式與 SFT 合併資料已具；九類端到端評估 harness 成立。
  **L5 探針已被證明可達**：`manual-sft-v15`（chat-foundation 4805 筆）L0–L7 全階梯通過——
  缺口為資料規模，已補；echo 逐字複誦為 deterministic fail（非 run-to-run 變異）。
  剩餘：echo 探針＝罕見字元逐字複製能力漸近線（兩輪補池實測，證據
  `l5-echo-charset-diagnosis-20260922.json`）。assistant-only loss mask 驗證與
  狀態式續接（非固定答案）測試已完成（`g40-lossmask-continuation-20260922.json`）。
  依賴：Phase 5D（格式層）／5E。驗收：九類探針通過率達標；回合邊界、逐字複誦、限定回答、多輪記憶。
- **Phase 5 待補**：命令理解探針（`star-semantic-plan/v1` 對齊）✅ `star-semantic-plan-probes/v1`
  24/24 通過（證據 `governance_rule/execution/audit/convergence/semantic-plan-probes-20260922.json`）；
  eval suite dialogue v1＋抽檢紀錄已補：v18（weights v21）對現役權重評估 **FAIL**
  （ppl 150.23 vs 基線 71.66＝+109.6%＞40% 閘；tps ratio 通過；抽檢 6/6 為背誦式
  parrot——證據 `dialogue-eval-v18-chatpath-20260922.json`）；人工裁決屬總督。
- **L5+ 認證權重現有兩個**（均未啟用）：xl-v3（v16，vocab 32000 與現行 82M 線 tokenizer 不相容，
  啟用屬 Phase 8 路由決策）＋`manual-sft-v15`（v19，2026-09-22，**8192 現行線首個 L0–L7
  全階梯認證**——small 27M 架構；L5 echo 探針有 run-to-run 變異）。v15 證明 L5 缺口為
  資料規模問題：chat-foundation 4805 筆即可達標；base 架構同 recipe 重訓或 v15 啟用裁決為後續路徑。
...
- ~~法典 `codex_semantic_duplication_scan` 仍為佔位列~~ → **已填充**（r10 amendment executed：
  `DUPSCAN@2026-09-22T03:49:55Z` 實測 800 條文／319,600 對；15 跨層重述＋151 禁止清單共享＋
  2 模板實例列已入法典，舊 UNKNOWN 列標 superseded；掃描器 `scripts/codex-semantic-dup-scan.py`）。
  **殘項**：合併／引用化收斂需另行 amendment（`merge_candidate=review-required`，不自動合併）。
  **去重已發布（rev68 `2026-09-22T10:23:53Z`）**：A601 承載三組共同禁止條款
  （SPECIAL_LAW_BOUNDARY／SENSITIVE_SCOPE_BOUNDARY／SQL_GOVERNANCE_BOUNDARY），
  27 條 active 條文已改 `INHERITS:A601#<clause>` 引用；已 superseded 條文（A236、A248、
  A529、A530 等）僅保留歷史原文。殘項＝dupscan `review-required` 之跨層重述與模板實例列
  （見 §3.5.0 A）。

**B. 法典 vs 工程實作要求（收斂＝工程改引用，不重述）**
- 【P2 進行中】AGENTS.md 重述段落清單已產出（①完成）——
  Git Commits／Hooks／Worktrees→A53／A163／A245／A375（A375 為單一控制條）；
  Self-Commit／Worktree-Sync／Git-Automation→A163＋A375；Self-Learning→A554；
  Model-Maturity→A556／A557；Data-Retention→A113／A114；GPU-Gate＋Auto-Release→A239＋A116；
  Lazy-RAG/CAG→A586／A587；On-Demand-Activation→A586；Resource-Governor→A30／A116／A593；
  Architecture-Registry→A201／A232／A281。
  `architecture-*.md` 屬法典投影檔（wire 重建，不手編）——重述責任在 AGENTS.md 單側。
  **②已完成**：AGENTS.md 12 節補 `Normative authority:` 條號標頭（操作手冊保留、
  規則以法典為準）。**③已完成**：各域政策檔即單一登錄檔——
  `automation-flows.json`（排程間隔）、`sleep-policy.json`、`cleanup-caps-policy.json`、
  `tool-isolation-policy.json`、`self-learning.json`、`retention.json`、`native-engine.json`；
  AGENTS.md 可調數值節補 `Tunables single source:` 指標指向擁有檔，數值不再雙來源。

**C. 權責衝突（真衝突；待裁決或下次修正）**

- A548（星澄自主修復）vs A366/A380/A408（使用者開關＋一次性確認）字面牴觸：**全面責任檢討已完成**
  （證據 `convergence/a548-responsibility-review-20260922.json`：兩域已於實作分離——內部故障屬 A548 自主受治理鏈、
  使用者面更新屬 A366/A380/A408 開關＋確認、學習屬 A554；repair 使用者開關已依 2026-09-18 指令退役）。
  **clarification amendment 已 staged**（`codex-amendment-request-a548-domain-clarification-2026-09-22.json`，
  change_class=clarification，四條文附加 DOMAIN-SCOPE 澄清、不改既有特別法效力）；
  殘項僅餘 codex 實際寫入——待協調窗口經五主宰一致稽核門執行。

**E. 法典收斂與業務規則下放（2026-09-22 總督核定；下放 amendment 已發布 rev68／A602）**
...
- 細項：①以 runtime-rule-index 內容分類為底，產出「條文→治理／業務規則」對照清單
  **①②已產出**（`scripts/build-business-rule-delegation.py` v2 → `convergence/business_rule_delegation.{json,csv}`；
  537 條文：44 純治理／23 純業務／470 混合；混合＝下放業務內容、法典保留治理殼層；
  歸屬依 §10.68 單元制 section→unit 映射；**歸屬已全決（0 unassigned）**——
  憲制 meta 章節（general-provisions／sovereignty／amendment-and-protection／
  separation-and-accountability／interpretation-and-conflict／closed-security／
  anti-jailbreak）裁定 codex-retained 不下放；system-reliability→UNIT_RUNTIME；
  分佈：RUNTIME 181／retained 173／PLATFORM 47／PERMISSION 46／ENGINE 45／
  AUTOMATION 21／DATA 15／ASSISTANT 5／TOOLS 2／BOOTSTRAP 1／DECISION 1）
  ...
  **掃描器已建**（`scripts/codex-business-rule-scan.py` → `convergence/codex-business-rule-conflicts.json`；
  七軸：能力鍵碰撞／能力擁有者／路由身分／權威語句（manifest＋config）／模組指派覆蓋／
  deadline-budget 對法典 metadata／config-contract 權威語句；最新跑 **0 真衝突／5 重疊**：
  retired global-cleaner `system-health-check` 殘留宣告已清（E14）、ai-connections／investment-manager
  為共用域 per-tool 設定檔、`startup_deadline_ms` 為法典 `startup_complete_deadline_ms` 同值複本；
  corpus 系列為資料集 manifest（非工具）——掃描器已排除）
