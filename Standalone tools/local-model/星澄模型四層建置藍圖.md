...
- 待補：命令理解探針 ✅（`star-semantic-plan-probes/v1` 24/24，見收斂證據）；eval suite dialogue v1、人工抽檢紀錄；xl-v3 啟用決策；run-to-run 變異穩定化。
- 依賴：Phase 0–4。
...
| E6 | **Python 常駐外移**：E4（治理核心受 C 閘控）＋雙軌 retire＋整體驗收 `[處理中 @devin 2026-09-22T13:15Z — E1 執行面 connection_watchdog shadow 接線]` | 中高：資源／穩定性（常駐 Python） | E1–E3 已部分落地 |
...
| G48 | Contract 相容驗證（部分） `[處理中 @devin-cli/thread-7a6b 2026-09-22T13:07Z]` | 五版本軸（codex／permission／ipc／sql_schema／frontend／frontend-backend）全軸分離與破壞性變更測試 |
...
- **Phase 5 待補**：命令理解探針（`star-semantic-plan/v1` 對齊）✅ `star-semantic-plan-probes/v1`
  ...
  eval suite dialogue v1、人工抽檢紀錄仍待補 `[處理中 @devin-cli/cosmic-papaya 2026-09-22T13:45Z — v18 對話套件評估＋抽檢紀錄]`。
