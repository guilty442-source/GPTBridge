...
| G29 | 推論引擎元件仍為 Python | C++ 執行層已落地並接入治理路由（`src/backend/cpp/`：mmap loader＋SHA-256、BPE（含 HF `["l","r"]` merge 形）、Dense forward、KV cache、temperature/top-k/top-p/seed/repetition sampling、bounded JSON；張量運算經 C ABI）；`XINGCHENG_CPP_RUNTIME`＝`off/required/fallback`（required fail-closed、fallback 記帳本後回 Python）；KV 上限＋auto_release 註冊＋`cpp-runtime-execution` 帳本已接線；11 tests PASS。MoE／量化／prefix cache 重用／CUDA／batch>1 仍 fail-closed | 部分落地；P3f 剩 GPU 協調語義與長時驗證 |
...
| G45 | 效能優化第五批新項 | Y5/Y6/Y13/Y15 已落地（見 §4.11）；Y14 閉包記憶化待核對；Y1–Y4/Y7–Y12 為重複併入項 | 工作流與觀測負載 |
...
- 現況（2026-09-21 第一切片）：`src/backend/cpp/`（`xingcheng_inference.hpp`／`engine.cpp`／
  ...
  與 tool-call 解析已驗（`test_cpp_inference.py` 11 tests）；生產路由 `XINGCHENG_CPP_RUNTIME`
  ...
  bundle 以 checkpoint path＋size＋mtime 決定重用或 staging re-export。MoE、INT8/4、prefix cache
  重用、CUDA、batch>1 皆 fail-closed；GPU 協調對 C++ CPU-only 路徑暫不適用。
