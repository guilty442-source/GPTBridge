...
- **未完成**：device-resident KV（待 GPU attention kernel）＋生產啟用（`XINGCHENG_CPP_RUNTIME` 現為 `off`；batch>1 無生產呼叫者）＋C 核心 grouped GEMM（現為逐專家 gather／GEMM 迴圈）（§4.2 P3b–P3f）。
...
| 編號 | 位置 | 行動 | 預期改善 |
| --- | --- | --- | --- |
| W1 | C++ attention（`cpp/src/engine.cpp`；證據 `c-w1-qk-simd-evidence-20260922.json`） | **已落地**：Q×K SIMD＋scores 緩衝重用（`8c9020e4`）；**殘項**：FlashAttention 式分塊／線上注意力未建 | 延遲、配置次數 |
| R5 | MoE 稀疏與權重管理 | **已落地**：top-k 稀疏執行（C++ 引擎逐專家 gather／GEMM、Python `moe.py` 同語意）＋專家權重載入（P3f；`p1-r5-moe-sparse-inference-20260922.json`）；**殘項**：C 核心 grouped GEMM（現為逐專家 GEMM 迴圈） | 運算量 |
...
- **呼叫點遷移（2026-09-22 落地）**：`get_lane_pool()` 共享單例加入 `workload_lanes.py`；
  ...
  殘：各呼叫點逐步改用 `pool.execute(WorkloadClass.X, ...)` 一次性介面（讀多寫少場景）。
...
| --- | --- | --- | --- |
| 高效能 | 純 C 核心＋SIMD；Triton 自研 GPU kernels；C++ 推論層；批次 GEMM；GPU 獨立軌；KV／prefix cache；INT8／INT4；MoE 稀疏 | C 核心 grouped GEMM 未建（R5 殘）；C++ 推論層生產接線未完成（G29）；FlashAttention kernel 未編譯（已驗證 fallback） | P3（引擎）、Phase 9（MoE） |
...
1. C 核心 grouped GEMM（現為逐專家 gather／GEMM 迴圈）＋線上分塊注意力（W1 殘）`[處理中 @devin-cli 2026-09-22T17:07Z]`。
...
| P1 | **C++ 推論執行層**（P3／G29／G31／G32／G34） | 殘項＝長時驗證（窗口型）＋device-resident KV（待 GPU attention kernel）＋bf16 GEMM／自研 kernel（nvcc/MSVC-18 不相容待新工具鏈）＋生產啟用（`XINGCHENG_CPP_RUNTIME` 預設 `off`；batch>1 無生產呼叫者）＋C 核心 grouped GEMM＋線上分塊注意力；已閉＝INT8／INT4 量化、CUDA cuBLAS 橋接、device-resident 權重、KV INT8 cache、W1（QK SIMD／緩衝重用）、R5（top-k 稀疏）、R9（C++ `generate_batch`）、P3f GPU 協調閘，證據見 `audit/convergence/` 與 Git 歷史 |
...
| G50 | SQL／RAG 一致性與模型排程殘項 | workload lanes 直連已清零（殘＝`pool.execute` 一次性介面遷移）；parity audit 已落地；model resource：broker＋local-embedding＋ollama-embedding 載入點已接 `request_load`，Ollama 需求啟動已落地（殘＝reranker／CUDA 載入點、ProcessRegistry 登錄、開機自啟落差裁定）；transport 殘＝LISTEN／NOTIFY 消費端 |
