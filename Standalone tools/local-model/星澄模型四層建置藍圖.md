...
- 評估（依職能、生態、效能與整體淨占用）：
  ...
  | 核心 | 語言 | 理由 |
  | --- | --- | --- |
  | 決策核心 | **Python** | 法典／政策／裁決與 fail-closed 判定；與 `governance_rule`、codex 工具鏈同源；低頻決策非效能瓶頸 |
  | 權限核心 | **Python（目錄／身份）＋ C／C++（審計引擎）** | 權限目錄、身份、測試工具鏈（`permission_directory`）；**審計以 C／C++ 實作**（≤30 s、可嵌入熱路徑） |
  | 星澄助理 | **TypeScript（Electron UI）＋ Python（合規稽查）** | 助理面為 UI／狀態投影；法典對齊合規稽查沿用治理工具鏈 |
  | 運行核心 | **C**（ABI 邊界必要時 C++） | 事件迴圈／佇列／狀態機／資源監控／截止取消／IPC；高頻、低延遲；落地 `native/core` |
  | 自動化核心 | **C#** | 業務流程編排（Git／清理／發布／排程／按需啟動）；與 `business-logic-csharp` 定位一致 |
  | 執行模組 | **C／C++／C#** | 依模組性質：計算／核心 C（C++ 推論層）、業務服務 C#、UI TS |
  | 訓練（按需） | **Python（PyTorch）** | 僅訓練能力按需啟用、不常駐 |
