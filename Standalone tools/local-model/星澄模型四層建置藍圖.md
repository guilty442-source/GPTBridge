...
- 細項：①以 runtime-rule-index 內容分類為底，產出「條文→治理／業務規則」對照清單
  ...
  **掃描器已建**（`scripts/codex-business-rule-scan.py` → `convergence/codex-business-rule-conflicts.json`；
  ...
  deadline-budget 對法典 metadata／config-contract 權威語句；最新跑 0 真衝突／8 重疊：
  retired global-cleaner 殘留 `system-health-check` 宣告、corpus 系列未登錄
  module_assignment_registry、ai-connections／investment-manager 為共用域 per-tool 設定檔、
  `startup_deadline_ms` 為法典 `startup_complete_deadline_ms` 同值複本）
