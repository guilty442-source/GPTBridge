...
| R5 | MoE 稀疏與權重管理 | **已落地**：top-k 稀疏執行（C++ 引擎逐專家 gather／GEMM、Python `moe.py` 同語意）＋專家權重載入（P3f；`p1-r5-moe-sparse-inference-20260922.json`）；**殘項**：C 核心 grouped GEMM 已落地（`matmul_grouped`，§11.2 W2-1） | 運算量 |
...
| P2 | **效能殘項** | 細項：MS6（C++／C# 承接；C# 仍 consumer-only）＋MS10（SLO 儀表）＋W1／R5／R9 收斂殘項（線上分塊、C 核心 grouped GEMM、生產啟用） |
...
| G50 | SQL／RAG 一致性與模型排程殘項 | workload lanes 直連已清零，`pool.execute` 一次性介面已遷移單次唯讀站點；parity audit 已落地；model resource：broker＋local-embedding＋ollama-embedding 載入點已接 `request_load`，Ollama 需求啟動已落地（殘＝reranker／CUDA 載入點、ProcessRegistry 登錄、開機自啟落差裁定）；transport 殘＝LISTEN／NOTIFY 消費端 |
