...
| G2 | 【已解】eval suite 與訓練分佈落差大 | **評估結案（2026-09-21）**：落差是設計而非缺陷——eval suite 量測目標能力（對話探針）非訓練分佈匹配度；上線判準已由成熟度階梯結構性取代（L0–L7 連續閘門、僅實測定級、SFT 晉升需 eval 對照現役權重基線）——eval ppl 從不是單一判準，而是多閘門之一 | 已解決 |
