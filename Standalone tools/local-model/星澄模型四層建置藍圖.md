...
> fail-closed 保留；`retention.json` 新鍵 `keep_weight_versions`（預設 2＝active＋上一代）。
...
| E2 | **Git 收斂**：推送 `main`（領先 origin 176 commits）＋單一寫者＋ahead==0 週期驗收 `[處理中 @devin 2026-09-22T12:43Z]` | 高：未推送＝歷史遺失風險（單機故障） | 稽核／測試 PASS；協調者推送 |
...
| E13 | **自主訓練殘項**（DPO/蒸餾、停機、rollback 裁定、3 循環、法典 amendment） | 中低：能力進化 | **rollback 語意已裁定（2026-09-22 總督）：prune-latest 優先**——只留最新一代；rollback 目標＝已認證且相容之任一留存版本，無留存版本時 rollback 閘 fail-closed |
