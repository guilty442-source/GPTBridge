...
`degradation_probe_enabled`／`degradation_probe_suite`／`degradation_min_examples`；推論互斥
偵測同行程引擎快取，跨行程由 executor GPU 閘門兜底；閘門測試 17/17＋既有迴歸 28/28；
...
| P1 | **星澄自主訓練**（§2.7） | 基礎管線已實作（star-self-learning/v1）＋閘門補齊（最低間隔／每日預算／連續失敗熔斷／推論互斥／品質漂移 `min_pool_avg_quality`／合成比例 `max_synthetic_ratio`＋前綴白名單；政策檔預設關閉、受管設定檔開啟）＋SFT 課程選擇（依 `model-maturity.json` 未達層映射課程（L4→dialogue／L5→reasoning／L6→evolution／L≥7→refresh）、L<4 fail-closed、`curriculum_intent_map` 篩池空即阻斷）＋能力退化探針觸發（below-threshold 時以套件 baseline 量測現役權重，退化且新例足量即降門檻觸發；失敗不觸發並記錄）＋升級後 maturity 重測與 `resource_account` 資源帳；證據 `self-learning-curriculum-gates-20260922.json`（閘門測試 17/17＋迴歸 28/28）；殘項：③DPO／蒸餾等課程型別⑧資料上限／資源超支停機⑦rollback 實測（保留上一代語意與現行 prune-latest 衝突，待裁決）⑩連續 3 循環自主驗收⑥法典 amendment（自主訓練權限邊界） |
