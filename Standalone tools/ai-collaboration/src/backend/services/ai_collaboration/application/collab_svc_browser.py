...
class CollabSvcBrowserMixin:
    ...
    async def _run_agent_message(
        ...
        *,
        ...
        try:
            result, task = await self._dispatch_agent_message(
                agent,
                content,
                business_task,
                business_scope,
    ...
    async def _dispatch_agent_message(
        ...
        if callable(send_task):
            ...
            return await send_task(agent, task), task
        return await self.session.send_prompt(agent, content), {}
    ...
    async def _wait_for_browser_or_fallback(
        ...
        try:
            for attempt in range(1, self.BROWSER_WAIT_CYCLES + 1):
                self._browser_wait_update(message_id, agent_id, attempt)
                try:
                    ...
                    continue
                return self._browser_completed_result(
                    task, agent_id, browser_content, attempt
                )
    ...
    def _browser_wait_update(
        self, message_id: str, agent_id: str, attempt: int
    ) -> None:
        self.repository.update_response(
            message_id,
            agent_id,
    ...
    async def _complete_browser_response(
        ...
        if future is not None and not future.done():
            ...
            message = self._store_browser_result(message_id, agent_id, content)
            if message is None:
    ...
    def _store_browser_result(
        self, message_id: str, agent_id: str, content: str
    ) -> str | None:
