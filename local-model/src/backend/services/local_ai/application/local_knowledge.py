...
class LocalKnowledgeService:
    ...
    def pool_manager(self) -> Any:
        if self._pool_manager is None:
            from ..infrastructure.postgres_pool import get_pool_manager
    ...
    def cognition(self) -> Any:
        if self._cognition is None:
            from ..infrastructure.postgres_cognition_repository import (
                PostgresCognitionRepository,
            )

            self._cognition = PostgresCognitionRepository(
    ...
    def identity(self) -> Any:
        if self._identity is None:
            from ..infrastructure.postgres_identity_repository import (
                PostgresIdentityRepository,
            )

            self._identity = PostgresIdentityRepository(
                self.tool_root, self.pool_manager
            )
    ...
    async def sql_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "message": "PostgreSQL cognition/identity 未設定。",
                "engine": "postgresql",
                "pooled": False,
            }
