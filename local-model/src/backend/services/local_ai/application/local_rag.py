...
from ..infrastructure.postgres_rag_repository import PostgresRagRepository
...
class LocalRagService:
    ...
    def __init__(
        ...
        *,
        ...
        repository: PostgresRagRepository | None = None,
    ) -> None:
        ...
        self.repository = repository or PostgresRagRepository(self.tool_root)
        qdrant_root = self.tool_root / "runtime" / "qdrant"
        self.qdrant = qdrant_store or QdrantStore(
            endpoint=os.environ.get("GPTBRIDGE_QDRANT_ENDPOINT", QdrantStore.DEFAULT_ENDPOINT),
            executable_path=str(qdrant_root / "bin" / "qdrant.exe"),
            working_directory=str(qdrant_root),
        )
        if qdrant_store is None:
            self.qdrant.ensure_running()
    ...
    def _dependency_error(self, exc: Exception, *, indexed: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        message = str(exc)
        qdrant_error = "QDRANT" in message.upper() or "6333" in message
        return {
            "ok": False,
            "error_code": "RAG_QDRANT_UNAVAILABLE" if qdrant_error else "RAG_EMBEDDING_UNAVAILABLE",
            "message": f"共享知識庫相依服務不可用：{message}",
            ...
            "qdrant_endpoint": self.qdrant.endpoint,
    ...
    def ingest(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "errors": errors,
            "embedding_model": embedding_model,
            "retrieval": "qdrant-dense+postgresql-fts",
    ...
    def query(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            vector_results = self.qdrant.query(
        ...
        return {
            ...
            "embedding_model": str(self.transformer_runtime.EMBEDDING_MODEL),
            "retrieval": "qdrant-dense+postgresql-fts+rrf+qwen3-reranker",
    ...
    def status(self) -> dict[str, Any]:
        return {
            ...
            "available_to_all_local_models": True,
            "embedding_model": str(self.transformer_runtime.EMBEDDING_MODEL),
            "vector_database": {"engine": "qdrant", **self.qdrant.status()},
            "keyword_index": {"engine": "postgresql-fts", **self.repository.status()},
