"""Packager processes — backend shutdown helpers.
...
from packager_processes_shutdown_helpers import (
...
def stop_verified_packaged_backend(tool_id: str, live_app_dir: Path) -> bool:
    ...
    descriptor_port = _resolve_descriptor_port(live_app_dir)
    owner_candidates = _collect_owner_candidates(
        tool_id, expected_root_text, expected_instance, descriptor_port,
    )
