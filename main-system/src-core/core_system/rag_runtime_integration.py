...
class _PostgresContentResolver:
    """'document' resolver — PostgreSQL is the content authority.

    ``locator_id`` resolves through ``gptbridge_index.resource``;
    content comes from ``chunk.metadata->>'content'`` (Qdrant payloads
    never carry it).  Tombstoned or unknown locators fail closed.  The
    async authority runs on the RAG worker loop; calling ``resolve``
    from that loop's own thread is refused instead of deadlocked.
    """

    resource_type = "document"

    def __init__(self, pipeline_getter: Callable, worker: Any) -> None:
    ...
    def resolve(
        ...
        if pipeline is None or worker is None:
            raise PermissionError("canonical content store unavailable")
        if worker._thread is not None and (
            threading.current_thread() is worker._thread
        ):
            raise PermissionError(
                "content resolve cannot block the RAG worker loop"
            )
        postgresql = pipeline.postgresql

        async def _fetch() -> Any:
            row = await postgresql.fetch_resource_by_locator(
                module_id, locator_id
            )
            if row is None:
                raise PermissionError(
                    f"unknown document locator: {locator_id}"
                )
            resource_id = str(row["resource_id"])
            if await postgresql.is_tombstoned(module_id, resource_id):
                raise PermissionError(
                    f"document tombstoned: {locator_id}"
                )
            chunks = await postgresql.fetch_resource_chunks(
            ...
            return ResolvedContent(
                ...
                resolver="postgres-canonical",
            )

        return worker.run(_fetch(), timeout=30)
