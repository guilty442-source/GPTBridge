...
class MaintenanceLearningMixin:
    ...
    def _ensure_learning_store(self) -> None:
        ...
        if self._learning_store is not None:
            return
        # A604: 星澄 owns its learner/store internally — maintenance keeps
        # its own learner bound to the same shared SQLite path instead of
        # reaching into another entity's state.
        try:
