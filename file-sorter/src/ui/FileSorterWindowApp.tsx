...
type CleanupFile = {
  ...
  width?: number
  height?: number
  visual_recognition_confidence?: number
  visual_recognition_model?: string
  visual_recognition_model_version?: string
}

type CleanupReport = {
  scan_folder_count?: number
  source_file_count?: number
  found_files?: CleanupFile[]
  found_file_count?: number
  person_detected_image_count?: number
  non_person_image_count?: number
  indeterminate_image_count?: number
  large_video_file_count?: number
  bad_video_file_count?: number
  similar_video_duplicate_count?: number
  similar_image_duplicate_count?: number
  warnings?: string[]
}
...
type SorterProfile = {
  ...
  path?: string
  enabled?: boolean
  duplicate_trash_enabled?: boolean
...
function parseProfileState(
  ...
  const parsed = parseJsonWithPrefixes<
    | SorterProfile[]
    | SorterProfile
    | { profile?: SorterProfile; profiles?: SorterProfile[] }
...
function cleanupFileSummary(file: CleanupFile): string {
  ...
  if (typeof file.visual_recognition_confidence === 'number') {
    parts.push(`模型信心 ${Math.round(file.visual_recognition_confidence * 100)}%`)
  }
...
const LAST_TARGET_DIR_STORAGE_KEY = 'gptbridge.file-sorter.last-target-dir.v1'

function loadLastTargetDir(): string {
  try {
    return window.localStorage.getItem(LAST_TARGET_DIR_STORAGE_KEY)?.trim() || ''
  } catch {
    return ''
  }
}
...
export function FileSorterWindowApp() {
  ...
  const [duplicateTrashEnabled, setDuplicateTrashEnabled] = useState(false)
  const [duplicateTrashStatus, setDuplicateTrashStatus] = useState('')
  ...
  const [cleanupSimilarImages, setCleanupSimilarImages] = useState(false)
  const [cleanupVideoIssues, setCleanupVideoIssues] = useState(true)
  const [cleanupSimilarVideos, setCleanupSimilarVideos] = useState(false)
  ...
  const [modelTopP, setModelTopP] = useState(0.9)
  const [modelContextWindow, setModelContextWindow] = useState(8192)
  const [modelMaxTokens, setModelMaxTokens] = useState(512)
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try {
        ...
        // Storage may be unavailable; folder selection still works for this session.
      }
    }, 300)
      ...
      void requestToolRun([target, SORTER_FLAGS.profiles], {
        ...
        .then((result) => {
          ...
          if (!profile || typeof profile.enabled !== 'boolean') {
            ...
            setAutoOrganizeFiles(false)
            setDuplicateTrashEnabled(false)
            setAutoOrganizeStatus('此資料夾尚未啟用背景自動分類')
            setDuplicateTrashStatus('完全重複檔自動回收尚未啟用')
            return
          }
          const requiresReview = profile.migration_required_review === true
          setProfileRequiresMigrationReview(requiresReview)
          setAutoOrganizeFiles(profile.enabled === true && !requiresReview)
          setDuplicateTrashEnabled(profile.duplicate_trash_enabled === true)
          setDuplicateTrashStatus(
            profile.duplicate_trash_enabled === true
              ? '完全重複檔自動回收已啟用'
              : '完全重複檔自動回收已關閉'
          )
          if (requiresReview) {
            ...
            setAutoOrganizeStatus('背景自動分類已啟用；即使關閉此工具視窗仍會持續監看')
          }
        })
        .catch(() => {
  ...
  const changeDuplicateTrashEnabled = (enabled: boolean) => {
    const target = targetDir.trim()
    if (!target || !backendConnected) {
      setDuplicateTrashStatus(
        target ? '後端連線後才能變更重複檔回收設定' : '請先選擇目標資料夾'
      )
      return
    }
    if (
    ...
    setDuplicateTrashStatus(enabled ? '正在啟用完全重複檔自動回收...' : '正在關閉完全重複檔自動回收...')
    void requestToolRun(
      [target, SORTER_FLAGS.setDuplicateTrashEnabled, String(enabled)],
      { mode: 'mutation', timeoutMs: SHORT_REQUEST_TIMEOUT_MS }
    )
      .then((result) => {
        if (result.ok !== true) {
          setDuplicateTrashStatus(result.message || '重複檔回收設定未變更')
          return
        }
        setDuplicateTrashEnabled(enabled)
        setDuplicateTrashStatus(
          enabled ? '完全重複檔自動回收已啟用' : '完全重複檔自動回收已關閉'
        )
      })
      .catch((error) => {
        setDuplicateTrashStatus(
          error instanceof Error ? error.message : '重複檔回收設定未變更'
        )
      })
  }
  ...
  const buildCleanupArgs = (): string[] => {
    const args = [targetDir.trim(), '--cleanup-scan', '--json']
    if (cleanupImageIssues) args.push('--image-cleanup')
    if (cleanupSimilarImages) args.push('--similar-image-analysis')
    ...
    args.push('--model-temperature', String(modelTemperature))
    args.push('--model-top-p', String(modelTopP))
    args.push('--model-context-window', String(modelContextWindow))
    args.push('--model-max-output-tokens', String(modelMaxTokens))
  ...
  const runCleanup = () => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (result.cancelled || cleanupStopRequestedRef.current) {
          setCleanupState('idle')
        ...
        if (!report) {
          setCleanupState('error')
          setCleanupMessage('清理掃描回傳格式無法辨識')
          setCleanupOutput(formatRunOutput(result))
          return
        }
        ...
        setCleanupProgress({
          ...
          message: '清理掃描完成',
          folder_current: Number(report.scan_folder_count || 0),
          folder_total: Number(report.scan_folder_count || 0),
          source_file_count: Number(report.source_file_count || 0),
          found_file_count: files.length,
        })
    ...
    <main style={styles.app}>
      <section style={styles.card}>
        <header style={styles.header}>
          <div>
            <div style={styles.kicker}>SMART FILE WORKSPACE</div>
            <h1 style={styles.title}>自動化檔案管理</h1>
            <p style={styles.muted}>選擇一個資料夾，剩下的整理交給系統。</p>
          </div>
          <span style={{ ...styles.badge, ...(backendConnected ? styles.badgeOnline : {}) }}>
            <span style={styles.badgeDot} />
            {backendConnected ? '系統已連線' : '正在連線'}
          </span>
        </header>

        <section style={styles.workspaceHero}>
          <div style={styles.workspaceCopy}>
            <span style={styles.eyebrow}>目前工作區</span>
            <strong style={styles.workspaceTitle}>
              {targetDir.trim() ? '資料夾已納入管理' : '從選擇資料夾開始'}
            </strong>
            <span style={styles.workspacePath}>
              {targetDir.trim() || '尚未選擇要整理的位置'}
            </span>
          </div>
          <button type="button" disabled={actionBusy} onClick={chooseTarget} style={styles.primaryButton}>
            {targetDir.trim() ? '更換資料夾' : '選擇資料夾'}
          </button>
        </section>

        <div style={styles.heroStats}>
          <div style={styles.statCard}><span>自動分類</span><strong>內建流程</strong></div>
          <div style={styles.statCard}><span>分類規則</span><strong>{keywords.length || '—'} 個待編輯關鍵字</strong></div>
          <div style={styles.statCard}><span>重複檔處理</span><strong>{duplicateTrashEnabled ? '移至回收筒' : '只偵測、不移除'}</strong></div>
        </div>

        <nav style={styles.navBar} aria-label="工具區段">
          {[
            ['automation', '工作區'],
            ['manual-sort', '手動整理'],
            ['rules', '分類規則'],
        ...
        <section id="automation" style={styles.notice}>
          <div style={styles.sectionHeading}>
            <div>
              <span style={styles.eyebrow}>AUTOMATION</span>
              <strong style={styles.sectionTitle}>工作區與自動分類</strong>
            </div>
            <span style={styles.alwaysOnBadge}>自動運作</span>
          </div>
          <p style={styles.noticeText}>選定工作區後，自動分類會直接啟用並持續監看新檔案，不需要額外設定開關。</p>
          ...
          <div style={styles.automationStatus}>
            <span style={{ ...styles.statusDot, background: targetDir.trim() ? '#34d399' : '#64748b' }} />
            <div>
              <strong>{targetDir.trim() ? '自動分類已納入流程' : '等待選擇工作區'}</strong>
              <p style={styles.noticeText}>檔案穩定寫入後，系統會依規則自動歸檔。</p>
            </div>
          </div>
          <label style={{ ...styles.checkboxRow, marginTop: 12 }}>
            <input
              type="checkbox"
              checked={duplicateTrashEnabled}
              disabled={!canChangeAutoClassification}
              onChange={(event) => changeDuplicateTrashEnabled(event.target.checked)}
            />
          ...
          {duplicateTrashStatus ? (
            <p style={styles.noticeText}>{duplicateTrashStatus}</p>
          ) : null}
        </section>

        <section id="manual-sort" style={styles.notice}>
        ...
        <section id="rules" style={styles.notice}>
        ...
        <section id="rule-editor" style={styles.notice}>
        ...
        <section id="smart-scan" style={styles.notice}>
          <strong>清理掃描</strong>
          <p style={styles.noticeText}>
            原始檔案分類不會啟動模型。只有勾選視覺辨識或相似度分析時才載入本機 MiniCPM-V 4.6，完成後立即卸載。
          </p>
          <div style={{ ...styles.fieldGroup, marginTop: 12 }}>
            ...
            <label style={{ ...styles.checkboxRow, marginTop: 8 }}>
              <input
                type="checkbox"
                checked={cleanupSimilarImages}
                onChange={(event) => setCleanupSimilarImages(event.target.checked)}
              />
              MiniCPM-V 相似圖片分析
            </label>
            <label style={{ ...styles.checkboxRow, marginTop: 8 }}>
          ...
          <div style={{ ...styles.resultPanel, marginTop: 14 }}>
            <strong>MiniCPM-V 模型參數</strong>
            <p style={styles.noticeText}>僅在啟用辨識功能並執行掃描時載入，完成後立即卸載。</p>
            <div style={{ ...styles.sliderGrid, marginTop: 10 }}>
              <label style={styles.sliderControl}>
                <span style={styles.sliderHeader}><span>Temperature</span><strong>{modelTemperature.toFixed(2)}</strong></span>
                <input type="range" min={0} max={1} step={0.05} value={modelTemperature} onChange={(event) => setModelTemperature(Number(event.target.value))} style={styles.rangeInput} />
              </label>
              <label style={styles.sliderControl}>
                <span style={styles.sliderHeader}><span>Top P</span><strong>{modelTopP.toFixed(2)}</strong></span>
                <input type="range" min={0.1} max={1} step={0.05} value={modelTopP} onChange={(event) => setModelTopP(Number(event.target.value))} style={styles.rangeInput} />
              </label>
              <label style={styles.sliderControl}>
                <span style={styles.sliderHeader}><span>Context Window</span><strong>{modelContextWindow}</strong></span>
                <input type="range" min={2048} max={16384} step={1024} value={modelContextWindow} onChange={(event) => setModelContextWindow(Number(event.target.value))} style={styles.rangeInput} />
              </label>
              <label style={styles.sliderControl}>
                <span style={styles.sliderHeader}><span>最大輸出 Tokens</span><strong>{modelMaxTokens}</strong></span>
                <input type="range" min={128} max={1024} step={64} value={modelMaxTokens} onChange={(event) => setModelMaxTokens(Number(event.target.value))} style={styles.rangeInput} />
              </label>
            </div>
          </div>
          <div style={styles.actions}>
