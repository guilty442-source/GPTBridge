...
if str(SRC_CORE_DIR) not in sys.path:
    sys.path.insert(0, str(SRC_CORE_DIR))

from tasks.package_integrity import (  # noqa: E402
...
TOOL_RUNTIME_CONTRACT_PATH = PROJECT_ROOT / "config" / "tool-runtime-contract.json"
...
UPGRADE_AUTO_REPAIR_ANOMALIES = frozenset(
    {
...
def _load_project_cleanup_service() -> type:
    module_name = "_gptbridge_upgrade_cleanup_engine"
    loaded = sys.modules.get(module_name)
    if loaded is not None:
        service = getattr(loaded, "ProjectCleanupService", None)
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError("Project Cleaner repair engine cannot be loaded")
    ...
    try:
        ...
        raise
    service = getattr(module, "ProjectCleanupService", None)
    if not isinstance(service, type):
        raise RuntimeError("Project Cleaner repair service is unavailable")
    return service
...
def run_upgrade_auto_repair(
    ...
    *,
    ...
    """Repair recoverable project anomalies before or after package promotion."""

    try:
        service_type = service_factory or _load_project_cleanup_service()
        service = service_type(project_root)
        preflight = service.repair_anomalies(
            dry_run=True,
            include_shared=True,
            selected_anomalies=set(UPGRADE_AUTO_REPAIR_ANOMALIES),
        )
        if not preflight.get("ok"):
            return {
                "ok": False,
                "error_code": "UPGRADE_REPAIR_SCAN_FAILED",
                "message": str(
                    preflight.get("message")
                    or "Upgrade anomaly scan failed"
                ),
                "preflight": preflight,
            }

        repairable_count = int(preflight.get("repairable_count") or 0)
        if repairable_count > 0:
            repair = service.repair_anomalies(
                dry_run=False,
                include_shared=True,
                selected_anomalies=set(UPGRADE_AUTO_REPAIR_ANOMALIES),
            )
            ...
            repair = {
                ...
                "skipped": [],
                "errors": [],
                "message": "No recoverable upgrade anomalies found",
            }
        verification = service.repair_anomalies(
            dry_run=True,
            include_shared=True,
            selected_anomalies=set(UPGRADE_AUTO_REPAIR_ANOMALIES),
        )
        ...
        return {
            "ok": False,
            "error_code": "UPGRADE_REPAIR_UNAVAILABLE",
            "message": f"Upgrade auto-repair is unavailable: {error}",
        }

    unresolved_count = int(verification.get("repairable_count") or 0)
    warning_count = len(verification.get("diagnostics") or [])
    ok = (
        bool(repair.get("ok"))
    ...
    return {
        "ok": ok,
        "error_code": "" if ok else "UPGRADE_REPAIR_INCOMPLETE",
        "preflight_anomaly_count": int(
            preflight.get("anomaly_count") or 0
        ),
        "preflight_repairable_count": repairable_count,
        "repaired_count": int(repair.get("repaired_count") or 0),
        "repaired_bytes": int(repair.get("repaired_bytes") or 0),
        ...
        "message": (
            "Upgrade auto-repair completed"
            if ok
            else "Upgrade stopped because recoverable anomalies remain"
        ),
    }
...
def copy_backend_source_bundle(
    ...
    source_project_root = tool_dir.resolve().parent.parent
...
def package_source_roots(tool_dir: Path, entry: Path) -> list[str]:
    ...
    candidates = [
        ...
        tool_dir / "README.md",
        runtime_root,
        PROJECT_ROOT / "config" / "tool-runtime-contract.json",
    ]
    roots: list[str] = []
...
def package_source_excluded_paths(tool_dir: Path, entry: Path) -> list[str]:
    ...
    source_project_root = tool_root.parent.parent
...
def verify_tool_package(
    ...
    expected_backend_entry = "src-core/main.py"
...
def _package_tool_locked(
    ...
    try:
        ...
        validate_staged_python_runtime(app_dir, tool_id)
...
def main() -> int:
    ...
    if not args.verify and not args.build_renderers_only:
        auto_repair = run_upgrade_auto_repair()
        if not auto_repair.get("ok"):
            payload = {
                "ok": False,
            ...
            if args.as_json:
                ...
                print(f"[FAIL] upgrade-auto-repair: {auto_repair['message']}")
            return 1
    if args.changed:
        tools = [
            item
    ...
    if args.verify:
        ...
        post_repair = run_upgrade_auto_repair()
        if post_repair.get("ok"):
            results = [
