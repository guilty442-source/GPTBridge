...
def synthetic_fixture() -> dict[str, Any]:
    ...
    return {
        "state_revision": "visual-fixture-v4",
        "state": state,
        "diagnostics": {
            "state": "ready",
            "state_label": "可用",
...
() => {
  ...
  if (tabs.length !== 7) {
    issues.push({ code: "tab_count", target: '[role="tab"]', detail: `Expected 7 visible tabs, found ${tabs.length}` });
  }
  if (selectedTabs.length !== 1) {
    issues.push({ code: "tab_selection", target: '[role="tab"]', detail: `Expected one selected tab, found ${selectedTabs.length}` });
  }
  selectedTabs.forEach((element) => {
    const panelId = element.getAttribute("aria-controls");
    if (!panelId || !document.getElementById(panelId)) {
      issues.push({ code: "tab_controls", target: nodeName(element), detail: "Selected tab does not control an existing panel" });
    }
  });
...
def capture_workbench(page: Page, output: Path, name: str) -> None:
    workbench = page.locator(".investment-v2")
    if workbench.count() != 1:
        raise AssertionError("Expected exactly one investment workbench")
    workbench.screenshot(path=str(output / name))


def exercise_tabs(page: Page, output: Path, *, capture: bool) -> dict[str, Any]:
    results: dict[str, Any] = {}
    for label in TAB_LABELS:
        locator = page.get_by_role("tab", name=label, exact=True)
        count = locator.count()
        if count != 1:
            raise AssertionError(f"Expected one {label} tab, found {count}")
        locator.click()
        page.wait_for_function(
            "(label) => document.querySelector('[role=tab][aria-selected=true]')?.textContent?.trim() === label",
            arg=label,
        )
        panel_count = page.locator('[role="tabpanel"]').count()
        panel_visible = (
            panel_count == 1
            and page.locator('[role="tabpanel"]').is_visible()
        )
        results[label] = {
            "selected": True,
            "tabpanel_count": panel_count,
            "tabpanel_visible": panel_visible,
        }
        if capture and label in {"今日總覽", "配置模擬", "自動化與備援", "決策與安全"}:
            capture_workbench(
                page,
                output,
                f"desktop-{TAB_LABELS.index(label) + 1}-{label}.png",
            )
    return results


def keyboard_tab_audit(page: Page) -> dict[str, str]:
    overview = page.get_by_role("tab", name="今日總覽", exact=True)
    if overview.count() != 1:
        raise AssertionError("Overview tab is not unique")
    overview.click()
    overview.focus()
    overview.press("ArrowRight")
    selected_after_right = page.locator(
        '[role="tab"][aria-selected="true"]'
    ).inner_text()
    page.locator('[role="tab"][aria-selected="true"]').press("End")
    selected_after_end = page.locator(
        '[role="tab"][aria-selected="true"]'
    ).inner_text()
    page.locator('[role="tab"][aria-selected="true"]').press("Home")
    selected_after_home = page.locator(
        '[role="tab"][aria-selected="true"]'
    ).inner_text()
    return {
        "after_arrow_right": selected_after_right.strip(),
        "after_end": selected_after_end.strip(),
        "after_home": selected_after_home.strip(),
    }
...
def run_page(
    ...
    *,
    ...
    if mobile:
        operations = page.get_by_role("tab", name="自動化與備援", exact=True)
        if operations.count() != 1:
            raise AssertionError("Operations tab is not unique on mobile")
        operations.click()
        capture_workbench(page, output, "mobile-operations.png")
        tab_results = {
            "自動化與備援": {
                "selected": True,
                "tabpanel_count": page.locator('[role="tabpanel"]').count(),
                "tabpanel_visible": page.locator(
                    '[role="tabpanel"]'
                ).is_visible(),
            }
        }
        keyboard_results: dict[str, str] = {}
    else:
        tab_results = exercise_tabs(page, output, capture=True)
        keyboard_results = keyboard_tab_audit(page)
        capture_workbench(page, output, "desktop-keyboard-home.png")
...
def main() -> int:
    ...
    for surface in ("desktop", "mobile"):
        ...
        for label, tab in (result.get("tabs") or {}).items():
            if not tab.get("selected") or not tab.get("tabpanel_visible"):
                failures.append(f"{surface}.tab.{label}")
    keyboard = (report.get("desktop") or {}).get("keyboard") or {}
    if keyboard and keyboard != {
        "after_arrow_right": "交易帳本",
        "after_end": "決策與安全",
        "after_home": "今日總覽",
    }:
        failures.append(f"desktop.keyboard: {keyboard}")
