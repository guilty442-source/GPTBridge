...
if (app.isPackaged || !app.commandLine.hasSwitch('user-data-dir')) {
  ...
  const toolUserDataPath = path.join(
    appDataRoot,
    'GPTBridge',
    'standalone',
    stableToolId,
...
function verifyPackagedPayload() {
  ...
  if (!app.isPackaged && !fs.existsSync(packageMetadataPath)) {
    packageVerification = {
      ok: true,
      development: true,
      packageDigest: 'development',
      protocolVersion: 1,
      backendVersion: String(manifest.version || ''),
    }
...
function backendHasCurrentTool(
  ...
  const serviceAliases = {
    'ai-assistant': 'ai_nexus',
    'ai-collaboration': 'ai_collaboration',
    vaultly: 'vaultly',
  }
  const serviceId = serviceAliases[toolId]
...
function backendReloaderActive(projectRoot, maxAgeMs = 5000) {
  const heartbeat = readJson(
    path.join(projectRoot, 'runtime', 'backend_hot_reload.json')
  )
  const heartbeatAt = Number(heartbeat.heartbeat_at_ms || 0)
  return (
    heartbeatAt > 0 &&
    Date.now() - heartbeatAt <= maxAgeMs &&
    Number(heartbeat.supervisor_pid || 0) > 0
  )
}
...
async function ensureBackendStarted() {
  ...
  const developmentHotReloadEnabled =
    !app.isPackaged &&
    process.env.GPTBRIDGE_BACKEND_HOT_RELOAD === '1'
  if (
    !portAlreadyInUse &&
    developmentHotReloadEnabled &&
    backendReloaderActive(projectRoot)
  ) {
    ...
    if (backendReloaderActive(projectRoot)) {
      return { ok: false, message: 'Backend hot reload timeout' }
    }
  }

  const backendWatcher = path.join(projectRoot, 'scripts', 'backend_hot_reload.py')
  if (!backendProcess || backendProcess.exitCode !== null) {
    ...
    const hotReloadEnabled =
      developmentHotReloadEnabled &&
      fs.existsSync(backendWatcher)
    const args = hotReloadEnabled
      ? [backendWatcher]
      : [backendEntry, '--serve']
    ...
    const backendEnvironment = {
      ...
      GPTBRIDGE_AI_ASSISTANT_DATA_ROOT: path.join(
        projectRoot,
        'runtime',
        'data',
        'ai-assistant'
      ),
      FILE_SORTER_STATE_ROOT: path.join(
        projectRoot,
        'runtime',
        'file-sorter-state'
      ),
