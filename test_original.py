...
# Use the exact same training data as the working test
additional_data = [
...
print('Chars:', [hex(ord(c)) for c in result['text']])

result = model.generate(intent='analysis', prompt='請分析這段文字', grounding='', max_tokens=50)
print('Analysis:', repr(result['text']))
print('Chars:', [hex(ord(c)) for c in result['text']])