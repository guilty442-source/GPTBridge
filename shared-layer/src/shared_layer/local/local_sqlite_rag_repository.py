...
def _resource_params(
    document: dict[str, Any], resource_id: str, metadata: dict[str, Any]
) -> tuple:
    """INSERT params for gptbridge_index_resource."""
    return (
        resource_id,
...
def _chunk_params(
    chunk: dict[str, Any], document: dict[str, Any], resource_id: str
) -> tuple:
    """INSERT params for gptbridge_rag_chunk."""
...
class LocalSqliteRagRepository:
    ...
    def replace_document(
        ...
        *,
        ...
        with self._connect() as connection:
            connection.execute(
                _RESOURCE_UPSERT_SQL,
                _resource_params(document, resource_id, metadata),
            )
            connection.execute(
    ...
    def keyword_search(
        ...
        for row in rows:
            record = self._keyword_row(query, row)
