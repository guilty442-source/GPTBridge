...
GOVERNANCE_CODEX_CHINESE: Final[ChineseCodexReference] = ChineseCodexReference(
    ...
    principles=(
        ...
        ChineseCodexPrinciple(id="P7", statement="Git、PostgreSQL、Qdrant、RAG、LLM 為正式工具，責任分離，彼此不得互相取代。", binding=False),
        ...
        ChineseCodexPrinciple(id="P16", statement="系統程式碼一律採用 Python 與 C++ 之混合架構；PostgreSQL、Qdrant、Git、RAG 為受治理之正式工具，任何程式碼不得以其他程式語言撰寫。", binding=False),
        ChineseCodexPrinciple(id="P17", statement="中文版法典僅作為備用參考，不具被引用為判定依據之地位；一切判定以法典正式權威本為唯一依據。", binding=False),
        ChineseCodexPrinciple(id="P18", statement="系統主宰及一切系統模組僅使用本機自有程式碼；PostgreSQL、Qdrant、Git 與 RAG 為受治理之正式工具，其餘第三方軟體、套件或外部服務不得引用或執行。", binding=False),
        ChineseCodexPrinciple(id="P19", statement="法典一律以程式語言撰寫；中文內容統一歸於中文法典，僅作備用參考，不具被引用判定依據。", binding=False),
        ChineseCodexPrinciple(id="P20", statement="使用者介面層僅為決策與狀態之呈現媒介，不具決策權或執行權；其技術棧亦僅限 Python 與 C++；本法典之效力含所有模組與一切執行。", binding=False),
        ...
        ChineseCodexPrinciple(id="P23", statement="PostgreSQL、Qdrant、Git、RAG、Python、C++ 為本法典指定之正式工具，悉數受本法典治理，禁止以非正式工具替代。", binding=False),
    ),
    articles=(
        ...
        ChineseCodexArticle(id="A8", section="四", subject="system-responsibility", rule="Git 保存系統版本與歷史，PostgreSQL 保存結構化正式資料，Qdrant 保存語意索引，RAG 負責檢索增強，LLM 負責理解推理與操作；PostgreSQL、Qdrant、Git、RAG、Python、C++ 為正式工具。", prohibition="四者不得互相取代；禁止以非正式工具替代"),
        ...
        ChineseCodexArticle(id="A35", section="四", subject="architecture-hybrid", rule="系統程式碼一律採用 Python 與 C++ 之混合架構：Python 負責上層編排與受治理邏輯，C++ 負責核心運算與原生效能，二者協作構成唯一允許之程式碼架構。", prohibition="禁止以 Python 與 C++ 以外之任何程式語言撰寫系統程式碼，亦禁止以其他架構取代混合架構"),
        ...
        ChineseCodexArticle(id="A37", section="四", subject="code-origin", rule="系統主宰及一切系統模組僅使用本機自有程式碼：Python 與 C++ 混合架構為唯一允許之程式語言；PostgreSQL、Qdrant、Git、RAG 為受治理之正式工具，其餘第三方軟體、套件、外部服務或非本機自有之執行環境不得依賴。", prohibition="禁止引用、安裝、匯入或執行任何非正式之第三方套件、外部服務、非本機自有之執行環境或二進位檔案"),
        ...
        ChineseCodexArticle(id="A44", section="四", subject="four-functions-local", rule="Git、PostgreSQL、語意索引、RAG 與 LLM 為正式工具與功能，由本機 Python 與 C++ 程式碼治理；若無法本機治理，則宣告不可用並保持關閉，不得以外部服務替代。", prohibition="禁止以非正式工具、外部服務或二進位替代任一本機功能"),
        ChineseCodexArticle(id="A45", section="四", subject="interface-layer", rule="使用者介面層僅為決策與狀態之呈現媒介，不具決策權與執行權；其技術棧同受 Python 與 C++ 混合架構規範；本法典效力含所有模組與一切執行。", prohibition="禁止介面層行使決策或執行，亦禁止以其他程式語言撰寫介面"),
        ...
        ChineseCodexArticle(id="A49", section="四", subject="formal-tools", rule="PostgreSQL、Qdrant、Git、RAG、Python、C++ 為本法典指定之正式工具，悉數受本法典治理且須於本機環境運作；禁止以非正式工具替代。", prohibition="禁止以非正式或未經法典宣告之工具替代正式工具"),
    ),
    edicts=(
        ...
        ChineseCodexEdict(id="E21", area="architecture", edict="系統程式碼一律採用 Python 與 C++ 之混合架構；PostgreSQL、Qdrant、Git、RAG 為受治理之正式工具；任何程式碼不得以其他程式語言撰寫。"),
        ChineseCodexEdict(id="E22", area="codex-reference", edict="中文版法典僅作為備用參考，不具被引用為判定依據之地位；一切判定以法典正式權威本為唯一依據。"),
        ChineseCodexEdict(id="E23", area="code-origin", edict="系統主宰及一切系統模組僅使用本機自有程式碼，Python 與 C++ 混合架構為唯一允許之程式語言；PostgreSQL、Qdrant、Git、RAG 為受治理之正式工具，禁止依賴任何非正式之第三方軟體、套件或外部服務。"),
        ...
        ChineseCodexEdict(id="E30", area="four-functions-local", edict="Git、PostgreSQL、語意索引、RAG 與 LLM 為正式工具與功能，由本機 Python 與 C++ 程式碼治理；無法本機治理即宣告關閉，不以外部服務替代。"),
        ChineseCodexEdict(id="E31", area="interface-layer", edict="使用者介面層僅為呈現，無決策、無執行；技術棧僅限 Python 與 C++；效力含所有模組與一切執行。"),
        ...
        ChineseCodexEdict(id="E35", area="formal-tools", edict="PostgreSQL、Qdrant、Git、RAG、Python、C++ 為本法典指定之正式工具，悉數受本法典治理，禁止以非正式工具替代。"),
    ),
