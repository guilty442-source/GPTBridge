...
GOVERNANCE_CODEX_CHINESE: Final[ChineseCodexReference] = ChineseCodexReference(
    ...
    articles=(
        ...
        ChineseCodexArticle(id="A48", section="七", subject="amendment-execution", rule="法典修訂採完整檔案替換執行：版本號採標準化小數點後5位制式，由系統自動遞增更新；替換須為治理權威（操作者）審慎明示之作為；替換後立即重新封印並於任何載入前恢復唯讀保護。", prohibition="禁止部分修補、跳號替換或未封印載入"),
    ...
    edicts=(
        ...
        ChineseCodexEdict(id="E34", area="amendment-execution", edict="法典修訂以完整檔案替換執行：版本號採標準化小數點後5位制式自動遞增更新、操作者審慎明示、替換後重新封印並恢復唯讀保護。"),
