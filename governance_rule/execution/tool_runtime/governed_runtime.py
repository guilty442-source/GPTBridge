        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                ...
                if command == LOCAL_CLEANUP_COMMAND:
                    # Module self-cleanup is a runtime-owned reserved command:
                    # every governed tool answers it uniformly without routing
                    # through its business executor. Only the governed
                    # maintenance scheduler (governance/main-system) may invoke
                    # it.
                    if (
                    ...
                    if not isinstance(result, dict):
                        raise permission_denied()
                    # The transport request id is authoritative. Business
                    # payloads may carry nested ids, but the websocket client
