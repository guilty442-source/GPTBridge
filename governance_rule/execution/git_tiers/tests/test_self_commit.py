...
def test_commit_lease_defers_sweep(scratch_repo: Path) -> None:
    """§10.69-E② single-writer: a fresh lease from another owner defers
    the sweep; foreign release must not clobber it; expiry is ignored."""
    import json
    import time
    ...
    (scratch_repo / "tracked.txt").write_text("v2\n", encoding="utf-8")
    assert claim_commit_lease(scratch_repo, owner="agent-1") is not None
    assert run_once(scratch_repo) == "commit-lease-held:agent-1"

    # A second writer must not overwrite the fresh lease.
    assert claim_commit_lease(scratch_repo, owner="agent-2") is None
    # A foreign release must not clobber it either.
    release_commit_lease(scratch_repo, owner="agent-2")
    assert run_once(scratch_repo) == "commit-lease-held:agent-1"

    release_commit_lease(scratch_repo, owner="agent-1")
    assert commit_lease_active(GitRepository(scratch_repo)) is None
    assert run_once(scratch_repo) == "committed"

    # Expired lease data is ignored.
    (scratch_repo / "tracked.txt").write_text("v3\n", encoding="utf-8")
    lease_path = scratch_repo / ".git" / "self-commit-lease.json"
    lease_path.write_text(
        json.dumps({"owner": "stale", "until": time.time() - 1}),
        encoding="utf-8",
    )
    assert commit_lease_active(GitRepository(scratch_repo)) is None
    assert run_once(scratch_repo) == "committed"
